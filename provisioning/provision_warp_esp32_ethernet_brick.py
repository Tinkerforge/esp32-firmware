...
def get_esp_ssid(serial_port, result, ssid_prefix):
    ...
    set_voltage_fuses, set_block_3, passphrase, uid = get_espefuse_tasks(override_port=serial_port, mac_address=mac_address)
...
def main():
    ...
    for k, v in list(relay_to_serial.items()):
        with contextlib.redirect_stdout(logs[k][0]):
            with contextlib.redirect_stderr(logs[k][1]):
                try:
