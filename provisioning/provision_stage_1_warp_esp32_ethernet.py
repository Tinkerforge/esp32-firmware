...
def main():
    ...
    test_bricklet_ports(ipcon, ESP_ETHERNET_DEVICE_ID, firmware_type in ("warp3"))
    ...
    while label_success != "y":
        run(["python3", "print-esp32-label.py", ssid, passphrase, "-c", "3" if firmware_type in ("warp3") else "1"])
        label_prompt = "Stick one label on the ESP, put ESP{} in the ESD bag. Press n to retry printing the label{}. [y/n]".format(
                " and the other two labels" if firmware_type in ("warp3") else "",
                "s" if firmware_type in ("warp3") else "")
