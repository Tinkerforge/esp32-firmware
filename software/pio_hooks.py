...
def main():
    ...
    # Build ordered list of status provider IDs from frontend_components
    status_provider_order = []
    for fc in frontend_components:
        if fc.mode is not None:  # Skip group markers
            continue
    ...
    # Add STATUS_PROVIDER_ORDER to options.ts (generated from custom_frontend_components)
    options_ts.append('// Status provider order (generated from custom_frontend_components)')
    options_ts.append(f'export const STATUS_PROVIDER_ORDER: string[] = {json.dumps(status_provider_order)};\n')
    ...
    metadata = json.dumps({
        ...
        'firmware_info_product_id': firmware_info_product_id,
        'firmware_info_product_name': firmware_info_product_name,
        'version': version,
        'build_timestamp': build_timestamp,
    }, separators=(',', ':'))

    with open(os.path.join(env.subst('$BUILD_DIR'), 'metadata.json'), 'w', encoding='utf-8') as f:
        f.write(metadata)
    ...
    # We assume all generated files are generated by this point
    if not build:
        return

    if env.GetProjectOption('custom_generate_compiledb', 'false') == 'true' and 'compiledb' not in COMMAND_LINE_TARGETS:
        print("Generating compile_commands.json")
        subprocess.run(["uv", "run", "pio", "run", "-s", "-e", env.subst('$PIOENV'), "-t", "compiledb"], check=True)

        print("Patching compile_commands.json")
        toolchain_dir = str(pathlib.Path(shutil.which('xtensa-esp32-elf-g++')).parent)

        replaced = re.sub(
             '\n        "command": "xtensa',
            f'\n        "command": "{toolchain_dir}{os.sep}xtensa',
            pathlib.Path('compile_commands.json').read_text())

        pathlib.Path('compile_commands.json').write_text(replaced)
    ...
    if not index_html_needs_update and os.path.exists('src/index_html.embedded.h') and os.path.exists('src/index_html.embedded.cpp'):
        ...
        util.embed_data(util.gzip_compress(html_bytes), 'src', 'index_html', 'char', 'size_t')
