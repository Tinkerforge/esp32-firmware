...
static bool tf_solid_state_relay_v2_callback_handler(void *device, uint8_t fid, TF_PacketBuffer *payload) {
    ...
    switch (fid) {
        case TF_SOLID_STATE_RELAY_V2_CALLBACK_MONOFLOP_DONE: {
            TF_SolidStateRelayV2_MonoflopDoneHandler fn = solid_state_relay_v2->monoflop_done_handler;
...
int tf_solid_state_relay_v2_create(TF_SolidStateRelayV2 *solid_state_relay_v2, const char *uid_or_port_name, TF_HAL *hal) {
    ...
    TF_TFP *tfp;
    int rc = tf_hal_get_attachable_tfp(hal, &tfp, uid_or_port_name, TF_SOLID_STATE_RELAY_V2_DEVICE_IDENTIFIER);
...
int tf_solid_state_relay_v2_set_state(TF_SolidStateRelayV2 *solid_state_relay_v2, bool state) {
    ...
    tf_solid_state_relay_v2_get_response_expected(solid_state_relay_v2, TF_SOLID_STATE_RELAY_V2_FUNCTION_SET_STATE, &_response_expected);
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_SET_STATE, 1, _response_expected);

    uint8_t *_send_buf = tf_tfp_get_send_payload_buffer(solid_state_relay_v2->tfp);

    _send_buf[0] = state ? 1 : 0;

    uint32_t _deadline = tf_hal_current_time_us(_hal) + tf_hal_get_common(_hal)->timeout;

    uint8_t _error_code = 0;
    uint8_t _length = 0;
    int _result = tf_tfp_send_packet(solid_state_relay_v2->tfp, _response_expected, _deadline, &_error_code, &_length, TF_NEW_PACKET);
...
int tf_solid_state_relay_v2_get_state(TF_SolidStateRelayV2 *solid_state_relay_v2, bool *ret_state) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_GET_STATE, 0, _response_expected);
    ...
    if (_result & TF_TICK_PACKET_RECEIVED) {
        TF_PacketBuffer *_recv_buf = tf_tfp_get_receive_buffer(solid_state_relay_v2->tfp);
        if (_error_code != 0 || _length != 1) {
            tf_packet_buffer_remove(_recv_buf, _length);
        } else {
            if (ret_state != NULL) { *ret_state = tf_packet_buffer_read_bool(_recv_buf); } else { tf_packet_buffer_remove(_recv_buf, 1); }
        }
        tf_tfp_packet_processed(solid_state_relay_v2->tfp);
    }
...
int tf_solid_state_relay_v2_set_monoflop(TF_SolidStateRelayV2 *solid_state_relay_v2, bool state, uint32_t time) {
    ...
    bool _response_expected = true;
    tf_solid_state_relay_v2_get_response_expected(solid_state_relay_v2, TF_SOLID_STATE_RELAY_V2_FUNCTION_SET_MONOFLOP, &_response_expected);
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_SET_MONOFLOP, 5, _response_expected);

    uint8_t *_send_buf = tf_tfp_get_send_payload_buffer(solid_state_relay_v2->tfp);

    _send_buf[0] = state ? 1 : 0;
    time = tf_leconvert_uint32_to(time); memcpy(_send_buf + 1, &time, 4);
...
int tf_solid_state_relay_v2_get_monoflop(TF_SolidStateRelayV2 *solid_state_relay_v2, bool *ret_state, uint32_t *ret_time, uint32_t *ret_time_remaining) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_GET_MONOFLOP, 0, _response_expected);
...
int tf_solid_state_relay_v2_get_spitfp_error_count(TF_SolidStateRelayV2 *solid_state_relay_v2, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_GET_SPITFP_ERROR_COUNT, 0, _response_expected);
...
int tf_solid_state_relay_v2_set_bootloader_mode(TF_SolidStateRelayV2 *solid_state_relay_v2, uint8_t mode, uint8_t *ret_status) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_SET_BOOTLOADER_MODE, 1, _response_expected);
...
int tf_solid_state_relay_v2_get_bootloader_mode(TF_SolidStateRelayV2 *solid_state_relay_v2, uint8_t *ret_mode) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_GET_BOOTLOADER_MODE, 0, _response_expected);
...
int tf_solid_state_relay_v2_set_write_firmware_pointer(TF_SolidStateRelayV2 *solid_state_relay_v2, uint32_t pointer) {
    ...
    bool _response_expected = true;
    tf_solid_state_relay_v2_get_response_expected(solid_state_relay_v2, TF_SOLID_STATE_RELAY_V2_FUNCTION_SET_WRITE_FIRMWARE_POINTER, &_response_expected);
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_SET_WRITE_FIRMWARE_POINTER, 4, _response_expected);
...
int tf_solid_state_relay_v2_write_firmware(TF_SolidStateRelayV2 *solid_state_relay_v2, const uint8_t data[64], uint8_t *ret_status) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_WRITE_FIRMWARE, 64, _response_expected);
...
int tf_solid_state_relay_v2_set_status_led_config(TF_SolidStateRelayV2 *solid_state_relay_v2, uint8_t config) {
    ...
    bool _response_expected = true;
    tf_solid_state_relay_v2_get_response_expected(solid_state_relay_v2, TF_SOLID_STATE_RELAY_V2_FUNCTION_SET_STATUS_LED_CONFIG, &_response_expected);
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_SET_STATUS_LED_CONFIG, 1, _response_expected);
...
int tf_solid_state_relay_v2_get_status_led_config(TF_SolidStateRelayV2 *solid_state_relay_v2, uint8_t *ret_config) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_GET_STATUS_LED_CONFIG, 0, _response_expected);
...
int tf_solid_state_relay_v2_get_chip_temperature(TF_SolidStateRelayV2 *solid_state_relay_v2, int16_t *ret_temperature) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_GET_CHIP_TEMPERATURE, 0, _response_expected);
...
int tf_solid_state_relay_v2_reset(TF_SolidStateRelayV2 *solid_state_relay_v2) {
    ...
    bool _response_expected = true;
    tf_solid_state_relay_v2_get_response_expected(solid_state_relay_v2, TF_SOLID_STATE_RELAY_V2_FUNCTION_RESET, &_response_expected);
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_RESET, 0, _response_expected);
...
int tf_solid_state_relay_v2_write_uid(TF_SolidStateRelayV2 *solid_state_relay_v2, uint32_t uid) {
    ...
    bool _response_expected = true;
    tf_solid_state_relay_v2_get_response_expected(solid_state_relay_v2, TF_SOLID_STATE_RELAY_V2_FUNCTION_WRITE_UID, &_response_expected);
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_WRITE_UID, 4, _response_expected);
...
int tf_solid_state_relay_v2_read_uid(TF_SolidStateRelayV2 *solid_state_relay_v2, uint32_t *ret_uid) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_READ_UID, 0, _response_expected);
...
int tf_solid_state_relay_v2_get_identity(TF_SolidStateRelayV2 *solid_state_relay_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(solid_state_relay_v2->tfp, TF_SOLID_STATE_RELAY_V2_FUNCTION_GET_IDENTITY, 0, _response_expected);
