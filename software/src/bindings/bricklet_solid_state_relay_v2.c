...
static bool tf_solid_state_relay_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_SOLID_STATE_RELAY_V2_CALLBACK_MONOFLOP_DONE: {
            ...
            bool state = tf_packetbuffer_read_bool(payload);
            TF_HalCommon *common = tf_hal_get_common((TF_HalContext*)solid_state_relay_v2->tfp->hal);
            common->locked = true;
...
int tf_solid_state_relay_v2_set_state(TF_SolidStateRelayV2 *solid_state_relay_v2, bool state) {
    ...
    if(tf_hal_get_common((TF_HalContext*)solid_state_relay_v2->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HalContext*)solid_state_relay_v2->tfp->hal) + tf_hal_get_common((TF_HalContext*)solid_state_relay_v2->tfp->hal)->timeout;
...
int tf_solid_state_relay_v2_get_state(TF_SolidStateRelayV2 *solid_state_relay_v2, bool *ret_state) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_state != NULL) { *ret_state = tf_packetbuffer_read_bool(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 1); }
...
int tf_solid_state_relay_v2_get_monoflop(TF_SolidStateRelayV2 *solid_state_relay_v2, bool *ret_state, uint32_t *ret_time, uint32_t *ret_time_remaining) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_state != NULL) { *ret_state = tf_packetbuffer_read_bool(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_time != NULL) { *ret_time = tf_packetbuffer_read_uint32_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_time_remaining != NULL) { *ret_time_remaining = tf_packetbuffer_read_uint32_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(solid_state_relay_v2->tfp);
    }
...
int tf_solid_state_relay_v2_get_spitfp_error_count(TF_SolidStateRelayV2 *solid_state_relay_v2, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packetbuffer_read_uint32_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packetbuffer_read_uint32_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packetbuffer_read_uint32_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packetbuffer_read_uint32_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(solid_state_relay_v2->tfp);
    }
...
int tf_solid_state_relay_v2_set_bootloader_mode(TF_SolidStateRelayV2 *solid_state_relay_v2, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packetbuffer_read_uint8_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(solid_state_relay_v2->tfp);
    }
...
int tf_solid_state_relay_v2_get_bootloader_mode(TF_SolidStateRelayV2 *solid_state_relay_v2, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packetbuffer_read_uint8_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(solid_state_relay_v2->tfp);
    }
...
int tf_solid_state_relay_v2_get_status_led_config(TF_SolidStateRelayV2 *solid_state_relay_v2, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packetbuffer_read_uint8_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(solid_state_relay_v2->tfp);
    }
...
int tf_solid_state_relay_v2_get_chip_temperature(TF_SolidStateRelayV2 *solid_state_relay_v2, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packetbuffer_read_int16_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(solid_state_relay_v2->tfp);
    }
...
int tf_solid_state_relay_v2_read_uid(TF_SolidStateRelayV2 *solid_state_relay_v2, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packetbuffer_read_uint32_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(solid_state_relay_v2->tfp);
    }
...
int tf_solid_state_relay_v2_get_identity(TF_SolidStateRelayV2 *solid_state_relay_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packetbuffer_pop_n(&solid_state_relay_v2->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 8); }
        tf_packetbuffer_pop_n(&solid_state_relay_v2->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packetbuffer_read_char(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packetbuffer_read_uint8_t(&solid_state_relay_v2->tfp->spitfp->recv_buf);} else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 3); }
        if (ret_firmware_version != NULL) { for (i = 0; i < 3; ++i) ret_firmware_version[i] = tf_packetbuffer_read_uint8_t(&solid_state_relay_v2->tfp->spitfp->recv_buf);} else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 3); }
        if (ret_device_identifier != NULL) { *ret_device_identifier = tf_packetbuffer_read_uint16_t(&solid_state_relay_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&solid_state_relay_v2->tfp->spitfp->recv_buf, 2); }
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HalContext*)solid_state_relay_v2->tfp->hal, solid_state_relay_v2->tfp->spitfp->port_id);
        }
...
int tf_solid_state_relay_v2_callback_tick(TF_SolidStateRelayV2 *solid_state_relay_v2, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(solid_state_relay_v2->tfp, tf_hal_current_time_us((TF_HalContext*)solid_state_relay_v2->tfp->hal) + timeout_us);
}
