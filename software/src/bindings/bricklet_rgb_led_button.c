...
int tf_rgb_led_button_create(TF_RGBLEDButton *rgb_led_button, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_RGB_LED_BUTTON_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_RGB_LED_BUTTON_DEVICE_IDENTIFIER;
...
int tf_rgb_led_button_set_color(TF_RGBLEDButton *rgb_led_button, uint8_t red, uint8_t green, uint8_t blue) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(rgb_led_button->tfp);

    send_buf[0] = (uint8_t)red;
    send_buf[1] = (uint8_t)green;
    send_buf[2] = (uint8_t)blue;

    uint32_t deadline = tf_hal_current_time_us(hal) + tf_hal_get_common(hal)->timeout;

    uint8_t error_code = 0;
    int result = tf_tfp_send_packet(rgb_led_button->tfp, response_expected, deadline, &error_code);
...
int tf_rgb_led_button_get_color(TF_RGBLEDButton *rgb_led_button, uint8_t *ret_red, uint8_t *ret_green, uint8_t *ret_blue) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(rgb_led_button->tfp);
        if (ret_red != NULL) { *ret_red = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_green != NULL) { *ret_green = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_blue != NULL) { *ret_blue = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
