...
int tf_rgb_led_button_create(TF_RGBLEDButton *rgb_led_button, const char *uid, TF_HAL *hal) {
    ...
    rc = tf_hal_get_tfp(hal, &rgb_led_button->tfp, TF_RGB_LED_BUTTON_DEVICE_IDENTIFIER, inventory_index);
    ...
    rgb_led_button->tfp->uid = numeric_uid;
...
int tf_rgb_led_button_get_color(TF_RGBLEDButton *rgb_led_button, uint8_t *ret_red, uint8_t *ret_green, uint8_t *ret_blue) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_red != NULL) { *ret_red = tf_packet_buffer_read_uint8_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 1); }
        if (ret_green != NULL) { *ret_green = tf_packet_buffer_read_uint8_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 1); }
        if (ret_blue != NULL) { *ret_blue = tf_packet_buffer_read_uint8_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 1); }
...
int tf_rgb_led_button_get_button_state(TF_RGBLEDButton *rgb_led_button, uint8_t *ret_state) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_state != NULL) { *ret_state = tf_packet_buffer_read_uint8_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(rgb_led_button->tfp);
    }
...
int tf_rgb_led_button_get_spitfp_error_count(TF_RGBLEDButton *rgb_led_button, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packet_buffer_read_uint32_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packet_buffer_read_uint32_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packet_buffer_read_uint32_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packet_buffer_read_uint32_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(rgb_led_button->tfp);
    }
...
int tf_rgb_led_button_set_bootloader_mode(TF_RGBLEDButton *rgb_led_button, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packet_buffer_read_uint8_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(rgb_led_button->tfp);
    }
...
int tf_rgb_led_button_get_bootloader_mode(TF_RGBLEDButton *rgb_led_button, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packet_buffer_read_uint8_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(rgb_led_button->tfp);
    }
...
int tf_rgb_led_button_get_status_led_config(TF_RGBLEDButton *rgb_led_button, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(rgb_led_button->tfp);
    }
...
int tf_rgb_led_button_get_chip_temperature(TF_RGBLEDButton *rgb_led_button, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packet_buffer_read_int16_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(rgb_led_button->tfp);
    }
...
int tf_rgb_led_button_read_uid(TF_RGBLEDButton *rgb_led_button, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packet_buffer_read_uint32_t(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(rgb_led_button->tfp);
    }
...
int tf_rgb_led_button_get_identity(TF_RGBLEDButton *rgb_led_button, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packet_buffer_pop_n(&rgb_led_button->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 8); }
        tf_packet_buffer_pop_n(&rgb_led_button->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packet_buffer_read_char(&rgb_led_button->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packet_buffer_read_uint8_t(&rgb_led_button->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&rgb_led_button->tfp->spitfp->recv_buf, 3); }
