...
int tf_evse_v2_create(TF_EVSEV2 *evse_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_EVSE_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_EVSE_V2_DEVICE_IDENTIFIER;
...
int tf_evse_v2_get_state(TF_EVSEV2 *evse_v2, uint8_t *ret_iec61851_state, uint8_t *ret_vehicle_state, uint8_t *ret_contactor_state, uint8_t *ret_contactor_error, uint8_t *ret_charge_release, uint16_t *ret_allowed_charging_current, uint8_t *ret_error_state, uint8_t *ret_lock_state, uint32_t *ret_time_since_state_change, uint32_t *ret_uptime) {
    ...
    int result = tf_tfp_send_packet(evse_v2->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_iec61851_state != NULL) { *ret_iec61851_state = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_vehicle_state != NULL) { *ret_vehicle_state = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_contactor_state != NULL) { *ret_contactor_state = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_contactor_error != NULL) { *ret_contactor_error = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_charge_release != NULL) { *ret_charge_release = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_allowed_charging_current != NULL) { *ret_allowed_charging_current = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_error_state != NULL) { *ret_error_state = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_lock_state != NULL) { *ret_lock_state = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_time_since_state_change != NULL) { *ret_time_since_state_change = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_uptime != NULL) { *ret_uptime = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
...
int tf_evse_v2_get_hardware_configuration(TF_EVSEV2 *evse_v2, uint8_t *ret_jumper_configuration, bool *ret_has_lock_switch) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_jumper_configuration != NULL) { *ret_jumper_configuration = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_has_lock_switch != NULL) { *ret_has_lock_switch = tf_packet_buffer_read_bool(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_low_level_state(TF_EVSEV2 *evse_v2, uint8_t *ret_led_state, uint16_t *ret_cp_pwm_duty_cycle, uint16_t ret_adc_values[7], int16_t ret_voltages[7], uint32_t ret_resistances[2], bool ret_gpio[24], uint32_t *ret_charging_time) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_led_state != NULL) { *ret_led_state = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_cp_pwm_duty_cycle != NULL) { *ret_cp_pwm_duty_cycle = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_adc_values != NULL) { for (i = 0; i < 7; ++i) ret_adc_values[i] = tf_packet_buffer_read_uint16_t(recv_buf);} else { tf_packet_buffer_remove(recv_buf, 14); }
        if (ret_voltages != NULL) { for (i = 0; i < 7; ++i) ret_voltages[i] = tf_packet_buffer_read_int16_t(recv_buf);} else { tf_packet_buffer_remove(recv_buf, 14); }
        if (ret_resistances != NULL) { for (i = 0; i < 2; ++i) ret_resistances[i] = tf_packet_buffer_read_uint32_t(recv_buf);} else { tf_packet_buffer_remove(recv_buf, 8); }
        if (ret_gpio != NULL) { tf_packet_buffer_read_bool_array(recv_buf, ret_gpio, 24);} else { tf_packet_buffer_remove(recv_buf, 3); }
        if (ret_charging_time != NULL) { *ret_charging_time = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_set_max_charging_current(TF_EVSEV2 *evse_v2, uint16_t max_current) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(evse_v2->tfp);

    max_current = tf_leconvert_uint16_to(max_current); memcpy(send_buf + 0, &max_current, 2);
...
int tf_evse_v2_get_max_charging_current(TF_EVSEV2 *evse_v2, uint16_t *ret_max_current_configured, uint16_t *ret_max_current_incoming_cable, uint16_t *ret_max_current_outgoing_cable, uint16_t *ret_max_current_managed) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_max_current_configured != NULL) { *ret_max_current_configured = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_max_current_incoming_cable != NULL) { *ret_max_current_incoming_cable = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_max_current_outgoing_cable != NULL) { *ret_max_current_outgoing_cable = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_max_current_managed != NULL) { *ret_max_current_managed = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_charging_autostart(TF_EVSEV2 *evse_v2, bool *ret_autostart) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_autostart != NULL) { *ret_autostart = tf_packet_buffer_read_bool(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_energy_meter_values(TF_EVSEV2 *evse_v2, float *ret_power, float *ret_energy_relative, float *ret_energy_absolute, bool ret_phases_active[3], bool ret_phases_connected[3]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_power != NULL) { *ret_power = tf_packet_buffer_read_float(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_energy_relative != NULL) { *ret_energy_relative = tf_packet_buffer_read_float(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_energy_absolute != NULL) { *ret_energy_absolute = tf_packet_buffer_read_float(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_phases_active != NULL) { tf_packet_buffer_read_bool_array(recv_buf, ret_phases_active, 3);} else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_phases_connected != NULL) { tf_packet_buffer_read_bool_array(recv_buf, ret_phases_connected, 3);} else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_energy_meter_detailed_values_low_level(TF_EVSEV2 *evse_v2, uint16_t *ret_values_chunk_offset, float ret_values_chunk_data[15]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_values_chunk_offset != NULL) { *ret_values_chunk_offset = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_values_chunk_data != NULL) { for (i = 0; i < 15; ++i) ret_values_chunk_data[i] = tf_packet_buffer_read_float(recv_buf);} else { tf_packet_buffer_remove(recv_buf, 60); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_energy_meter_state(TF_EVSEV2 *evse_v2, bool *ret_available, uint32_t ret_error_count[6]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_available != NULL) { *ret_available = tf_packet_buffer_read_bool(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_error_count != NULL) { for (i = 0; i < 6; ++i) ret_error_count[i] = tf_packet_buffer_read_uint32_t(recv_buf);} else { tf_packet_buffer_remove(recv_buf, 24); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_dc_fault_current_state(TF_EVSEV2 *evse_v2, uint8_t *ret_dc_fault_current_state) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_dc_fault_current_state != NULL) { *ret_dc_fault_current_state = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_reset_dc_fault_current(TF_EVSEV2 *evse_v2, uint32_t password) {
    ...
    password = tf_leconvert_uint32_to(password); memcpy(send_buf + 0, &password, 4);
...
int tf_evse_v2_set_gpio_configuration(TF_EVSEV2 *evse_v2, uint8_t shutdown_input_configuration, uint8_t input_configuration, uint8_t output_configuration) {
    ...
    send_buf[0] = (uint8_t)shutdown_input_configuration;
    send_buf[1] = (uint8_t)input_configuration;
    send_buf[2] = (uint8_t)output_configuration;
...
int tf_evse_v2_get_gpio_configuration(TF_EVSEV2 *evse_v2, uint8_t *ret_shutdown_input_configuration, uint8_t *ret_input_configuration, uint8_t *ret_output_configuration) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_shutdown_input_configuration != NULL) { *ret_shutdown_input_configuration = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_input_configuration != NULL) { *ret_input_configuration = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_output_configuration != NULL) { *ret_output_configuration = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_managed(TF_EVSEV2 *evse_v2, bool *ret_managed) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_managed != NULL) { *ret_managed = tf_packet_buffer_read_bool(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_set_managed(TF_EVSEV2 *evse_v2, bool managed, uint32_t password) {
    ...
    send_buf[0] = managed ? 1 : 0;
    password = tf_leconvert_uint32_to(password); memcpy(send_buf + 1, &password, 4);
...
int tf_evse_v2_get_data_storage(TF_EVSEV2 *evse_v2, uint8_t page, uint8_t ret_data[63]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_data != NULL) { for (i = 0; i < 63; ++i) ret_data[i] = tf_packet_buffer_read_uint8_t(recv_buf);} else { tf_packet_buffer_remove(recv_buf, 63); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_indicator_led(TF_EVSEV2 *evse_v2, int16_t *ret_indication, uint16_t *ret_duration) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_indication != NULL) { *ret_indication = tf_packet_buffer_read_int16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_duration != NULL) { *ret_duration = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_set_indicator_led(TF_EVSEV2 *evse_v2, int16_t indication, uint16_t duration, uint8_t *ret_status) {
    ...
    indication = tf_leconvert_int16_to(indication); memcpy(send_buf + 0, &indication, 2);
    duration = tf_leconvert_uint16_to(duration); memcpy(send_buf + 2, &duration, 2);
...
int tf_evse_v2_set_button_configuration(TF_EVSEV2 *evse_v2, uint8_t button_configuration) {
    ...
    send_buf[0] = (uint8_t)button_configuration;
...
int tf_evse_v2_get_button_configuration(TF_EVSEV2 *evse_v2, uint8_t *ret_button_configuration) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_button_configuration != NULL) { *ret_button_configuration = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_button_state(TF_EVSEV2 *evse_v2, uint32_t *ret_button_press_time, uint32_t *ret_button_release_time, bool *ret_button_pressed) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_button_press_time != NULL) { *ret_button_press_time = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_button_release_time != NULL) { *ret_button_release_time = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_button_pressed != NULL) { *ret_button_pressed = tf_packet_buffer_read_bool(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_set_control_pilot_configuration(TF_EVSEV2 *evse_v2, uint8_t control_pilot) {
    ...
    send_buf[0] = (uint8_t)control_pilot;
...
int tf_evse_v2_get_control_pilot_configuration(TF_EVSEV2 *evse_v2, uint8_t *ret_control_pilot) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(evse_v2->tfp);
        if (ret_control_pilot != NULL) { *ret_control_pilot = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_energy_meter_detailed_values(TF_EVSEV2 *evse_v2, float *ret_values, uint16_t *ret_values_length) {
    ...
    int ret = tf_stream_out(evse_v2, tf_evse_v2_get_energy_meter_detailed_values_ll_wrapper, NULL, ret_values, &values_length, values_chunk_data, 15, tf_copy_items_float);

    if (ret_values_length != NULL) {
        *ret_values_length = (uint16_t)values_length;
    }
    return ret;
}
