...
int tf_evse_v2_get_state(TF_EVSEV2 *evse_v2, uint8_t *ret_iec61851_state, uint8_t *ret_vehicle_state, uint8_t *ret_contactor_state, uint8_t *ret_contactor_error, uint8_t *ret_charge_release, uint16_t *ret_allowed_charging_current, uint8_t *ret_error_state, uint8_t *ret_lock_state, uint32_t *ret_time_since_state_change, uint32_t *ret_uptime) {
    ...
    if (tf_hal_get_common((TF_HAL *)evse_v2->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HAL *)evse_v2->tfp->hal) + tf_hal_get_common((TF_HAL *)evse_v2->tfp->hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_iec61851_state != NULL) { *ret_iec61851_state = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_vehicle_state != NULL) { *ret_vehicle_state = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_contactor_state != NULL) { *ret_contactor_state = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_contactor_error != NULL) { *ret_contactor_error = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_charge_release != NULL) { *ret_charge_release = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_allowed_charging_current != NULL) { *ret_allowed_charging_current = tf_packet_buffer_read_uint16_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_error_state != NULL) { *ret_error_state = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_lock_state != NULL) { *ret_lock_state = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_time_since_state_change != NULL) { *ret_time_since_state_change = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_uptime != NULL) { *ret_uptime = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
...
int tf_evse_v2_get_hardware_configuration(TF_EVSEV2 *evse_v2, uint8_t *ret_jumper_configuration, bool *ret_has_lock_switch) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_jumper_configuration != NULL) { *ret_jumper_configuration = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_has_lock_switch != NULL) { *ret_has_lock_switch = tf_packet_buffer_read_bool(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_low_level_state(TF_EVSEV2 *evse_v2, uint8_t *ret_led_state, uint16_t *ret_cp_pwm_duty_cycle, uint16_t ret_adc_values[7], int16_t ret_voltages[7], uint32_t ret_resistances[2], bool ret_gpio[24], uint32_t *ret_charging_time) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_led_state != NULL) { *ret_led_state = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_cp_pwm_duty_cycle != NULL) { *ret_cp_pwm_duty_cycle = tf_packet_buffer_read_uint16_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_adc_values != NULL) { for (i = 0; i < 7; ++i) ret_adc_values[i] = tf_packet_buffer_read_uint16_t(&evse_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 14); }
        if (ret_voltages != NULL) { for (i = 0; i < 7; ++i) ret_voltages[i] = tf_packet_buffer_read_int16_t(&evse_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 14); }
        if (ret_resistances != NULL) { for (i = 0; i < 2; ++i) ret_resistances[i] = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 8); }
        if (ret_gpio != NULL) { tf_packet_buffer_read_bool_array(&evse_v2->tfp->spitfp->recv_buf, ret_gpio, 24);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 3); }
        if (ret_charging_time != NULL) { *ret_charging_time = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_max_charging_current(TF_EVSEV2 *evse_v2, uint16_t *ret_max_current_configured, uint16_t *ret_max_current_incoming_cable, uint16_t *ret_max_current_outgoing_cable, uint16_t *ret_max_current_managed) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_max_current_configured != NULL) { *ret_max_current_configured = tf_packet_buffer_read_uint16_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_max_current_incoming_cable != NULL) { *ret_max_current_incoming_cable = tf_packet_buffer_read_uint16_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_max_current_outgoing_cable != NULL) { *ret_max_current_outgoing_cable = tf_packet_buffer_read_uint16_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_max_current_managed != NULL) { *ret_max_current_managed = tf_packet_buffer_read_uint16_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_charging_autostart(TF_EVSEV2 *evse_v2, bool *ret_autostart) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_autostart != NULL) { *ret_autostart = tf_packet_buffer_read_bool(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_energy_meter_values(TF_EVSEV2 *evse_v2, float *ret_power, float *ret_energy_relative, float *ret_energy_absolute, bool ret_phases_active[3], bool ret_phases_connected[3]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_power != NULL) { *ret_power = tf_packet_buffer_read_float(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_energy_relative != NULL) { *ret_energy_relative = tf_packet_buffer_read_float(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_energy_absolute != NULL) { *ret_energy_absolute = tf_packet_buffer_read_float(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_phases_active != NULL) { tf_packet_buffer_read_bool_array(&evse_v2->tfp->spitfp->recv_buf, ret_phases_active, 3);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_phases_connected != NULL) { tf_packet_buffer_read_bool_array(&evse_v2->tfp->spitfp->recv_buf, ret_phases_connected, 3);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_energy_meter_detailed_values_low_level(TF_EVSEV2 *evse_v2, uint16_t *ret_values_chunk_offset, float ret_values_chunk_data[15]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_values_chunk_offset != NULL) { *ret_values_chunk_offset = tf_packet_buffer_read_uint16_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_values_chunk_data != NULL) { for (i = 0; i < 15; ++i) ret_values_chunk_data[i] = tf_packet_buffer_read_float(&evse_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 60); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_energy_meter_state(TF_EVSEV2 *evse_v2, bool *ret_available, uint32_t ret_error_count[6]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_available != NULL) { *ret_available = tf_packet_buffer_read_bool(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_error_count != NULL) { for (i = 0; i < 6; ++i) ret_error_count[i] = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 24); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_dc_fault_current_state(TF_EVSEV2 *evse_v2, uint8_t *ret_dc_fault_current_state) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_dc_fault_current_state != NULL) { *ret_dc_fault_current_state = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_gpio_configuration(TF_EVSEV2 *evse_v2, uint8_t *ret_shutdown_input_configuration, uint8_t *ret_input_configuration, uint8_t *ret_output_configuration) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_shutdown_input_configuration != NULL) { *ret_shutdown_input_configuration = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_input_configuration != NULL) { *ret_input_configuration = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_output_configuration != NULL) { *ret_output_configuration = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_managed(TF_EVSEV2 *evse_v2, bool *ret_managed) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_managed != NULL) { *ret_managed = tf_packet_buffer_read_bool(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_data_storage(TF_EVSEV2 *evse_v2, uint8_t page, uint8_t ret_data[63]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_data != NULL) { for (i = 0; i < 63; ++i) ret_data[i] = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 63); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_indicator_led(TF_EVSEV2 *evse_v2, int16_t *ret_indication, uint16_t *ret_duration) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_indication != NULL) { *ret_indication = tf_packet_buffer_read_int16_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_duration != NULL) { *ret_duration = tf_packet_buffer_read_uint16_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_set_indicator_led(TF_EVSEV2 *evse_v2, int16_t indication, uint16_t duration, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_button_configuration(TF_EVSEV2 *evse_v2, uint8_t *ret_button_configuration) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_button_configuration != NULL) { *ret_button_configuration = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_button_state(TF_EVSEV2 *evse_v2, uint32_t *ret_button_press_time, uint32_t *ret_button_release_time, bool *ret_button_pressed) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_button_press_time != NULL) { *ret_button_press_time = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_button_release_time != NULL) { *ret_button_release_time = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_button_pressed != NULL) { *ret_button_pressed = tf_packet_buffer_read_bool(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_control_pilot_configuration(TF_EVSEV2 *evse_v2, uint8_t *ret_control_pilot) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_control_pilot != NULL) { *ret_control_pilot = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_spitfp_error_count(TF_EVSEV2 *evse_v2, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_bootloader_mode(TF_EVSEV2 *evse_v2, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_status_led_config(TF_EVSEV2 *evse_v2, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_chip_temperature(TF_EVSEV2 *evse_v2, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packet_buffer_read_int16_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_read_uid(TF_EVSEV2 *evse_v2, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packet_buffer_read_uint32_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(evse_v2->tfp);
    }
...
int tf_evse_v2_get_identity(TF_EVSEV2 *evse_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packet_buffer_pop_n(&evse_v2->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 8); }
        tf_packet_buffer_pop_n(&evse_v2->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packet_buffer_read_char(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 3); }
        if (ret_firmware_version != NULL) { for (i = 0; i < 3; ++i) ret_firmware_version[i] = tf_packet_buffer_read_uint8_t(&evse_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 3); }
        if (ret_device_identifier != NULL) { *ret_device_identifier = tf_packet_buffer_read_uint16_t(&evse_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&evse_v2->tfp->spitfp->recv_buf, 2); }
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HAL *)evse_v2->tfp->hal, evse_v2->tfp->spitfp->port_id);
        }
...
int tf_evse_v2_callback_tick(TF_EVSEV2 *evse_v2, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(evse_v2->tfp, tf_hal_current_time_us((TF_HAL *)evse_v2->tfp->hal) + timeout_us);
}
