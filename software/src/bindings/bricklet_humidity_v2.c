...
int tf_humidity_v2_create(TF_HumidityV2 *humidity_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_HUMIDITY_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_HUMIDITY_V2_DEVICE_IDENTIFIER;
...
int tf_humidity_v2_get_humidity(TF_HumidityV2 *humidity_v2, uint16_t *ret_humidity) {
    ...
    int result = tf_tfp_send_packet(humidity_v2->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(humidity_v2->tfp);
...
int tf_humidity_v2_set_humidity_callback_configuration(TF_HumidityV2 *humidity_v2, uint32_t period, bool value_has_to_change, char option, uint16_t min, uint16_t max) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(humidity_v2->tfp);
...
int tf_humidity_v2_set_heater_configuration(TF_HumidityV2 *humidity_v2, uint8_t heater_config) {
    ...
    send_buf[0] = (uint8_t)heater_config;
...
int tf_humidity_v2_get_heater_configuration(TF_HumidityV2 *humidity_v2, uint8_t *ret_heater_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(humidity_v2->tfp);
        if (ret_heater_config != NULL) { *ret_heater_config = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(humidity_v2->tfp);
    }
...
int tf_humidity_v2_set_moving_average_configuration(TF_HumidityV2 *humidity_v2, uint16_t moving_average_length_humidity, uint16_t moving_average_length_temperature) {
    ...
    moving_average_length_humidity = tf_leconvert_uint16_to(moving_average_length_humidity); memcpy(send_buf + 0, &moving_average_length_humidity, 2);
...
int tf_humidity_v2_get_moving_average_configuration(TF_HumidityV2 *humidity_v2, uint16_t *ret_moving_average_length_humidity, uint16_t *ret_moving_average_length_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(humidity_v2->tfp);
        if (ret_moving_average_length_humidity != NULL) { *ret_moving_average_length_humidity = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
...
int tf_humidity_v2_get_samples_per_second(TF_HumidityV2 *humidity_v2, uint8_t *ret_sps) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(humidity_v2->tfp);
        if (ret_sps != NULL) { *ret_sps = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(humidity_v2->tfp);
    }
