...
#endif
int tf_io4_v2_create(TF_IO4V2 *io4_v2, const char *uid, TF_HAL *hal) {
...
int tf_io4_v2_get_value(TF_IO4V2 *io4_v2, bool ret_value[4]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_value != NULL) { tf_packet_buffer_read_bool_array(&io4_v2->tfp->spitfp->recv_buf, ret_value, 4);} else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 1); }
...
int tf_io4_v2_get_configuration(TF_IO4V2 *io4_v2, uint8_t channel, char *ret_direction, bool *ret_value) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_direction != NULL) { *ret_direction = tf_packet_buffer_read_char(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_value != NULL) { *ret_value = tf_packet_buffer_read_bool(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_get_input_value_callback_configuration(TF_IO4V2 *io4_v2, uint8_t channel, uint32_t *ret_period, bool *ret_value_has_to_change) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_period != NULL) { *ret_period = tf_packet_buffer_read_uint32_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_value_has_to_change != NULL) { *ret_value_has_to_change = tf_packet_buffer_read_bool(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_get_monoflop(TF_IO4V2 *io4_v2, uint8_t channel, bool *ret_value, uint32_t *ret_time, uint32_t *ret_time_remaining) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_value != NULL) { *ret_value = tf_packet_buffer_read_bool(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_time != NULL) { *ret_time = tf_packet_buffer_read_uint32_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_time_remaining != NULL) { *ret_time_remaining = tf_packet_buffer_read_uint32_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_get_edge_count(TF_IO4V2 *io4_v2, uint8_t channel, bool reset_counter, uint32_t *ret_count) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_count != NULL) { *ret_count = tf_packet_buffer_read_uint32_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_get_edge_count_configuration(TF_IO4V2 *io4_v2, uint8_t channel, uint8_t *ret_edge_type, uint8_t *ret_debounce) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_edge_type != NULL) { *ret_edge_type = tf_packet_buffer_read_uint8_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_debounce != NULL) { *ret_debounce = tf_packet_buffer_read_uint8_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_get_pwm_configuration(TF_IO4V2 *io4_v2, uint8_t channel, uint32_t *ret_frequency, uint16_t *ret_duty_cycle) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_frequency != NULL) { *ret_frequency = tf_packet_buffer_read_uint32_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_duty_cycle != NULL) { *ret_duty_cycle = tf_packet_buffer_read_uint16_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_get_spitfp_error_count(TF_IO4V2 *io4_v2, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packet_buffer_read_uint32_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packet_buffer_read_uint32_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packet_buffer_read_uint32_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packet_buffer_read_uint32_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_set_bootloader_mode(TF_IO4V2 *io4_v2, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packet_buffer_read_uint8_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_get_bootloader_mode(TF_IO4V2 *io4_v2, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packet_buffer_read_uint8_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_get_status_led_config(TF_IO4V2 *io4_v2, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_get_chip_temperature(TF_IO4V2 *io4_v2, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packet_buffer_read_int16_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_read_uid(TF_IO4V2 *io4_v2, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packet_buffer_read_uint32_t(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(io4_v2->tfp);
    }
...
int tf_io4_v2_get_identity(TF_IO4V2 *io4_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packet_buffer_pop_n(&io4_v2->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 8); }
        tf_packet_buffer_pop_n(&io4_v2->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packet_buffer_read_char(&io4_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packet_buffer_read_uint8_t(&io4_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&io4_v2->tfp->spitfp->recv_buf, 3); }
