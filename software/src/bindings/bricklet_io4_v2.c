...
int tf_io4_v2_create(TF_IO4V2 *io4_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_IO4_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_IO4_V2_DEVICE_IDENTIFIER;
...
int tf_io4_v2_set_value(TF_IO4V2 *io4_v2, const bool value[4]) {
    ...
    size_t i;
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(io4_v2->tfp);
    ...
    int result = tf_tfp_send_packet(io4_v2->tfp, response_expected, deadline, &error_code);
...
int tf_io4_v2_get_value(TF_IO4V2 *io4_v2, bool ret_value[4]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(io4_v2->tfp);
