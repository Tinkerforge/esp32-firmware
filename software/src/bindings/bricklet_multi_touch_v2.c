...
int tf_multi_touch_v2_create(TF_MultiTouchV2 *multi_touch_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_MULTI_TOUCH_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_MULTI_TOUCH_V2_DEVICE_IDENTIFIER;
...
int tf_multi_touch_v2_get_touch_state(TF_MultiTouchV2 *multi_touch_v2, bool ret_state[13]) {
    ...
    int result = tf_tfp_send_packet(multi_touch_v2->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(multi_touch_v2->tfp);
        if (ret_state != NULL) { tf_packet_buffer_read_bool_array(recv_buf, ret_state, 13);} else { tf_packet_buffer_remove(recv_buf, 2); }
...
int tf_multi_touch_v2_set_touch_state_callback_configuration(TF_MultiTouchV2 *multi_touch_v2, uint32_t period, bool value_has_to_change) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(multi_touch_v2->tfp);
...
int tf_multi_touch_v2_set_electrode_config(TF_MultiTouchV2 *multi_touch_v2, const bool enabled_electrodes[13]) {
    ...
    memset(send_buf + 0, 0, 2); for (i = 0; i < 13; ++i) send_buf[0 + (i / 8)] |= (enabled_electrodes[i] ? 1 : 0) << (i % 8);
...
int tf_multi_touch_v2_get_electrode_config(TF_MultiTouchV2 *multi_touch_v2, bool ret_enabled_electrodes[13]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(multi_touch_v2->tfp);
        if (ret_enabled_electrodes != NULL) { tf_packet_buffer_read_bool_array(recv_buf, ret_enabled_electrodes, 13);} else { tf_packet_buffer_remove(recv_buf, 2); }
        tf_tfp_packet_processed(multi_touch_v2->tfp);
    }
