...
int tf_multi_touch_v2_get_touch_state(TF_MultiTouchV2 *multi_touch_v2, bool ret_state[13]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_state != NULL) { tf_packet_buffer_read_bool_array(&multi_touch_v2->tfp->spitfp->recv_buf, ret_state, 13);} else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 2); }
...
int tf_multi_touch_v2_get_touch_state_callback_configuration(TF_MultiTouchV2 *multi_touch_v2, uint32_t *ret_period, bool *ret_value_has_to_change) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_period != NULL) { *ret_period = tf_packet_buffer_read_uint32_t(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_value_has_to_change != NULL) { *ret_value_has_to_change = tf_packet_buffer_read_bool(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(multi_touch_v2->tfp);
    }
...
int tf_multi_touch_v2_get_electrode_config(TF_MultiTouchV2 *multi_touch_v2, bool ret_enabled_electrodes[13]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_enabled_electrodes != NULL) { tf_packet_buffer_read_bool_array(&multi_touch_v2->tfp->spitfp->recv_buf, ret_enabled_electrodes, 13);} else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(multi_touch_v2->tfp);
    }
...
int tf_multi_touch_v2_get_electrode_sensitivity(TF_MultiTouchV2 *multi_touch_v2, uint8_t *ret_sensitivity) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_sensitivity != NULL) { *ret_sensitivity = tf_packet_buffer_read_uint8_t(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(multi_touch_v2->tfp);
    }
...
int tf_multi_touch_v2_get_touch_led_config(TF_MultiTouchV2 *multi_touch_v2, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(multi_touch_v2->tfp);
    }
...
int tf_multi_touch_v2_get_spitfp_error_count(TF_MultiTouchV2 *multi_touch_v2, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packet_buffer_read_uint32_t(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packet_buffer_read_uint32_t(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packet_buffer_read_uint32_t(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packet_buffer_read_uint32_t(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(multi_touch_v2->tfp);
    }
...
int tf_multi_touch_v2_set_bootloader_mode(TF_MultiTouchV2 *multi_touch_v2, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packet_buffer_read_uint8_t(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(multi_touch_v2->tfp);
    }
...
int tf_multi_touch_v2_get_bootloader_mode(TF_MultiTouchV2 *multi_touch_v2, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packet_buffer_read_uint8_t(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(multi_touch_v2->tfp);
    }
...
int tf_multi_touch_v2_get_chip_temperature(TF_MultiTouchV2 *multi_touch_v2, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packet_buffer_read_int16_t(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(multi_touch_v2->tfp);
    }
...
int tf_multi_touch_v2_read_uid(TF_MultiTouchV2 *multi_touch_v2, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packet_buffer_read_uint32_t(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(multi_touch_v2->tfp);
    }
...
int tf_multi_touch_v2_get_identity(TF_MultiTouchV2 *multi_touch_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packet_buffer_pop_n(&multi_touch_v2->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 8); }
        tf_packet_buffer_pop_n(&multi_touch_v2->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packet_buffer_read_char(&multi_touch_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packet_buffer_read_uint8_t(&multi_touch_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&multi_touch_v2->tfp->spitfp->recv_buf, 3); }
