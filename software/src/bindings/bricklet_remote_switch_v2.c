...
int tf_remote_switch_v2_create(TF_RemoteSwitchV2 *remote_switch_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_REMOTE_SWITCH_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_REMOTE_SWITCH_V2_DEVICE_IDENTIFIER;
...
int tf_remote_switch_v2_destroy(TF_RemoteSwitchV2 *remote_switch_v2) {
    if (remote_switch_v2 == NULL || remote_switch_v2->tfp == NULL) {
        return TF_E_NULL;
    }
...
int tf_remote_switch_v2_get_switching_state(TF_RemoteSwitchV2 *remote_switch_v2, uint8_t *ret_state) {
    ...
    int result = tf_tfp_send_packet(remote_switch_v2->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(remote_switch_v2->tfp);
...
int tf_remote_switch_v2_set_repeats(TF_RemoteSwitchV2 *remote_switch_v2, uint8_t repeats) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(remote_switch_v2->tfp);

    send_buf[0] = (uint8_t)repeats;
...
int tf_remote_switch_v2_get_repeats(TF_RemoteSwitchV2 *remote_switch_v2, uint8_t *ret_repeats) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(remote_switch_v2->tfp);
        if (ret_repeats != NULL) { *ret_repeats = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(remote_switch_v2->tfp);
    }
...
int tf_remote_switch_v2_switch_socket_a(TF_RemoteSwitchV2 *remote_switch_v2, uint8_t house_code, uint8_t receiver_code, uint8_t switch_to) {
    ...
    send_buf[0] = (uint8_t)house_code;
    send_buf[1] = (uint8_t)receiver_code;
    send_buf[2] = (uint8_t)switch_to;
...
int tf_remote_switch_v2_switch_socket_b(TF_RemoteSwitchV2 *remote_switch_v2, uint32_t address, uint8_t unit, uint8_t switch_to) {
    ...
    address = tf_leconvert_uint32_to(address); memcpy(send_buf + 0, &address, 4);
    send_buf[4] = (uint8_t)unit;
    send_buf[5] = (uint8_t)switch_to;
...
int tf_remote_switch_v2_switch_socket_c(TF_RemoteSwitchV2 *remote_switch_v2, char system_code, uint8_t device_code, uint8_t switch_to) {
    ...
    send_buf[0] = (uint8_t)system_code;
    send_buf[1] = (uint8_t)device_code;
    send_buf[2] = (uint8_t)switch_to;
...
int tf_remote_switch_v2_set_remote_configuration(TF_RemoteSwitchV2 *remote_switch_v2, uint8_t remote_type, uint16_t minimum_repeats, bool callback_enabled) {
    ...
    send_buf[0] = (uint8_t)remote_type;
    minimum_repeats = tf_leconvert_uint16_to(minimum_repeats); memcpy(send_buf + 1, &minimum_repeats, 2);
    send_buf[3] = callback_enabled ? 1 : 0;
...
int tf_remote_switch_v2_get_remote_configuration(TF_RemoteSwitchV2 *remote_switch_v2, uint8_t *ret_remote_type, uint16_t *ret_minimum_repeats, bool *ret_callback_enabled) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(remote_switch_v2->tfp);
        if (ret_remote_type != NULL) { *ret_remote_type = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_minimum_repeats != NULL) { *ret_minimum_repeats = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_callback_enabled != NULL) { *ret_callback_enabled = tf_packet_buffer_read_bool(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(remote_switch_v2->tfp);
    }
...
int tf_remote_switch_v2_get_remote_status_a(TF_RemoteSwitchV2 *remote_switch_v2, uint8_t *ret_house_code, uint8_t *ret_receiver_code, uint8_t *ret_switch_to, uint16_t *ret_repeats) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(remote_switch_v2->tfp);
        if (ret_house_code != NULL) { *ret_house_code = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_receiver_code != NULL) { *ret_receiver_code = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_switch_to != NULL) { *ret_switch_to = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_repeats != NULL) { *ret_repeats = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        tf_tfp_packet_processed(remote_switch_v2->tfp);
    }
...
int tf_remote_switch_v2_get_remote_status_b(TF_RemoteSwitchV2 *remote_switch_v2, uint32_t *ret_address, uint8_t *ret_unit, uint8_t *ret_switch_to, uint8_t *ret_dim_value, uint16_t *ret_repeats) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(remote_switch_v2->tfp);
        if (ret_address != NULL) { *ret_address = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_unit != NULL) { *ret_unit = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_switch_to != NULL) { *ret_switch_to = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_dim_value != NULL) { *ret_dim_value = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_repeats != NULL) { *ret_repeats = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        tf_tfp_packet_processed(remote_switch_v2->tfp);
    }
...
int tf_remote_switch_v2_get_remote_status_c(TF_RemoteSwitchV2 *remote_switch_v2, char *ret_system_code, uint8_t *ret_device_code, uint8_t *ret_switch_to, uint16_t *ret_repeats) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(remote_switch_v2->tfp);
        if (ret_system_code != NULL) { *ret_system_code = tf_packet_buffer_read_char(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_device_code != NULL) { *ret_device_code = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_switch_to != NULL) { *ret_switch_to = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_repeats != NULL) { *ret_repeats = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        tf_tfp_packet_processed(remote_switch_v2->tfp);
    }
