...
int tf_industrial_dual_relay_create(TF_IndustrialDualRelay *industrial_dual_relay, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_INDUSTRIAL_DUAL_RELAY_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_INDUSTRIAL_DUAL_RELAY_DEVICE_IDENTIFIER;
...
int tf_industrial_dual_relay_set_value(TF_IndustrialDualRelay *industrial_dual_relay, bool channel0, bool channel1) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(industrial_dual_relay->tfp);
    ...
    int result = tf_tfp_send_packet(industrial_dual_relay->tfp, response_expected, deadline, &error_code);
...
int tf_industrial_dual_relay_get_value(TF_IndustrialDualRelay *industrial_dual_relay, bool *ret_channel0, bool *ret_channel1) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(industrial_dual_relay->tfp);
