...
static bool tf_industrial_dual_relay_callback_handler(void *dev, uint8_t fid, TF_PacketBuffer *payload) {
    ...
    switch (fid) {
        ...
        case TF_INDUSTRIAL_DUAL_RELAY_CALLBACK_MONOFLOP_DONE: {
            ...
            TF_HALCommon *hal_common = tf_hal_get_common((TF_HAL*)industrial_dual_relay->tfp->hal);
...
#endif
int tf_industrial_dual_relay_create(TF_IndustrialDualRelay *industrial_dual_relay, const char *uid, TF_HAL *hal) {
    if (industrial_dual_relay == NULL || uid == NULL || hal == NULL) {
        return TF_E_NULL;
    }
...
int tf_industrial_dual_relay_set_value(TF_IndustrialDualRelay *industrial_dual_relay, bool channel0, bool channel1) {
    ...
    if (tf_hal_get_common((TF_HAL*)industrial_dual_relay->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HAL*)industrial_dual_relay->tfp->hal) + tf_hal_get_common((TF_HAL*)industrial_dual_relay->tfp->hal)->timeout;
...
int tf_industrial_dual_relay_get_value(TF_IndustrialDualRelay *industrial_dual_relay, bool *ret_channel0, bool *ret_channel1) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_channel0 != NULL) { *ret_channel0 = tf_packet_buffer_read_bool(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 1); }
        if (ret_channel1 != NULL) { *ret_channel1 = tf_packet_buffer_read_bool(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 1); }
...
int tf_industrial_dual_relay_get_monoflop(TF_IndustrialDualRelay *industrial_dual_relay, uint8_t channel, bool *ret_value, uint32_t *ret_time, uint32_t *ret_time_remaining) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_value != NULL) { *ret_value = tf_packet_buffer_read_bool(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 1); }
        if (ret_time != NULL) { *ret_time = tf_packet_buffer_read_uint32_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 4); }
        if (ret_time_remaining != NULL) { *ret_time_remaining = tf_packet_buffer_read_uint32_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(industrial_dual_relay->tfp);
    }
...
int tf_industrial_dual_relay_get_spitfp_error_count(TF_IndustrialDualRelay *industrial_dual_relay, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packet_buffer_read_uint32_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packet_buffer_read_uint32_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packet_buffer_read_uint32_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packet_buffer_read_uint32_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(industrial_dual_relay->tfp);
    }
...
int tf_industrial_dual_relay_set_bootloader_mode(TF_IndustrialDualRelay *industrial_dual_relay, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packet_buffer_read_uint8_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(industrial_dual_relay->tfp);
    }
...
int tf_industrial_dual_relay_get_bootloader_mode(TF_IndustrialDualRelay *industrial_dual_relay, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packet_buffer_read_uint8_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(industrial_dual_relay->tfp);
    }
...
int tf_industrial_dual_relay_get_status_led_config(TF_IndustrialDualRelay *industrial_dual_relay, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(industrial_dual_relay->tfp);
    }
...
int tf_industrial_dual_relay_get_chip_temperature(TF_IndustrialDualRelay *industrial_dual_relay, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packet_buffer_read_int16_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(industrial_dual_relay->tfp);
    }
...
int tf_industrial_dual_relay_read_uid(TF_IndustrialDualRelay *industrial_dual_relay, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packet_buffer_read_uint32_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(industrial_dual_relay->tfp);
    }
...
int tf_industrial_dual_relay_get_identity(TF_IndustrialDualRelay *industrial_dual_relay, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packet_buffer_pop_n(&industrial_dual_relay->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 8); }
        tf_packet_buffer_pop_n(&industrial_dual_relay->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packet_buffer_read_char(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packet_buffer_read_uint8_t(&industrial_dual_relay->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 3); }
        if (ret_firmware_version != NULL) { for (i = 0; i < 3; ++i) ret_firmware_version[i] = tf_packet_buffer_read_uint8_t(&industrial_dual_relay->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 3); }
        if (ret_device_identifier != NULL) { *ret_device_identifier = tf_packet_buffer_read_uint16_t(&industrial_dual_relay->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_dual_relay->tfp->spitfp->recv_buf, 2); }
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HAL*)industrial_dual_relay->tfp->hal, industrial_dual_relay->tfp->spitfp->port_id);
        }
...
#if TF_IMPLEMENT_CALLBACKS != 0
int tf_industrial_dual_relay_register_monoflop_done_callback(TF_IndustrialDualRelay *industrial_dual_relay, TF_IndustrialDualRelayMonoflopDoneHandler handler, void *user_data) {
    if (industrial_dual_relay == NULL) {
        return TF_E_NULL;
    }
...
int tf_industrial_dual_relay_callback_tick(TF_IndustrialDualRelay *industrial_dual_relay, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(industrial_dual_relay->tfp, tf_hal_current_time_us((TF_HAL*)industrial_dual_relay->tfp->hal) + timeout_us);
}
