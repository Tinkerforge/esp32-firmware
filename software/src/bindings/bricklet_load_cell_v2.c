...
int tf_load_cell_v2_get_weight(TF_LoadCellV2 *load_cell_v2, int32_t *ret_weight) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_weight != NULL) { *ret_weight = tf_packet_buffer_read_int32_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 4); }
...
int tf_load_cell_v2_get_weight_callback_configuration(TF_LoadCellV2 *load_cell_v2, uint32_t *ret_period, bool *ret_value_has_to_change, char *ret_option, int32_t *ret_min, int32_t *ret_max) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_period != NULL) { *ret_period = tf_packet_buffer_read_uint32_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_value_has_to_change != NULL) { *ret_value_has_to_change = tf_packet_buffer_read_bool(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_option != NULL) { *ret_option = tf_packet_buffer_read_char(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_min != NULL) { *ret_min = tf_packet_buffer_read_int32_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_max != NULL) { *ret_max = tf_packet_buffer_read_int32_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(load_cell_v2->tfp);
    }
...
int tf_load_cell_v2_get_moving_average(TF_LoadCellV2 *load_cell_v2, uint16_t *ret_average) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_average != NULL) { *ret_average = tf_packet_buffer_read_uint16_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(load_cell_v2->tfp);
    }
...
int tf_load_cell_v2_get_info_led_config(TF_LoadCellV2 *load_cell_v2, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(load_cell_v2->tfp);
    }
...
int tf_load_cell_v2_get_configuration(TF_LoadCellV2 *load_cell_v2, uint8_t *ret_rate, uint8_t *ret_gain) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_rate != NULL) { *ret_rate = tf_packet_buffer_read_uint8_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_gain != NULL) { *ret_gain = tf_packet_buffer_read_uint8_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(load_cell_v2->tfp);
    }
...
int tf_load_cell_v2_get_spitfp_error_count(TF_LoadCellV2 *load_cell_v2, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packet_buffer_read_uint32_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packet_buffer_read_uint32_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packet_buffer_read_uint32_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packet_buffer_read_uint32_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(load_cell_v2->tfp);
    }
...
int tf_load_cell_v2_set_bootloader_mode(TF_LoadCellV2 *load_cell_v2, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packet_buffer_read_uint8_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(load_cell_v2->tfp);
    }
...
int tf_load_cell_v2_get_bootloader_mode(TF_LoadCellV2 *load_cell_v2, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packet_buffer_read_uint8_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(load_cell_v2->tfp);
    }
...
int tf_load_cell_v2_get_chip_temperature(TF_LoadCellV2 *load_cell_v2, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packet_buffer_read_int16_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(load_cell_v2->tfp);
    }
...
int tf_load_cell_v2_read_uid(TF_LoadCellV2 *load_cell_v2, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packet_buffer_read_uint32_t(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(load_cell_v2->tfp);
    }
...
int tf_load_cell_v2_get_identity(TF_LoadCellV2 *load_cell_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packet_buffer_pop_n(&load_cell_v2->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 8); }
        tf_packet_buffer_pop_n(&load_cell_v2->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packet_buffer_read_char(&load_cell_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packet_buffer_read_uint8_t(&load_cell_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&load_cell_v2->tfp->spitfp->recv_buf, 3); }
