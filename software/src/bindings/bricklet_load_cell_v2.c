...
int tf_load_cell_v2_create(TF_LoadCellV2 *load_cell_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_LOAD_CELL_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_LOAD_CELL_V2_DEVICE_IDENTIFIER;
...
int tf_load_cell_v2_get_weight(TF_LoadCellV2 *load_cell_v2, int32_t *ret_weight) {
    ...
    int result = tf_tfp_send_packet(load_cell_v2->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(load_cell_v2->tfp);
        if (ret_weight != NULL) { *ret_weight = tf_packet_buffer_read_int32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
...
int tf_load_cell_v2_set_weight_callback_configuration(TF_LoadCellV2 *load_cell_v2, uint32_t period, bool value_has_to_change, char option, int32_t min, int32_t max) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(load_cell_v2->tfp);
...
int tf_load_cell_v2_set_moving_average(TF_LoadCellV2 *load_cell_v2, uint16_t average) {
    ...
    average = tf_leconvert_uint16_to(average); memcpy(send_buf + 0, &average, 2);
...
int tf_load_cell_v2_get_moving_average(TF_LoadCellV2 *load_cell_v2, uint16_t *ret_average) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(load_cell_v2->tfp);
        if (ret_average != NULL) { *ret_average = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        tf_tfp_packet_processed(load_cell_v2->tfp);
    }
...
int tf_load_cell_v2_calibrate(TF_LoadCellV2 *load_cell_v2, uint32_t weight) {
    ...
    weight = tf_leconvert_uint32_to(weight); memcpy(send_buf + 0, &weight, 4);
