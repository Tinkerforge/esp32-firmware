...
int tf_warp_energy_manager_get_contactor(TF_WARPEnergyManager *warp_energy_manager, bool *ret_value) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_value != NULL) { *ret_value = tf_packet_buffer_read_bool(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
...
int tf_warp_energy_manager_get_rgb_value(TF_WARPEnergyManager *warp_energy_manager, uint8_t *ret_r, uint8_t *ret_g, uint8_t *ret_b) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_r != NULL) { *ret_r = tf_packet_buffer_read_uint8_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        if (ret_g != NULL) { *ret_g = tf_packet_buffer_read_uint8_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        if (ret_b != NULL) { *ret_b = tf_packet_buffer_read_uint8_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_energy_meter_values(TF_WARPEnergyManager *warp_energy_manager, float *ret_power, float *ret_energy_relative, float *ret_energy_absolute, bool ret_phases_active[3], bool ret_phases_connected[3]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_power != NULL) { *ret_power = tf_packet_buffer_read_float(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 4); }
        if (ret_energy_relative != NULL) { *ret_energy_relative = tf_packet_buffer_read_float(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 4); }
        if (ret_energy_absolute != NULL) { *ret_energy_absolute = tf_packet_buffer_read_float(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 4); }
        if (ret_phases_active != NULL) { tf_packet_buffer_read_bool_array(&warp_energy_manager->tfp->spitfp->recv_buf, ret_phases_active, 3);} else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        if (ret_phases_connected != NULL) { tf_packet_buffer_read_bool_array(&warp_energy_manager->tfp->spitfp->recv_buf, ret_phases_connected, 3);} else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_energy_meter_detailed_values_low_level(TF_WARPEnergyManager *warp_energy_manager, uint16_t *ret_values_chunk_offset, float ret_values_chunk_data[15]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_values_chunk_offset != NULL) { *ret_values_chunk_offset = tf_packet_buffer_read_uint16_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 2); }
        if (ret_values_chunk_data != NULL) { for (i = 0; i < 15; ++i) ret_values_chunk_data[i] = tf_packet_buffer_read_float(&warp_energy_manager->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 60); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_energy_meter_state(TF_WARPEnergyManager *warp_energy_manager, bool *ret_available, uint32_t ret_error_count[6]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_available != NULL) { *ret_available = tf_packet_buffer_read_bool(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        if (ret_error_count != NULL) { for (i = 0; i < 6; ++i) ret_error_count[i] = tf_packet_buffer_read_uint32_t(&warp_energy_manager->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 24); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_input(TF_WARPEnergyManager *warp_energy_manager, bool ret_input[2]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_input != NULL) { tf_packet_buffer_read_bool_array(&warp_energy_manager->tfp->spitfp->recv_buf, ret_input, 2);} else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_output(TF_WARPEnergyManager *warp_energy_manager, bool *ret_output) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_output != NULL) { *ret_output = tf_packet_buffer_read_bool(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_input_configuration(TF_WARPEnergyManager *warp_energy_manager, uint8_t ret_input_configuration[2]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_input_configuration != NULL) { for (i = 0; i < 2; ++i) ret_input_configuration[i] = tf_packet_buffer_read_uint8_t(&warp_energy_manager->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_input_voltage(TF_WARPEnergyManager *warp_energy_manager, uint16_t *ret_voltage) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_voltage != NULL) { *ret_voltage = tf_packet_buffer_read_uint16_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_state(TF_WARPEnergyManager *warp_energy_manager, uint8_t *ret_contactor_check_state) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_contactor_check_state != NULL) { *ret_contactor_check_state = tf_packet_buffer_read_uint8_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_spitfp_error_count(TF_WARPEnergyManager *warp_energy_manager, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packet_buffer_read_uint32_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packet_buffer_read_uint32_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packet_buffer_read_uint32_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packet_buffer_read_uint32_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_set_bootloader_mode(TF_WARPEnergyManager *warp_energy_manager, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packet_buffer_read_uint8_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_bootloader_mode(TF_WARPEnergyManager *warp_energy_manager, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packet_buffer_read_uint8_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_status_led_config(TF_WARPEnergyManager *warp_energy_manager, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_chip_temperature(TF_WARPEnergyManager *warp_energy_manager, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packet_buffer_read_int16_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_read_uid(TF_WARPEnergyManager *warp_energy_manager, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packet_buffer_read_uint32_t(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(warp_energy_manager->tfp);
    }
...
int tf_warp_energy_manager_get_identity(TF_WARPEnergyManager *warp_energy_manager, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packet_buffer_pop_n(&warp_energy_manager->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 8); }
        tf_packet_buffer_pop_n(&warp_energy_manager->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packet_buffer_read_char(&warp_energy_manager->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packet_buffer_read_uint8_t(&warp_energy_manager->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&warp_energy_manager->tfp->spitfp->recv_buf, 3); }
