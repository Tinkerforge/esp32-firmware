...
static bool tf_distance_us_v2_callback_handler(void *device, uint8_t fid, TF_PacketBuffer *payload) {
    TF_DistanceUSV2 *distance_us_v2 = (TF_DistanceUSV2 *)device;
    TF_HALCommon *hal_common = tf_hal_get_common(distance_us_v2->tfp->spitfp->hal);
    (void)payload;
...
#endif
int tf_distance_us_v2_create(TF_DistanceUSV2 *distance_us_v2, const char *uid_or_port_name, TF_HAL *hal) {
    if (distance_us_v2 == NULL || hal == NULL) {
        return TF_E_NULL;
    }
    ...
    TF_TFP *tfp;
    int rc = tf_hal_get_attachable_tfp(hal, &tfp, uid_or_port_name, TF_DISTANCE_US_V2_DEVICE_IDENTIFIER);
...
int tf_distance_us_v2_get_distance(TF_DistanceUSV2 *distance_us_v2, uint16_t *ret_distance) {
    ...
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_GET_DISTANCE, 0, _response_expected);

    uint32_t _deadline = tf_hal_current_time_us(_hal) + tf_hal_get_common(_hal)->timeout;

    uint8_t _error_code = 0;
    uint8_t _length = 0;
    int _result = tf_tfp_send_packet(distance_us_v2->tfp, _response_expected, _deadline, &_error_code, &_length, TF_NEW_PACKET);
    ...
    if (_result & TF_TICK_PACKET_RECEIVED) {
        TF_PacketBuffer *_recv_buf = tf_tfp_get_receive_buffer(distance_us_v2->tfp);
    ...
    if (_result & TF_TICK_TIMEOUT) {
        _result = tf_tfp_finish_send(distance_us_v2->tfp, _result, _deadline);
        (void) _result;
        return TF_E_TIMEOUT;
    }
...
int tf_distance_us_v2_set_distance_callback_configuration(TF_DistanceUSV2 *distance_us_v2, uint32_t period, bool value_has_to_change, char option, uint16_t min, uint16_t max) {
    ...
    bool _response_expected = true;
    tf_distance_us_v2_get_response_expected(distance_us_v2, TF_DISTANCE_US_V2_FUNCTION_SET_DISTANCE_CALLBACK_CONFIGURATION, &_response_expected);
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_SET_DISTANCE_CALLBACK_CONFIGURATION, 10, _response_expected);

    uint8_t *_send_buf = tf_tfp_get_send_payload_buffer(distance_us_v2->tfp);
...
int tf_distance_us_v2_get_distance_callback_configuration(TF_DistanceUSV2 *distance_us_v2, uint32_t *ret_period, bool *ret_value_has_to_change, char *ret_option, uint16_t *ret_min, uint16_t *ret_max) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_GET_DISTANCE_CALLBACK_CONFIGURATION, 0, _response_expected);
...
int tf_distance_us_v2_set_update_rate(TF_DistanceUSV2 *distance_us_v2, uint8_t update_rate) {
    ...
    bool _response_expected = true;
    tf_distance_us_v2_get_response_expected(distance_us_v2, TF_DISTANCE_US_V2_FUNCTION_SET_UPDATE_RATE, &_response_expected);
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_SET_UPDATE_RATE, 1, _response_expected);

    uint8_t *_send_buf = tf_tfp_get_send_payload_buffer(distance_us_v2->tfp);

    _send_buf[0] = (uint8_t)update_rate;
...
int tf_distance_us_v2_get_update_rate(TF_DistanceUSV2 *distance_us_v2, uint8_t *ret_update_rate) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_GET_UPDATE_RATE, 0, _response_expected);
    ...
    if (_result & TF_TICK_PACKET_RECEIVED) {
        ...
        if (_error_code != 0 || _length != 1) {
            ...
            if (ret_update_rate != NULL) { *ret_update_rate = tf_packet_buffer_read_uint8_t(_recv_buf); } else { tf_packet_buffer_remove(_recv_buf, 1); }
        }
        tf_tfp_packet_processed(distance_us_v2->tfp);
    }
...
int tf_distance_us_v2_set_distance_led_config(TF_DistanceUSV2 *distance_us_v2, uint8_t config) {
    ...
    bool _response_expected = true;
    tf_distance_us_v2_get_response_expected(distance_us_v2, TF_DISTANCE_US_V2_FUNCTION_SET_DISTANCE_LED_CONFIG, &_response_expected);
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_SET_DISTANCE_LED_CONFIG, 1, _response_expected);
...
int tf_distance_us_v2_get_distance_led_config(TF_DistanceUSV2 *distance_us_v2, uint8_t *ret_config) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_GET_DISTANCE_LED_CONFIG, 0, _response_expected);
...
int tf_distance_us_v2_get_spitfp_error_count(TF_DistanceUSV2 *distance_us_v2, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_GET_SPITFP_ERROR_COUNT, 0, _response_expected);
...
int tf_distance_us_v2_set_bootloader_mode(TF_DistanceUSV2 *distance_us_v2, uint8_t mode, uint8_t *ret_status) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_SET_BOOTLOADER_MODE, 1, _response_expected);
...
int tf_distance_us_v2_get_bootloader_mode(TF_DistanceUSV2 *distance_us_v2, uint8_t *ret_mode) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_GET_BOOTLOADER_MODE, 0, _response_expected);
...
int tf_distance_us_v2_set_write_firmware_pointer(TF_DistanceUSV2 *distance_us_v2, uint32_t pointer) {
    ...
    bool _response_expected = true;
    tf_distance_us_v2_get_response_expected(distance_us_v2, TF_DISTANCE_US_V2_FUNCTION_SET_WRITE_FIRMWARE_POINTER, &_response_expected);
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_SET_WRITE_FIRMWARE_POINTER, 4, _response_expected);
...
int tf_distance_us_v2_write_firmware(TF_DistanceUSV2 *distance_us_v2, const uint8_t data[64], uint8_t *ret_status) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_WRITE_FIRMWARE, 64, _response_expected);
...
int tf_distance_us_v2_set_status_led_config(TF_DistanceUSV2 *distance_us_v2, uint8_t config) {
    ...
    bool _response_expected = true;
    tf_distance_us_v2_get_response_expected(distance_us_v2, TF_DISTANCE_US_V2_FUNCTION_SET_STATUS_LED_CONFIG, &_response_expected);
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_SET_STATUS_LED_CONFIG, 1, _response_expected);
...
int tf_distance_us_v2_get_status_led_config(TF_DistanceUSV2 *distance_us_v2, uint8_t *ret_config) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_GET_STATUS_LED_CONFIG, 0, _response_expected);
...
int tf_distance_us_v2_get_chip_temperature(TF_DistanceUSV2 *distance_us_v2, int16_t *ret_temperature) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_GET_CHIP_TEMPERATURE, 0, _response_expected);
...
int tf_distance_us_v2_reset(TF_DistanceUSV2 *distance_us_v2) {
    ...
    bool _response_expected = true;
    tf_distance_us_v2_get_response_expected(distance_us_v2, TF_DISTANCE_US_V2_FUNCTION_RESET, &_response_expected);
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_RESET, 0, _response_expected);
...
int tf_distance_us_v2_write_uid(TF_DistanceUSV2 *distance_us_v2, uint32_t uid) {
    ...
    bool _response_expected = true;
    tf_distance_us_v2_get_response_expected(distance_us_v2, TF_DISTANCE_US_V2_FUNCTION_WRITE_UID, &_response_expected);
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_WRITE_UID, 4, _response_expected);
...
int tf_distance_us_v2_read_uid(TF_DistanceUSV2 *distance_us_v2, uint32_t *ret_uid) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_READ_UID, 0, _response_expected);
...
int tf_distance_us_v2_get_identity(TF_DistanceUSV2 *distance_us_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(distance_us_v2->tfp, TF_DISTANCE_US_V2_FUNCTION_GET_IDENTITY, 0, _response_expected);
...
int tf_distance_us_v2_callback_tick(TF_DistanceUSV2 *distance_us_v2, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(distance_us_v2->tfp, tf_hal_current_time_us(hal) + timeout_us);
}
