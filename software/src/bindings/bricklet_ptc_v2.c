...
int tf_ptc_v2_create(TF_PTCV2 *ptc_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_PTC_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_PTC_V2_DEVICE_IDENTIFIER;
...
int tf_ptc_v2_destroy(TF_PTCV2 *ptc_v2) {
    if (ptc_v2 == NULL || ptc_v2->tfp == NULL) {
        return TF_E_NULL;
    }
...
int tf_ptc_v2_get_temperature(TF_PTCV2 *ptc_v2, int32_t *ret_temperature) {
    ...
    int result = tf_tfp_send_packet(ptc_v2->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(ptc_v2->tfp);
...
int tf_ptc_v2_set_temperature_callback_configuration(TF_PTCV2 *ptc_v2, uint32_t period, bool value_has_to_change, char option, int32_t min, int32_t max) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(ptc_v2->tfp);
