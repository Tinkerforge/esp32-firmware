...
static bool tf_dual_button_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_DUAL_BUTTON_V2_CALLBACK_STATE_CHANGED: {
            ...
            TF_HalCommon *common = tf_hal_get_common((TF_HalContext*)dual_button_v2->tfp->hal);
            common->locked = true;
...
int tf_dual_button_v2_set_led_state(TF_DualButtonV2 *dual_button_v2, uint8_t led_l, uint8_t led_r) {
    ...
    if(tf_hal_get_common((TF_HalContext*)dual_button_v2->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HalContext*)dual_button_v2->tfp->hal) + tf_hal_get_common((TF_HalContext*)dual_button_v2->tfp->hal)->timeout;
...
int tf_dual_button_v2_get_identity(TF_DualButtonV2 *dual_button_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        tf_packetbuffer_pop_n(&dual_button_v2->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HalContext*)dual_button_v2->tfp->hal, dual_button_v2->tfp->spitfp->port_id);
        }
...
int tf_dual_button_v2_callback_tick(TF_DualButtonV2 *dual_button_v2, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(dual_button_v2->tfp, tf_hal_current_time_us((TF_HalContext*)dual_button_v2->tfp->hal) + timeout_us);
}
