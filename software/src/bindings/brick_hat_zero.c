...
static bool tf_hat_zero_callback_handler(void *dev, uint8_t fid, TF_PacketBuffer *payload) {
    ...
    switch (fid) {
        case TF_HAT_ZERO_CALLBACK_USB_VOLTAGE: {
            ...
            TF_HALCommon *hal_common = tf_hal_get_common((TF_HAL *)hat_zero->tfp->hal);
...
int tf_hat_zero_get_usb_voltage(TF_HATZero *hat_zero, uint16_t *ret_voltage) {
    ...
    if (tf_hal_get_common((TF_HAL *)hat_zero->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HAL *)hat_zero->tfp->hal) + tf_hal_get_common((TF_HAL *)hat_zero->tfp->hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_voltage != NULL) { *ret_voltage = tf_packet_buffer_read_uint16_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 2); }
...
int tf_hat_zero_get_usb_voltage_callback_configuration(TF_HATZero *hat_zero, uint32_t *ret_period, bool *ret_value_has_to_change, char *ret_option, uint16_t *ret_min, uint16_t *ret_max) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_period != NULL) { *ret_period = tf_packet_buffer_read_uint32_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 4); }
        if (ret_value_has_to_change != NULL) { *ret_value_has_to_change = tf_packet_buffer_read_bool(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 1); }
        if (ret_option != NULL) { *ret_option = tf_packet_buffer_read_char(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 1); }
        if (ret_min != NULL) { *ret_min = tf_packet_buffer_read_uint16_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 2); }
        if (ret_max != NULL) { *ret_max = tf_packet_buffer_read_uint16_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(hat_zero->tfp);
    }
...
int tf_hat_zero_get_spitfp_error_count(TF_HATZero *hat_zero, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packet_buffer_read_uint32_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packet_buffer_read_uint32_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packet_buffer_read_uint32_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packet_buffer_read_uint32_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(hat_zero->tfp);
    }
...
int tf_hat_zero_set_bootloader_mode(TF_HATZero *hat_zero, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packet_buffer_read_uint8_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(hat_zero->tfp);
    }
...
int tf_hat_zero_get_bootloader_mode(TF_HATZero *hat_zero, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packet_buffer_read_uint8_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(hat_zero->tfp);
    }
...
int tf_hat_zero_get_status_led_config(TF_HATZero *hat_zero, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(hat_zero->tfp);
    }
...
int tf_hat_zero_get_chip_temperature(TF_HATZero *hat_zero, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packet_buffer_read_int16_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(hat_zero->tfp);
    }
...
int tf_hat_zero_read_uid(TF_HATZero *hat_zero, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packet_buffer_read_uint32_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(hat_zero->tfp);
    }
...
int tf_hat_zero_get_identity(TF_HATZero *hat_zero, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packet_buffer_pop_n(&hat_zero->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 8); }
        tf_packet_buffer_pop_n(&hat_zero->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packet_buffer_read_char(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packet_buffer_read_uint8_t(&hat_zero->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 3); }
        if (ret_firmware_version != NULL) { for (i = 0; i < 3; ++i) ret_firmware_version[i] = tf_packet_buffer_read_uint8_t(&hat_zero->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 3); }
        if (ret_device_identifier != NULL) { *ret_device_identifier = tf_packet_buffer_read_uint16_t(&hat_zero->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&hat_zero->tfp->spitfp->recv_buf, 2); }
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HAL *)hat_zero->tfp->hal, hat_zero->tfp->spitfp->port_id);
        }
...
int tf_hat_zero_callback_tick(TF_HATZero *hat_zero, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(hat_zero->tfp, tf_hal_current_time_us((TF_HAL *)hat_zero->tfp->hal) + timeout_us);
}
