...
int tf_real_time_clock_v2_create(TF_RealTimeClockV2 *real_time_clock_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_REAL_TIME_CLOCK_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_REAL_TIME_CLOCK_V2_DEVICE_IDENTIFIER;
...
int tf_real_time_clock_v2_set_date_time(TF_RealTimeClockV2 *real_time_clock_v2, uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t second, uint8_t centisecond, uint8_t weekday) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(real_time_clock_v2->tfp);

    year = tf_leconvert_uint16_to(year); memcpy(send_buf + 0, &year, 2);
    send_buf[2] = (uint8_t)month;
    send_buf[3] = (uint8_t)day;
    send_buf[4] = (uint8_t)hour;
    send_buf[5] = (uint8_t)minute;
    send_buf[6] = (uint8_t)second;
    send_buf[7] = (uint8_t)centisecond;
    send_buf[8] = (uint8_t)weekday;

    uint32_t deadline = tf_hal_current_time_us(hal) + tf_hal_get_common(hal)->timeout;

    uint8_t error_code = 0;
    int result = tf_tfp_send_packet(real_time_clock_v2->tfp, response_expected, deadline, &error_code);
...
int tf_real_time_clock_v2_get_date_time(TF_RealTimeClockV2 *real_time_clock_v2, uint16_t *ret_year, uint8_t *ret_month, uint8_t *ret_day, uint8_t *ret_hour, uint8_t *ret_minute, uint8_t *ret_second, uint8_t *ret_centisecond, uint8_t *ret_weekday, int64_t *ret_timestamp) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(real_time_clock_v2->tfp);
        if (ret_year != NULL) { *ret_year = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_month != NULL) { *ret_month = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_day != NULL) { *ret_day = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_hour != NULL) { *ret_hour = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_minute != NULL) { *ret_minute = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_second != NULL) { *ret_second = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_centisecond != NULL) { *ret_centisecond = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_weekday != NULL) { *ret_weekday = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_timestamp != NULL) { *ret_timestamp = tf_packet_buffer_read_int64_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 8); }
...
int tf_real_time_clock_v2_get_offset(TF_RealTimeClockV2 *real_time_clock_v2, int8_t *ret_offset) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(real_time_clock_v2->tfp);
        if (ret_offset != NULL) { *ret_offset = tf_packet_buffer_read_int8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(real_time_clock_v2->tfp);
    }
...
int tf_real_time_clock_v2_set_alarm(TF_RealTimeClockV2 *real_time_clock_v2, int8_t month, int8_t day, int8_t hour, int8_t minute, int8_t second, int8_t weekday, int32_t interval) {
    ...
    send_buf[0] = (uint8_t)month;
    send_buf[1] = (uint8_t)day;
    send_buf[2] = (uint8_t)hour;
    send_buf[3] = (uint8_t)minute;
    send_buf[4] = (uint8_t)second;
    send_buf[5] = (uint8_t)weekday;
    interval = tf_leconvert_int32_to(interval); memcpy(send_buf + 6, &interval, 4);
...
int tf_real_time_clock_v2_get_alarm(TF_RealTimeClockV2 *real_time_clock_v2, int8_t *ret_month, int8_t *ret_day, int8_t *ret_hour, int8_t *ret_minute, int8_t *ret_second, int8_t *ret_weekday, int32_t *ret_interval) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(real_time_clock_v2->tfp);
        if (ret_month != NULL) { *ret_month = tf_packet_buffer_read_int8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_day != NULL) { *ret_day = tf_packet_buffer_read_int8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_hour != NULL) { *ret_hour = tf_packet_buffer_read_int8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_minute != NULL) { *ret_minute = tf_packet_buffer_read_int8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_second != NULL) { *ret_second = tf_packet_buffer_read_int8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_weekday != NULL) { *ret_weekday = tf_packet_buffer_read_int8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_interval != NULL) { *ret_interval = tf_packet_buffer_read_int32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        tf_tfp_packet_processed(real_time_clock_v2->tfp);
    }
