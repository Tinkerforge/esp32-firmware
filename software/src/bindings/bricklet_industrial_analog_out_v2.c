...
int tf_industrial_analog_out_v2_create(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_INDUSTRIAL_ANALOG_OUT_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_INDUSTRIAL_ANALOG_OUT_V2_DEVICE_IDENTIFIER;
...
int tf_industrial_analog_out_v2_destroy(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2) {
    if (industrial_analog_out_v2 == NULL || industrial_analog_out_v2->tfp == NULL) {
        return TF_E_NULL;
    }
...
int tf_industrial_analog_out_v2_set_enabled(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, bool enabled) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(industrial_analog_out_v2->tfp);
    ...
    int result = tf_tfp_send_packet(industrial_analog_out_v2->tfp, response_expected, deadline, &error_code);
...
int tf_industrial_analog_out_v2_get_enabled(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, bool *ret_enabled) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(industrial_analog_out_v2->tfp);
...
int tf_industrial_analog_out_v2_set_configuration(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint8_t voltage_range, uint8_t current_range) {
    ...
    send_buf[0] = (uint8_t)voltage_range;
    send_buf[1] = (uint8_t)current_range;
...
int tf_industrial_analog_out_v2_get_configuration(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint8_t *ret_voltage_range, uint8_t *ret_current_range) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(industrial_analog_out_v2->tfp);
        if (ret_voltage_range != NULL) { *ret_voltage_range = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_current_range != NULL) { *ret_current_range = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(industrial_analog_out_v2->tfp);
    }
...
int tf_industrial_analog_out_v2_set_out_led_status_config(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint16_t min, uint16_t max, uint8_t config) {
    ...
    min = tf_leconvert_uint16_to(min); memcpy(send_buf + 0, &min, 2);
    max = tf_leconvert_uint16_to(max); memcpy(send_buf + 2, &max, 2);
    send_buf[4] = (uint8_t)config;
