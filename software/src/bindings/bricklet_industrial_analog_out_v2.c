...
int tf_industrial_analog_out_v2_set_enabled(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, bool enabled) {
    ...
    if (tf_hal_get_common((TF_HAL*)industrial_analog_out_v2->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HAL*)industrial_analog_out_v2->tfp->hal) + tf_hal_get_common((TF_HAL*)industrial_analog_out_v2->tfp->hal)->timeout;
...
int tf_industrial_analog_out_v2_get_enabled(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, bool *ret_enabled) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_enabled != NULL) { *ret_enabled = tf_packet_buffer_read_bool(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 1); }
...
int tf_industrial_analog_out_v2_get_voltage(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint16_t *ret_voltage) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_voltage != NULL) { *ret_voltage = tf_packet_buffer_read_uint16_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(industrial_analog_out_v2->tfp);
    }
...
int tf_industrial_analog_out_v2_get_current(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint16_t *ret_current) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_current != NULL) { *ret_current = tf_packet_buffer_read_uint16_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(industrial_analog_out_v2->tfp);
    }
...
int tf_industrial_analog_out_v2_get_configuration(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint8_t *ret_voltage_range, uint8_t *ret_current_range) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_voltage_range != NULL) { *ret_voltage_range = tf_packet_buffer_read_uint8_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_current_range != NULL) { *ret_current_range = tf_packet_buffer_read_uint8_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(industrial_analog_out_v2->tfp);
    }
...
int tf_industrial_analog_out_v2_get_out_led_config(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(industrial_analog_out_v2->tfp);
    }
...
int tf_industrial_analog_out_v2_get_out_led_status_config(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint16_t *ret_min, uint16_t *ret_max, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_min != NULL) { *ret_min = tf_packet_buffer_read_uint16_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_max != NULL) { *ret_max = tf_packet_buffer_read_uint16_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(industrial_analog_out_v2->tfp);
    }
...
int tf_industrial_analog_out_v2_get_spitfp_error_count(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packet_buffer_read_uint32_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packet_buffer_read_uint32_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packet_buffer_read_uint32_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packet_buffer_read_uint32_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(industrial_analog_out_v2->tfp);
    }
...
int tf_industrial_analog_out_v2_set_bootloader_mode(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packet_buffer_read_uint8_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(industrial_analog_out_v2->tfp);
    }
...
int tf_industrial_analog_out_v2_get_bootloader_mode(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packet_buffer_read_uint8_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(industrial_analog_out_v2->tfp);
    }
...
int tf_industrial_analog_out_v2_get_chip_temperature(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packet_buffer_read_int16_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(industrial_analog_out_v2->tfp);
    }
...
int tf_industrial_analog_out_v2_read_uid(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packet_buffer_read_uint32_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(industrial_analog_out_v2->tfp);
    }
...
int tf_industrial_analog_out_v2_get_identity(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packet_buffer_pop_n(&industrial_analog_out_v2->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 8); }
        tf_packet_buffer_pop_n(&industrial_analog_out_v2->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packet_buffer_read_char(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packet_buffer_read_uint8_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 3); }
        if (ret_firmware_version != NULL) { for (i = 0; i < 3; ++i) ret_firmware_version[i] = tf_packet_buffer_read_uint8_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 3); }
        if (ret_device_identifier != NULL) { *ret_device_identifier = tf_packet_buffer_read_uint16_t(&industrial_analog_out_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&industrial_analog_out_v2->tfp->spitfp->recv_buf, 2); }
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HAL*)industrial_analog_out_v2->tfp->hal, industrial_analog_out_v2->tfp->spitfp->port_id);
        }
...
int tf_industrial_analog_out_v2_callback_tick(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(industrial_analog_out_v2->tfp, tf_hal_current_time_us((TF_HAL*)industrial_analog_out_v2->tfp->hal) + timeout_us);
}
