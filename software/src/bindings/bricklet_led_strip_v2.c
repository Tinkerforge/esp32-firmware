...
int tf_led_strip_v2_create(TF_LEDStripV2 *led_strip_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_LED_STRIP_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_LED_STRIP_V2_DEVICE_IDENTIFIER;
...
int tf_led_strip_v2_set_led_values_low_level(TF_LEDStripV2 *led_strip_v2, uint16_t index, uint16_t value_length, uint16_t value_chunk_offset, const uint8_t value_chunk_data[58]) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(led_strip_v2->tfp);

    index = tf_leconvert_uint16_to(index); memcpy(send_buf + 0, &index, 2);
    value_length = tf_leconvert_uint16_to(value_length); memcpy(send_buf + 2, &value_length, 2);
    value_chunk_offset = tf_leconvert_uint16_to(value_chunk_offset); memcpy(send_buf + 4, &value_chunk_offset, 2);
    memcpy(send_buf + 6, value_chunk_data, 58);

    uint32_t deadline = tf_hal_current_time_us(hal) + tf_hal_get_common(hal)->timeout;

    uint8_t error_code = 0;
    int result = tf_tfp_send_packet(led_strip_v2->tfp, response_expected, deadline, &error_code);
...
int tf_led_strip_v2_get_led_values_low_level(TF_LEDStripV2 *led_strip_v2, uint16_t index, uint16_t length, uint16_t *ret_value_length, uint16_t *ret_value_chunk_offset, uint8_t ret_value_chunk_data[60]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(led_strip_v2->tfp);
        if (ret_value_length != NULL) { *ret_value_length = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_value_chunk_offset != NULL) { *ret_value_chunk_offset = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_value_chunk_data != NULL) { for (i = 0; i < 60; ++i) ret_value_chunk_data[i] = tf_packet_buffer_read_uint8_t(recv_buf);} else { tf_packet_buffer_remove(recv_buf, 60); }
...
int tf_led_strip_v2_set_frame_duration(TF_LEDStripV2 *led_strip_v2, uint16_t duration) {
    ...
    duration = tf_leconvert_uint16_to(duration); memcpy(send_buf + 0, &duration, 2);
...
int tf_led_strip_v2_set_clock_frequency(TF_LEDStripV2 *led_strip_v2, uint32_t frequency) {
    ...
    frequency = tf_leconvert_uint32_to(frequency); memcpy(send_buf + 0, &frequency, 4);
...
int tf_led_strip_v2_set_chip_type(TF_LEDStripV2 *led_strip_v2, uint16_t chip) {
    ...
    chip = tf_leconvert_uint16_to(chip); memcpy(send_buf + 0, &chip, 2);
...
int tf_led_strip_v2_get_chip_type(TF_LEDStripV2 *led_strip_v2, uint16_t *ret_chip) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(led_strip_v2->tfp);
        if (ret_chip != NULL) { *ret_chip = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        tf_tfp_packet_processed(led_strip_v2->tfp);
    }
...
int tf_led_strip_v2_get_channel_mapping(TF_LEDStripV2 *led_strip_v2, uint8_t *ret_mapping) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(led_strip_v2->tfp);
        if (ret_mapping != NULL) { *ret_mapping = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(led_strip_v2->tfp);
    }
...
int tf_led_strip_v2_set_led_values(TF_LEDStripV2 *led_strip_v2, uint16_t index, const uint8_t *value, uint16_t value_length) {
    ...
    TF_LEDStripV2_SetLEDValuesLLWrapperData wrapper_data;
    memset(&wrapper_data, 0, sizeof(wrapper_data));
    wrapper_data.index = index;

    uint32_t stream_length = value_length;
    uint32_t value_written = 0;
    uint8_t chunk_data[58];

    int ret = tf_stream_in(led_strip_v2, tf_led_strip_v2_set_led_values_ll_wrapper, &wrapper_data, value, stream_length, chunk_data, &value_written, 58, tf_copy_items_uint8_t);
...
int tf_led_strip_v2_get_led_values(TF_LEDStripV2 *led_strip_v2, uint16_t index, uint16_t length, uint8_t *ret_value, uint16_t *ret_value_length) {
    ...
    TF_LEDStripV2_GetLEDValuesLLWrapperData wrapper_data;
    memset(&wrapper_data, 0, sizeof(wrapper_data));
    wrapper_data.index = index;
    ...
    int ret = tf_stream_out(led_strip_v2, tf_led_strip_v2_get_led_values_ll_wrapper, &wrapper_data, ret_value, &value_length, value_chunk_data, 60, tf_copy_items_uint8_t);

    if (ret_value_length != NULL) {
        *ret_value_length = (uint16_t)value_length;
    }
    return ret;
}
#if TF_IMPLEMENT_CALLBACKS != 0
