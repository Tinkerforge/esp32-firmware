...
static bool tf_nfc_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_NFC_CALLBACK_READER_STATE_CHANGED: {
            ...
            TF_HalCommon *common = tf_hal_get_common((TF_HalContext*)nfc->tfp->hal);
            common->locked = true;
...
int tf_nfc_set_mode(TF_NFC *nfc, uint8_t mode) {
    ...
    if(tf_hal_get_common((TF_HalContext*)nfc->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HalContext*)nfc->tfp->hal) + tf_hal_get_common((TF_HalContext*)nfc->tfp->hal)->timeout;
...
int tf_nfc_simple_get_tag_id_low_level(TF_NFC *nfc, uint8_t index, uint8_t *ret_tag_type, uint8_t *ret_tag_id_length, uint8_t ret_tag_id_data[10], uint32_t *ret_last_seen) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_tag_type != NULL) { *ret_tag_type = tf_packetbuffer_read_uint8_t(&nfc->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&nfc->tfp->spitfp->recv_buf, 1); }
        if (ret_tag_id_length != NULL) { *ret_tag_id_length = tf_packetbuffer_read_uint8_t(&nfc->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&nfc->tfp->spitfp->recv_buf, 1); }
        if (ret_tag_id_data != NULL) { for (i = 0; i < 10; ++i) ret_tag_id_data[i] = tf_packetbuffer_read_uint8_t(&nfc->tfp->spitfp->recv_buf);} else { tf_packetbuffer_remove(&nfc->tfp->spitfp->recv_buf, 10); }
        if (ret_last_seen != NULL) { *ret_last_seen = tf_packetbuffer_read_uint32_t(&nfc->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&nfc->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(nfc->tfp);
    }
...
int tf_nfc_get_identity(TF_NFC *nfc, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HalContext*)nfc->tfp->hal, nfc->tfp->spitfp->port_id);
        }
...
int tf_nfc_callback_tick(TF_NFC *nfc, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(nfc->tfp, tf_hal_current_time_us((TF_HalContext*)nfc->tfp->hal) + timeout_us);
}
