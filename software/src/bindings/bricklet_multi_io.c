...
int tf_multi_io_create(TF_MultiIO *multi_io, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_MULTI_IO_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_MULTI_IO_DEVICE_IDENTIFIER;
...
int tf_multi_io_get_inputs(TF_MultiIO *multi_io, uint8_t ret_value[16]) {
    ...
    int result = tf_tfp_send_packet(multi_io->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(multi_io->tfp);
        if (ret_value != NULL) { for (i = 0; i < 16; ++i) ret_value[i] = tf_packet_buffer_read_uint8_t(recv_buf);} else { tf_packet_buffer_remove(recv_buf, 16); }
...
int tf_multi_io_set_outputs(TF_MultiIO *multi_io, const bool value[2]) {
    ...
    size_t i;
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(multi_io->tfp);

    memset(send_buf + 0, 0, 1); for (i = 0; i < 2; ++i) send_buf[0 + (i / 8)] |= (value[i] ? 1 : 0) << (i % 8);
...
int tf_multi_io_get_outputs(TF_MultiIO *multi_io, bool ret_value[2]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(multi_io->tfp);
        if (ret_value != NULL) { tf_packet_buffer_read_bool_array(recv_buf, ret_value, 2);} else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(multi_io->tfp);
    }
