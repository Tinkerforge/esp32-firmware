...
static bool tf_rs485_callback_handler(void *device, uint8_t fid, TF_PacketBuffer *payload) {
    ...
    switch (fid) {
        ...
        case TF_RS485_CALLBACK_MODBUS_MASTER_READ_HOLDING_REGISTERS_RESPONSE_LOW_LEVEL: {
            ...
            uint16_t holding_registers_chunk_data[29]; for (i = 0; i < 29; ++i) holding_registers_chunk_data[i] = tf_packet_buffer_read_uint16_t(payload);
            hal_common->locked = true;
        ...
        case TF_RS485_CALLBACK_MODBUS_SLAVE_WRITE_MULTIPLE_REGISTERS_REQUEST_LOW_LEVEL: {
            ...
            uint16_t registers_chunk_data[27]; for (i = 0; i < 27; ++i) registers_chunk_data[i] = tf_packet_buffer_read_uint16_t(payload);
            hal_common->locked = true;
        ...
        case TF_RS485_CALLBACK_MODBUS_MASTER_READ_INPUT_REGISTERS_RESPONSE_LOW_LEVEL: {
            ...
            uint16_t input_registers_chunk_data[29]; for (i = 0; i < 29; ++i) input_registers_chunk_data[i] = tf_packet_buffer_read_uint16_t(payload);
            hal_common->locked = true;
...
int tf_rs485_create(TF_RS485 *rs485, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_RS485_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_RS485_DEVICE_IDENTIFIER;
...
int tf_rs485_write_low_level(TF_RS485 *rs485, uint16_t message_length, uint16_t message_chunk_offset, const char message_chunk_data[60], uint8_t *ret_message_chunk_written) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(rs485->tfp);
    ...
    int result = tf_tfp_send_packet(rs485->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(rs485->tfp);
...
int tf_rs485_get_rs485_configuration(TF_RS485 *rs485, uint32_t *ret_baudrate, uint8_t *ret_parity, uint8_t *ret_stopbits, uint8_t *ret_wordlength, uint8_t *ret_duplex) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_duplex != NULL) { *ret_duplex = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(rs485->tfp);
    }
...
int tf_rs485_set_modbus_configuration(TF_RS485 *rs485, uint8_t slave_address, uint32_t master_request_timeout) {
    ...
    send_buf[0] = (uint8_t)slave_address;
    master_request_timeout = tf_leconvert_uint32_to(master_request_timeout); memcpy(send_buf + 1, &master_request_timeout, 4);
...
int tf_rs485_get_modbus_configuration(TF_RS485 *rs485, uint8_t *ret_slave_address, uint32_t *ret_master_request_timeout) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(rs485->tfp);
        if (ret_slave_address != NULL) { *ret_slave_address = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_master_request_timeout != NULL) { *ret_master_request_timeout = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        tf_tfp_packet_processed(rs485->tfp);
    }
...
int tf_rs485_get_error_count(TF_RS485 *rs485, uint32_t *ret_overrun_error_count, uint32_t *ret_parity_error_count) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(rs485->tfp);
        if (ret_overrun_error_count != NULL) { *ret_overrun_error_count = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_parity_error_count != NULL) { *ret_parity_error_count = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        tf_tfp_packet_processed(rs485->tfp);
    }
...
int tf_rs485_get_modbus_common_error_count(TF_RS485 *rs485, uint32_t *ret_timeout_error_count, uint32_t *ret_checksum_error_count, uint32_t *ret_frame_too_big_error_count, uint32_t *ret_illegal_function_error_count, uint32_t *ret_illegal_data_address_error_count, uint32_t *ret_illegal_data_value_error_count, uint32_t *ret_slave_device_failure_error_count) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(rs485->tfp);
        if (ret_timeout_error_count != NULL) { *ret_timeout_error_count = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_checksum_error_count != NULL) { *ret_checksum_error_count = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_frame_too_big_error_count != NULL) { *ret_frame_too_big_error_count = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_illegal_function_error_count != NULL) { *ret_illegal_function_error_count = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_illegal_data_address_error_count != NULL) { *ret_illegal_data_address_error_count = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_illegal_data_value_error_count != NULL) { *ret_illegal_data_value_error_count = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        if (ret_slave_device_failure_error_count != NULL) { *ret_slave_device_failure_error_count = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        tf_tfp_packet_processed(rs485->tfp);
    }
...
int tf_rs485_modbus_slave_report_exception(TF_RS485 *rs485, uint8_t request_id, int8_t exception_code) {
    ...
    send_buf[0] = (uint8_t)request_id;
    send_buf[1] = (uint8_t)exception_code;
...
int tf_rs485_modbus_slave_answer_read_coils_request_low_level(TF_RS485 *rs485, uint8_t request_id, uint16_t coils_length, uint16_t coils_chunk_offset, const bool coils_chunk_data[472]) {
    ...
    send_buf[0] = (uint8_t)request_id;
    coils_length = tf_leconvert_uint16_to(coils_length); memcpy(send_buf + 1, &coils_length, 2);
    coils_chunk_offset = tf_leconvert_uint16_to(coils_chunk_offset); memcpy(send_buf + 3, &coils_chunk_offset, 2);
    memset(send_buf + 5, 0, 59); for (i = 0; i < 472; ++i) send_buf[5 + (i / 8)] |= (coils_chunk_data[i] ? 1 : 0) << (i % 8);
...
int tf_rs485_modbus_master_read_coils(TF_RS485 *rs485, uint8_t slave_address, uint32_t starting_address, uint16_t count, uint8_t *ret_request_id) {
    ...
    send_buf[0] = (uint8_t)slave_address;
    starting_address = tf_leconvert_uint32_to(starting_address); memcpy(send_buf + 1, &starting_address, 4);
    count = tf_leconvert_uint16_to(count); memcpy(send_buf + 5, &count, 2);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(rs485->tfp);
        if (ret_request_id != NULL) { *ret_request_id = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(rs485->tfp);
    }
...
int tf_rs485_modbus_slave_answer_read_holding_registers_request_low_level(TF_RS485 *rs485, uint8_t request_id, uint16_t holding_registers_length, uint16_t holding_registers_chunk_offset, const uint16_t holding_registers_chunk_data[29]) {
    ...
    send_buf[0] = (uint8_t)request_id;
    holding_registers_length = tf_leconvert_uint16_to(holding_registers_length); memcpy(send_buf + 1, &holding_registers_length, 2);
    holding_registers_chunk_offset = tf_leconvert_uint16_to(holding_registers_chunk_offset); memcpy(send_buf + 3, &holding_registers_chunk_offset, 2);
    for (i = 0; i < 29; i++) { uint16_t tmp_holding_registers_chunk_data = tf_leconvert_uint16_to(holding_registers_chunk_data[i]); memcpy(send_buf + 5 + (i * sizeof(uint16_t)), &tmp_holding_registers_chunk_data, sizeof(uint16_t)); }
...
int tf_rs485_modbus_master_write_single_coil(TF_RS485 *rs485, uint8_t slave_address, uint32_t coil_address, bool coil_value, uint8_t *ret_request_id) {
    ...
    send_buf[0] = (uint8_t)slave_address;
    coil_address = tf_leconvert_uint32_to(coil_address); memcpy(send_buf + 1, &coil_address, 4);
    send_buf[5] = coil_value ? 1 : 0;
...
int tf_rs485_modbus_master_write_single_register(TF_RS485 *rs485, uint8_t slave_address, uint32_t register_address, uint16_t register_value, uint8_t *ret_request_id) {
    ...
    send_buf[0] = (uint8_t)slave_address;
    register_address = tf_leconvert_uint32_to(register_address); memcpy(send_buf + 1, &register_address, 4);
    register_value = tf_leconvert_uint16_to(register_value); memcpy(send_buf + 5, &register_value, 2);
...
int tf_rs485_modbus_master_write_multiple_coils_low_level(TF_RS485 *rs485, uint8_t slave_address, uint32_t starting_address, uint16_t coils_length, uint16_t coils_chunk_offset, const bool coils_chunk_data[440], uint8_t *ret_request_id) {
    ...
    send_buf[0] = (uint8_t)slave_address;
    starting_address = tf_leconvert_uint32_to(starting_address); memcpy(send_buf + 1, &starting_address, 4);
    coils_length = tf_leconvert_uint16_to(coils_length); memcpy(send_buf + 5, &coils_length, 2);
    coils_chunk_offset = tf_leconvert_uint16_to(coils_chunk_offset); memcpy(send_buf + 7, &coils_chunk_offset, 2);
    memset(send_buf + 9, 0, 55); for (i = 0; i < 440; ++i) send_buf[9 + (i / 8)] |= (coils_chunk_data[i] ? 1 : 0) << (i % 8);
...
int tf_rs485_modbus_master_write_multiple_registers_low_level(TF_RS485 *rs485, uint8_t slave_address, uint32_t starting_address, uint16_t registers_length, uint16_t registers_chunk_offset, const uint16_t registers_chunk_data[27], uint8_t *ret_request_id) {
    ...
    send_buf[0] = (uint8_t)slave_address;
    starting_address = tf_leconvert_uint32_to(starting_address); memcpy(send_buf + 1, &starting_address, 4);
    registers_length = tf_leconvert_uint16_to(registers_length); memcpy(send_buf + 5, &registers_length, 2);
    registers_chunk_offset = tf_leconvert_uint16_to(registers_chunk_offset); memcpy(send_buf + 7, &registers_chunk_offset, 2);
    for (i = 0; i < 27; i++) { uint16_t tmp_registers_chunk_data = tf_leconvert_uint16_to(registers_chunk_data[i]); memcpy(send_buf + 9 + (i * sizeof(uint16_t)), &tmp_registers_chunk_data, sizeof(uint16_t)); }
...
int tf_rs485_modbus_slave_answer_read_discrete_inputs_request_low_level(TF_RS485 *rs485, uint8_t request_id, uint16_t discrete_inputs_length, uint16_t discrete_inputs_chunk_offset, const bool discrete_inputs_chunk_data[472]) {
    ...
    send_buf[0] = (uint8_t)request_id;
    discrete_inputs_length = tf_leconvert_uint16_to(discrete_inputs_length); memcpy(send_buf + 1, &discrete_inputs_length, 2);
    discrete_inputs_chunk_offset = tf_leconvert_uint16_to(discrete_inputs_chunk_offset); memcpy(send_buf + 3, &discrete_inputs_chunk_offset, 2);
    memset(send_buf + 5, 0, 59); for (i = 0; i < 472; ++i) send_buf[5 + (i / 8)] |= (discrete_inputs_chunk_data[i] ? 1 : 0) << (i % 8);
...
int tf_rs485_modbus_slave_answer_read_input_registers_request_low_level(TF_RS485 *rs485, uint8_t request_id, uint16_t input_registers_length, uint16_t input_registers_chunk_offset, const uint16_t input_registers_chunk_data[29]) {
    ...
    send_buf[0] = (uint8_t)request_id;
    input_registers_length = tf_leconvert_uint16_to(input_registers_length); memcpy(send_buf + 1, &input_registers_length, 2);
    input_registers_chunk_offset = tf_leconvert_uint16_to(input_registers_chunk_offset); memcpy(send_buf + 3, &input_registers_chunk_offset, 2);
    for (i = 0; i < 29; i++) { uint16_t tmp_input_registers_chunk_data = tf_leconvert_uint16_to(input_registers_chunk_data[i]); memcpy(send_buf + 5 + (i * sizeof(uint16_t)), &tmp_input_registers_chunk_data, sizeof(uint16_t)); }
...
int tf_rs485_write(TF_RS485 *rs485, const char *message, uint16_t message_length, uint16_t *ret_message_written) {
    ...
    int ret = tf_stream_in(rs485, tf_rs485_write_ll_wrapper, NULL, message, stream_length, chunk_data, &message_written, 60, tf_copy_items_char);
...
int tf_rs485_read(TF_RS485 *rs485, uint16_t length, char *ret_message, uint16_t *ret_message_length) {
    ...
    TF_RS485_ReadLLWrapperData wrapper_data;
    memset(&wrapper_data, 0, sizeof(wrapper_data));
    wrapper_data.length = length;
    uint32_t message_length = 0;
    char message_chunk_data[60];

    int ret = tf_stream_out(rs485, tf_rs485_read_ll_wrapper, &wrapper_data, ret_message, &message_length, message_chunk_data, 60, tf_copy_items_char);
...
int tf_rs485_modbus_slave_answer_read_coils_request(TF_RS485 *rs485, uint8_t request_id, const bool *coils, uint16_t coils_length) {
    ...
    TF_RS485_ModbusSlaveAnswerReadCoilsRequestLLWrapperData wrapper_data;
    memset(&wrapper_data, 0, sizeof(wrapper_data));
    wrapper_data.request_id = request_id;

    uint32_t stream_length = coils_length;
    uint32_t coils_written = 0;
    bool chunk_data[472];

    int ret = tf_stream_in(rs485, tf_rs485_modbus_slave_answer_read_coils_request_ll_wrapper, &wrapper_data, coils, stream_length, chunk_data, &coils_written, 472, tf_copy_items_bool);
...
int tf_rs485_modbus_slave_answer_read_holding_registers_request(TF_RS485 *rs485, uint8_t request_id, const uint16_t *holding_registers, uint16_t holding_registers_length) {
    ...
    TF_RS485_ModbusSlaveAnswerReadHoldingRegistersRequestLLWrapperData wrapper_data;
    memset(&wrapper_data, 0, sizeof(wrapper_data));
    wrapper_data.request_id = request_id;

    uint32_t stream_length = holding_registers_length;
    uint32_t holding_registers_written = 0;
    uint16_t chunk_data[29];

    int ret = tf_stream_in(rs485, tf_rs485_modbus_slave_answer_read_holding_registers_request_ll_wrapper, &wrapper_data, holding_registers, stream_length, chunk_data, &holding_registers_written, 29, tf_copy_items_uint16_t);
...
int tf_rs485_modbus_master_write_multiple_coils(TF_RS485 *rs485, uint8_t slave_address, uint32_t starting_address, const bool *coils, uint16_t coils_length, uint8_t *ret_request_id) {
    ...
    TF_RS485_ModbusMasterWriteMultipleCoilsLLWrapperData wrapper_data;
    memset(&wrapper_data, 0, sizeof(wrapper_data));
    wrapper_data.slave_address = slave_address;
    wrapper_data.starting_address = starting_address;
    wrapper_data.ret_request_id = ret_request_id;

    uint32_t stream_length = coils_length;
    uint32_t coils_written = 0;
    bool chunk_data[440];

    int ret = tf_stream_in(rs485, tf_rs485_modbus_master_write_multiple_coils_ll_wrapper, &wrapper_data, coils, stream_length, chunk_data, &coils_written, 440, tf_copy_items_bool);
...
int tf_rs485_modbus_master_write_multiple_registers(TF_RS485 *rs485, uint8_t slave_address, uint32_t starting_address, const uint16_t *registers, uint16_t registers_length, uint8_t *ret_request_id) {
    ...
    TF_RS485_ModbusMasterWriteMultipleRegistersLLWrapperData wrapper_data;
    memset(&wrapper_data, 0, sizeof(wrapper_data));
    wrapper_data.slave_address = slave_address;
    wrapper_data.starting_address = starting_address;
    wrapper_data.ret_request_id = ret_request_id;

    uint32_t stream_length = registers_length;
    uint32_t registers_written = 0;
    uint16_t chunk_data[27];

    int ret = tf_stream_in(rs485, tf_rs485_modbus_master_write_multiple_registers_ll_wrapper, &wrapper_data, registers, stream_length, chunk_data, &registers_written, 27, tf_copy_items_uint16_t);
...
int tf_rs485_modbus_slave_answer_read_discrete_inputs_request(TF_RS485 *rs485, uint8_t request_id, const bool *discrete_inputs, uint16_t discrete_inputs_length) {
    ...
    TF_RS485_ModbusSlaveAnswerReadDiscreteInputsRequestLLWrapperData wrapper_data;
    memset(&wrapper_data, 0, sizeof(wrapper_data));
    wrapper_data.request_id = request_id;

    uint32_t stream_length = discrete_inputs_length;
    uint32_t discrete_inputs_written = 0;
    bool chunk_data[472];

    int ret = tf_stream_in(rs485, tf_rs485_modbus_slave_answer_read_discrete_inputs_request_ll_wrapper, &wrapper_data, discrete_inputs, stream_length, chunk_data, &discrete_inputs_written, 472, tf_copy_items_bool);
...
int tf_rs485_modbus_slave_answer_read_input_registers_request(TF_RS485 *rs485, uint8_t request_id, const uint16_t *input_registers, uint16_t input_registers_length) {
    ...
    TF_RS485_ModbusSlaveAnswerReadInputRegistersRequestLLWrapperData wrapper_data;
    memset(&wrapper_data, 0, sizeof(wrapper_data));
    wrapper_data.request_id = request_id;

    uint32_t stream_length = input_registers_length;
    uint32_t input_registers_written = 0;
    uint16_t chunk_data[29];

    int ret = tf_stream_in(rs485, tf_rs485_modbus_slave_answer_read_input_registers_request_ll_wrapper, &wrapper_data, input_registers, stream_length, chunk_data, &input_registers_written, 29, tf_copy_items_uint16_t);
