...
int tf_accelerometer_v2_create(TF_AccelerometerV2 *accelerometer_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_ACCELEROMETER_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_ACCELEROMETER_V2_DEVICE_IDENTIFIER;
...
int tf_accelerometer_v2_get_acceleration(TF_AccelerometerV2 *accelerometer_v2, int32_t *ret_x, int32_t *ret_y, int32_t *ret_z) {
    ...
    int result = tf_tfp_send_packet(accelerometer_v2->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(accelerometer_v2->tfp);
...
int tf_accelerometer_v2_set_configuration(TF_AccelerometerV2 *accelerometer_v2, uint8_t data_rate, uint8_t full_scale) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(accelerometer_v2->tfp);

    send_buf[0] = (uint8_t)data_rate;
    send_buf[1] = (uint8_t)full_scale;
...
int tf_accelerometer_v2_get_configuration(TF_AccelerometerV2 *accelerometer_v2, uint8_t *ret_data_rate, uint8_t *ret_full_scale) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_full_scale != NULL) { *ret_full_scale = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(accelerometer_v2->tfp);
    }
...
int tf_accelerometer_v2_set_continuous_acceleration_configuration(TF_AccelerometerV2 *accelerometer_v2, bool enable_x, bool enable_y, bool enable_z, uint8_t resolution) {
    ...
    send_buf[0] = enable_x ? 1 : 0;
    send_buf[1] = enable_y ? 1 : 0;
    send_buf[2] = enable_z ? 1 : 0;
    send_buf[3] = (uint8_t)resolution;
...
int tf_accelerometer_v2_get_continuous_acceleration_configuration(TF_AccelerometerV2 *accelerometer_v2, bool *ret_enable_x, bool *ret_enable_y, bool *ret_enable_z, uint8_t *ret_resolution) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(accelerometer_v2->tfp);
        if (ret_enable_x != NULL) { *ret_enable_x = tf_packet_buffer_read_bool(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_enable_y != NULL) { *ret_enable_y = tf_packet_buffer_read_bool(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_enable_z != NULL) { *ret_enable_z = tf_packet_buffer_read_bool(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
...
int tf_accelerometer_v2_set_filter_configuration(TF_AccelerometerV2 *accelerometer_v2, uint8_t iir_bypass, uint8_t low_pass_filter) {
    ...
    send_buf[0] = (uint8_t)iir_bypass;
    send_buf[1] = (uint8_t)low_pass_filter;
...
int tf_accelerometer_v2_get_filter_configuration(TF_AccelerometerV2 *accelerometer_v2, uint8_t *ret_iir_bypass, uint8_t *ret_low_pass_filter) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(accelerometer_v2->tfp);
        if (ret_iir_bypass != NULL) { *ret_iir_bypass = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_low_pass_filter != NULL) { *ret_low_pass_filter = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(accelerometer_v2->tfp);
    }
