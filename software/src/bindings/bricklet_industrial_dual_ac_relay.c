...
int tf_industrial_dual_ac_relay_create(TF_IndustrialDualACRelay *industrial_dual_ac_relay, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_INDUSTRIAL_DUAL_AC_RELAY_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_INDUSTRIAL_DUAL_AC_RELAY_DEVICE_IDENTIFIER;
...
int tf_industrial_dual_ac_relay_set_value(TF_IndustrialDualACRelay *industrial_dual_ac_relay, bool channel0, bool channel1) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(industrial_dual_ac_relay->tfp);

    send_buf[0] = channel0 ? 1 : 0;
    send_buf[1] = channel1 ? 1 : 0;

    uint32_t deadline = tf_hal_current_time_us(hal) + tf_hal_get_common(hal)->timeout;

    uint8_t error_code = 0;
    int result = tf_tfp_send_packet(industrial_dual_ac_relay->tfp, response_expected, deadline, &error_code);
...
int tf_industrial_dual_ac_relay_get_value(TF_IndustrialDualACRelay *industrial_dual_ac_relay, bool *ret_channel0, bool *ret_channel1) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(industrial_dual_ac_relay->tfp);
        if (ret_channel0 != NULL) { *ret_channel0 = tf_packet_buffer_read_bool(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_channel1 != NULL) { *ret_channel1 = tf_packet_buffer_read_bool(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
