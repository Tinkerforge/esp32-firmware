...
int tf_hall_effect_v2_create(TF_HallEffectV2 *hall_effect_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_HALL_EFFECT_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_HALL_EFFECT_V2_DEVICE_IDENTIFIER;
...
int tf_hall_effect_v2_get_magnetic_flux_density(TF_HallEffectV2 *hall_effect_v2, int16_t *ret_magnetic_flux_density) {
    ...
    int result = tf_tfp_send_packet(hall_effect_v2->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(hall_effect_v2->tfp);
        if (ret_magnetic_flux_density != NULL) { *ret_magnetic_flux_density = tf_packet_buffer_read_int16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
...
int tf_hall_effect_v2_set_magnetic_flux_density_callback_configuration(TF_HallEffectV2 *hall_effect_v2, uint32_t period, bool value_has_to_change, char option, int16_t min, int16_t max) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(hall_effect_v2->tfp);
...
int tf_hall_effect_v2_set_counter_config(TF_HallEffectV2 *hall_effect_v2, int16_t high_threshold, int16_t low_threshold, uint32_t debounce) {
    ...
    high_threshold = tf_leconvert_int16_to(high_threshold); memcpy(send_buf + 0, &high_threshold, 2);
    low_threshold = tf_leconvert_int16_to(low_threshold); memcpy(send_buf + 2, &low_threshold, 2);
    debounce = tf_leconvert_uint32_to(debounce); memcpy(send_buf + 4, &debounce, 4);
...
int tf_hall_effect_v2_get_counter_config(TF_HallEffectV2 *hall_effect_v2, int16_t *ret_high_threshold, int16_t *ret_low_threshold, uint32_t *ret_debounce) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(hall_effect_v2->tfp);
        if (ret_high_threshold != NULL) { *ret_high_threshold = tf_packet_buffer_read_int16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_low_threshold != NULL) { *ret_low_threshold = tf_packet_buffer_read_int16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        if (ret_debounce != NULL) { *ret_debounce = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        tf_tfp_packet_processed(hall_effect_v2->tfp);
    }
