...
int tf_rotary_poti_v2_create(TF_RotaryPotiV2 *rotary_poti_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_ROTARY_POTI_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_ROTARY_POTI_V2_DEVICE_IDENTIFIER;
...
int tf_rotary_poti_v2_get_position(TF_RotaryPotiV2 *rotary_poti_v2, int16_t *ret_position) {
    ...
    int result = tf_tfp_send_packet(rotary_poti_v2->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(rotary_poti_v2->tfp);
...
int tf_rotary_poti_v2_set_position_callback_configuration(TF_RotaryPotiV2 *rotary_poti_v2, uint32_t period, bool value_has_to_change, char option, int16_t min, int16_t max) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(rotary_poti_v2->tfp);
