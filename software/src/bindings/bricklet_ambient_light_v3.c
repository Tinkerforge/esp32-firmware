...
static bool tf_ambient_light_v3_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_AMBIENT_LIGHT_V3_CALLBACK_ILLUMINANCE: {
            ...
            TF_HalCommon *common = tf_hal_get_common((TF_HalContext*)ambient_light_v3->tfp->hal);
            common->locked = true;
...
int tf_ambient_light_v3_get_illuminance(TF_AmbientLightV3 *ambient_light_v3, uint32_t *ret_illuminance) {
    ...
    if(tf_hal_get_common((TF_HalContext*)ambient_light_v3->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HalContext*)ambient_light_v3->tfp->hal) + tf_hal_get_common((TF_HalContext*)ambient_light_v3->tfp->hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_illuminance != NULL) { *ret_illuminance = tf_packetbuffer_read_uint32_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 4); }
...
int tf_ambient_light_v3_get_illuminance_callback_configuration(TF_AmbientLightV3 *ambient_light_v3, uint32_t *ret_period, bool *ret_value_has_to_change, char *ret_option, uint32_t *ret_min, uint32_t *ret_max) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_period != NULL) { *ret_period = tf_packetbuffer_read_uint32_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 4); }
        if (ret_value_has_to_change != NULL) { *ret_value_has_to_change = tf_packetbuffer_read_bool(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 1); }
        if (ret_option != NULL) { *ret_option = tf_packetbuffer_read_char(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 1); }
        if (ret_min != NULL) { *ret_min = tf_packetbuffer_read_uint32_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 4); }
        if (ret_max != NULL) { *ret_max = tf_packetbuffer_read_uint32_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(ambient_light_v3->tfp);
    }
...
int tf_ambient_light_v3_get_configuration(TF_AmbientLightV3 *ambient_light_v3, uint8_t *ret_illuminance_range, uint8_t *ret_integration_time) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_illuminance_range != NULL) { *ret_illuminance_range = tf_packetbuffer_read_uint8_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 1); }
        if (ret_integration_time != NULL) { *ret_integration_time = tf_packetbuffer_read_uint8_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(ambient_light_v3->tfp);
    }
...
int tf_ambient_light_v3_get_spitfp_error_count(TF_AmbientLightV3 *ambient_light_v3, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packetbuffer_read_uint32_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packetbuffer_read_uint32_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packetbuffer_read_uint32_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packetbuffer_read_uint32_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(ambient_light_v3->tfp);
    }
...
int tf_ambient_light_v3_set_bootloader_mode(TF_AmbientLightV3 *ambient_light_v3, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packetbuffer_read_uint8_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(ambient_light_v3->tfp);
    }
...
int tf_ambient_light_v3_get_bootloader_mode(TF_AmbientLightV3 *ambient_light_v3, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packetbuffer_read_uint8_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(ambient_light_v3->tfp);
    }
...
int tf_ambient_light_v3_get_status_led_config(TF_AmbientLightV3 *ambient_light_v3, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packetbuffer_read_uint8_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(ambient_light_v3->tfp);
    }
...
int tf_ambient_light_v3_get_chip_temperature(TF_AmbientLightV3 *ambient_light_v3, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packetbuffer_read_int16_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(ambient_light_v3->tfp);
    }
...
int tf_ambient_light_v3_read_uid(TF_AmbientLightV3 *ambient_light_v3, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packetbuffer_read_uint32_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(ambient_light_v3->tfp);
    }
...
int tf_ambient_light_v3_get_identity(TF_AmbientLightV3 *ambient_light_v3, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packetbuffer_pop_n(&ambient_light_v3->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 8); }
        tf_packetbuffer_pop_n(&ambient_light_v3->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packetbuffer_read_char(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packetbuffer_read_uint8_t(&ambient_light_v3->tfp->spitfp->recv_buf);} else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 3); }
        if (ret_firmware_version != NULL) { for (i = 0; i < 3; ++i) ret_firmware_version[i] = tf_packetbuffer_read_uint8_t(&ambient_light_v3->tfp->spitfp->recv_buf);} else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 3); }
        if (ret_device_identifier != NULL) { *ret_device_identifier = tf_packetbuffer_read_uint16_t(&ambient_light_v3->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&ambient_light_v3->tfp->spitfp->recv_buf, 2); }
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HalContext*)ambient_light_v3->tfp->hal, ambient_light_v3->tfp->spitfp->port_id);
        }
...
int tf_ambient_light_v3_callback_tick(TF_AmbientLightV3 *ambient_light_v3, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(ambient_light_v3->tfp, tf_hal_current_time_us((TF_HalContext*)ambient_light_v3->tfp->hal) + timeout_us);
}
