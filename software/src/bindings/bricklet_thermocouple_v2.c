...
static bool tf_thermocouple_v2_callback_handler(void *device, uint8_t fid, TF_PacketBuffer *payload) {
    TF_ThermocoupleV2 *thermocouple_v2 = (TF_ThermocoupleV2 *)device;
    TF_HALCommon *hal_common = tf_hal_get_common(thermocouple_v2->tfp->spitfp->hal);
    (void)payload;
...
int tf_thermocouple_v2_create(TF_ThermocoupleV2 *thermocouple_v2, const char *uid_or_port_name, TF_HAL *hal) {
    if (thermocouple_v2 == NULL || hal == NULL) {
        return TF_E_NULL;
    }
    ...
    TF_TFP *tfp;
    int rc = tf_hal_get_attachable_tfp(hal, &tfp, uid_or_port_name, TF_THERMOCOUPLE_V2_DEVICE_IDENTIFIER);

    if (rc != TF_E_OK) {
        return rc;
    }

    thermocouple_v2->tfp = tfp;
    thermocouple_v2->tfp->device = thermocouple_v2;
    thermocouple_v2->tfp->cb_handler = tf_thermocouple_v2_callback_handler;
    thermocouple_v2->magic = 0x5446;
...
int tf_thermocouple_v2_destroy(TF_ThermocoupleV2 *thermocouple_v2) {
    ...
    if (thermocouple_v2->magic != 0x5446 || thermocouple_v2->tfp == NULL) {
        return TF_E_NOT_INITIALIZED;
    }

    thermocouple_v2->tfp->cb_handler = NULL;
    thermocouple_v2->tfp->device = NULL;
    thermocouple_v2->tfp = NULL;
    thermocouple_v2->magic = 0;
...
int tf_thermocouple_v2_get_temperature(TF_ThermocoupleV2 *thermocouple_v2, int32_t *ret_temperature) {
    ...
    TF_HAL *_hal = thermocouple_v2->tfp->spitfp->hal;
    ...
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_GET_TEMPERATURE, 0, _response_expected);

    uint32_t _deadline = tf_hal_current_time_us(_hal) + tf_hal_get_common(_hal)->timeout;

    uint8_t _error_code = 0;
    uint8_t _length = 0;
    int _result = tf_tfp_send_packet(thermocouple_v2->tfp, _response_expected, _deadline, &_error_code, &_length, TF_NEW_PACKET);
    ...
    if (_result & TF_TICK_PACKET_RECEIVED) {
        TF_PacketBuffer *_recv_buf = tf_tfp_get_receive_buffer(thermocouple_v2->tfp);
...
int tf_thermocouple_v2_set_temperature_callback_configuration(TF_ThermocoupleV2 *thermocouple_v2, uint32_t period, bool value_has_to_change, char option, int32_t min, int32_t max) {
    ...
    bool _response_expected = true;
    tf_thermocouple_v2_get_response_expected(thermocouple_v2, TF_THERMOCOUPLE_V2_FUNCTION_SET_TEMPERATURE_CALLBACK_CONFIGURATION, &_response_expected);
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_SET_TEMPERATURE_CALLBACK_CONFIGURATION, 14, _response_expected);

    uint8_t *_send_buf = tf_tfp_get_send_payload_buffer(thermocouple_v2->tfp);
...
int tf_thermocouple_v2_get_temperature_callback_configuration(TF_ThermocoupleV2 *thermocouple_v2, uint32_t *ret_period, bool *ret_value_has_to_change, char *ret_option, int32_t *ret_min, int32_t *ret_max) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_GET_TEMPERATURE_CALLBACK_CONFIGURATION, 0, _response_expected);
...
int tf_thermocouple_v2_set_configuration(TF_ThermocoupleV2 *thermocouple_v2, uint8_t averaging, uint8_t thermocouple_type, uint8_t filter) {
    ...
    bool _response_expected = true;
    tf_thermocouple_v2_get_response_expected(thermocouple_v2, TF_THERMOCOUPLE_V2_FUNCTION_SET_CONFIGURATION, &_response_expected);
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_SET_CONFIGURATION, 3, _response_expected);

    uint8_t *_send_buf = tf_tfp_get_send_payload_buffer(thermocouple_v2->tfp);

    _send_buf[0] = (uint8_t)averaging;
    _send_buf[1] = (uint8_t)thermocouple_type;
    _send_buf[2] = (uint8_t)filter;
...
int tf_thermocouple_v2_get_configuration(TF_ThermocoupleV2 *thermocouple_v2, uint8_t *ret_averaging, uint8_t *ret_thermocouple_type, uint8_t *ret_filter) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_GET_CONFIGURATION, 0, _response_expected);
    ...
    if (_result & TF_TICK_PACKET_RECEIVED) {
        ...
        if (_error_code != 0 || _length != 3) {
            ...
            if (ret_thermocouple_type != NULL) { *ret_thermocouple_type = tf_packet_buffer_read_uint8_t(_recv_buf); } else { tf_packet_buffer_remove(_recv_buf, 1); }
...
int tf_thermocouple_v2_get_error_state(TF_ThermocoupleV2 *thermocouple_v2, bool *ret_over_under, bool *ret_open_circuit) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_GET_ERROR_STATE, 0, _response_expected);
    ...
    if (_result & TF_TICK_PACKET_RECEIVED) {
        ...
        if (_error_code != 0 || _length != 2) {
            ...
            if (ret_over_under != NULL) { *ret_over_under = tf_packet_buffer_read_bool(_recv_buf); } else { tf_packet_buffer_remove(_recv_buf, 1); }
            if (ret_open_circuit != NULL) { *ret_open_circuit = tf_packet_buffer_read_bool(_recv_buf); } else { tf_packet_buffer_remove(_recv_buf, 1); }
        }
        tf_tfp_packet_processed(thermocouple_v2->tfp);
    }
...
int tf_thermocouple_v2_get_spitfp_error_count(TF_ThermocoupleV2 *thermocouple_v2, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_GET_SPITFP_ERROR_COUNT, 0, _response_expected);
...
int tf_thermocouple_v2_set_bootloader_mode(TF_ThermocoupleV2 *thermocouple_v2, uint8_t mode, uint8_t *ret_status) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_SET_BOOTLOADER_MODE, 1, _response_expected);
...
int tf_thermocouple_v2_get_bootloader_mode(TF_ThermocoupleV2 *thermocouple_v2, uint8_t *ret_mode) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_GET_BOOTLOADER_MODE, 0, _response_expected);
...
int tf_thermocouple_v2_set_write_firmware_pointer(TF_ThermocoupleV2 *thermocouple_v2, uint32_t pointer) {
    ...
    bool _response_expected = true;
    tf_thermocouple_v2_get_response_expected(thermocouple_v2, TF_THERMOCOUPLE_V2_FUNCTION_SET_WRITE_FIRMWARE_POINTER, &_response_expected);
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_SET_WRITE_FIRMWARE_POINTER, 4, _response_expected);
...
int tf_thermocouple_v2_write_firmware(TF_ThermocoupleV2 *thermocouple_v2, const uint8_t data[64], uint8_t *ret_status) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_WRITE_FIRMWARE, 64, _response_expected);
...
int tf_thermocouple_v2_set_status_led_config(TF_ThermocoupleV2 *thermocouple_v2, uint8_t config) {
    ...
    bool _response_expected = true;
    tf_thermocouple_v2_get_response_expected(thermocouple_v2, TF_THERMOCOUPLE_V2_FUNCTION_SET_STATUS_LED_CONFIG, &_response_expected);
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_SET_STATUS_LED_CONFIG, 1, _response_expected);
...
int tf_thermocouple_v2_get_status_led_config(TF_ThermocoupleV2 *thermocouple_v2, uint8_t *ret_config) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_GET_STATUS_LED_CONFIG, 0, _response_expected);
...
int tf_thermocouple_v2_get_chip_temperature(TF_ThermocoupleV2 *thermocouple_v2, int16_t *ret_temperature) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_GET_CHIP_TEMPERATURE, 0, _response_expected);
...
int tf_thermocouple_v2_reset(TF_ThermocoupleV2 *thermocouple_v2) {
    ...
    bool _response_expected = true;
    tf_thermocouple_v2_get_response_expected(thermocouple_v2, TF_THERMOCOUPLE_V2_FUNCTION_RESET, &_response_expected);
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_RESET, 0, _response_expected);
...
int tf_thermocouple_v2_write_uid(TF_ThermocoupleV2 *thermocouple_v2, uint32_t uid) {
    ...
    bool _response_expected = true;
    tf_thermocouple_v2_get_response_expected(thermocouple_v2, TF_THERMOCOUPLE_V2_FUNCTION_WRITE_UID, &_response_expected);
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_WRITE_UID, 4, _response_expected);
...
int tf_thermocouple_v2_read_uid(TF_ThermocoupleV2 *thermocouple_v2, uint32_t *ret_uid) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_READ_UID, 0, _response_expected);
...
int tf_thermocouple_v2_get_identity(TF_ThermocoupleV2 *thermocouple_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(thermocouple_v2->tfp, TF_THERMOCOUPLE_V2_FUNCTION_GET_IDENTITY, 0, _response_expected);
...
int tf_thermocouple_v2_callback_tick(TF_ThermocoupleV2 *thermocouple_v2, uint32_t timeout_us) {
    ...
    TF_HAL *hal = thermocouple_v2->tfp->spitfp->hal;

    return tf_tfp_callback_tick(thermocouple_v2->tfp, tf_hal_current_time_us(hal) + timeout_us);
}
