...
int tf_io16_v2_create(TF_IO16V2 *io16_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_IO16_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_IO16_V2_DEVICE_IDENTIFIER;
...
int tf_io16_v2_set_value(TF_IO16V2 *io16_v2, const bool value[16]) {
    ...
    size_t i;
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(io16_v2->tfp);

    memset(send_buf + 0, 0, 2); for (i = 0; i < 16; ++i) send_buf[0 + (i / 8)] |= (value[i] ? 1 : 0) << (i % 8);

    uint32_t deadline = tf_hal_current_time_us(hal) + tf_hal_get_common(hal)->timeout;

    uint8_t error_code = 0;
    int result = tf_tfp_send_packet(io16_v2->tfp, response_expected, deadline, &error_code);
...
int tf_io16_v2_get_value(TF_IO16V2 *io16_v2, bool ret_value[16]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(io16_v2->tfp);
        if (ret_value != NULL) { tf_packet_buffer_read_bool_array(recv_buf, ret_value, 16);} else { tf_packet_buffer_remove(recv_buf, 2); }
...
int tf_io16_v2_set_configuration(TF_IO16V2 *io16_v2, uint8_t channel, char direction, bool value) {
    ...
    send_buf[0] = (uint8_t)channel;
    send_buf[1] = (uint8_t)direction;
    send_buf[2] = value ? 1 : 0;
...
int tf_io16_v2_get_configuration(TF_IO16V2 *io16_v2, uint8_t channel, char *ret_direction, bool *ret_value) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(io16_v2->tfp);
        if (ret_direction != NULL) { *ret_direction = tf_packet_buffer_read_char(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
...
int tf_io16_v2_get_edge_count(TF_IO16V2 *io16_v2, uint8_t channel, bool reset_counter, uint32_t *ret_count) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(io16_v2->tfp);
        if (ret_count != NULL) { *ret_count = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        tf_tfp_packet_processed(io16_v2->tfp);
    }
...
int tf_io16_v2_set_edge_count_configuration(TF_IO16V2 *io16_v2, uint8_t channel, uint8_t edge_type, uint8_t debounce) {
    ...
    send_buf[0] = (uint8_t)channel;
    send_buf[1] = (uint8_t)edge_type;
    send_buf[2] = (uint8_t)debounce;
...
int tf_io16_v2_get_edge_count_configuration(TF_IO16V2 *io16_v2, uint8_t channel, uint8_t *ret_edge_type, uint8_t *ret_debounce) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(io16_v2->tfp);
        if (ret_edge_type != NULL) { *ret_edge_type = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        if (ret_debounce != NULL) { *ret_debounce = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(io16_v2->tfp);
    }
