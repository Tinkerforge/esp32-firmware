...
static bool tf_xmc1400_breakout_callback_handler(void *dev, uint8_t fid, TF_PacketBuffer *payload) {
    ...
    switch (fid) {
        ...
        case TF_XMC1400_BREAKOUT_CALLBACK_ADC_VALUES: {
            ...
            size_t i;
            uint16_t values[8]; for (i = 0; i < 8; ++i) values[i] = tf_packet_buffer_read_uint16_t(payload);
            TF_HALCommon *hal_common = tf_hal_get_common((TF_HAL*)xmc1400_breakout->tfp->hal);
...
int tf_xmc1400_breakout_set_gpio_config(TF_XMC1400Breakout *xmc1400_breakout, uint8_t port, uint8_t pin, uint8_t mode, uint8_t input_hysteresis, bool output_level) {
    ...
    if (tf_hal_get_common((TF_HAL*)xmc1400_breakout->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HAL*)xmc1400_breakout->tfp->hal) + tf_hal_get_common((TF_HAL*)xmc1400_breakout->tfp->hal)->timeout;
...
int tf_xmc1400_breakout_get_gpio_input(TF_XMC1400Breakout *xmc1400_breakout, uint8_t port, uint8_t pin, bool *ret_value) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_value != NULL) { *ret_value = tf_packet_buffer_read_bool(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 1); }
...
int tf_xmc1400_breakout_get_adc_channel_config(TF_XMC1400Breakout *xmc1400_breakout, uint8_t channel, bool *ret_enable) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_enable != NULL) { *ret_enable = tf_packet_buffer_read_bool(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_get_adc_channel_value(TF_XMC1400Breakout *xmc1400_breakout, uint8_t channel, uint16_t *ret_value) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_value != NULL) { *ret_value = tf_packet_buffer_read_uint16_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_get_adc_values(TF_XMC1400Breakout *xmc1400_breakout, uint16_t ret_values[8]) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_values != NULL) { for (i = 0; i < 8; ++i) ret_values[i] = tf_packet_buffer_read_uint16_t(&xmc1400_breakout->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 16); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_get_adc_values_callback_configuration(TF_XMC1400Breakout *xmc1400_breakout, uint32_t *ret_period, bool *ret_value_has_to_change) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_period != NULL) { *ret_period = tf_packet_buffer_read_uint32_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 4); }
        if (ret_value_has_to_change != NULL) { *ret_value_has_to_change = tf_packet_buffer_read_bool(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_get_count(TF_XMC1400Breakout *xmc1400_breakout, uint32_t *ret_count) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_count != NULL) { *ret_count = tf_packet_buffer_read_uint32_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_get_count_callback_configuration(TF_XMC1400Breakout *xmc1400_breakout, uint32_t *ret_period, bool *ret_value_has_to_change, char *ret_option, uint32_t *ret_min, uint32_t *ret_max) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_period != NULL) { *ret_period = tf_packet_buffer_read_uint32_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 4); }
        if (ret_value_has_to_change != NULL) { *ret_value_has_to_change = tf_packet_buffer_read_bool(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 1); }
        if (ret_option != NULL) { *ret_option = tf_packet_buffer_read_char(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 1); }
        if (ret_min != NULL) { *ret_min = tf_packet_buffer_read_uint32_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 4); }
        if (ret_max != NULL) { *ret_max = tf_packet_buffer_read_uint32_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_get_spitfp_error_count(TF_XMC1400Breakout *xmc1400_breakout, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packet_buffer_read_uint32_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packet_buffer_read_uint32_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packet_buffer_read_uint32_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packet_buffer_read_uint32_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_set_bootloader_mode(TF_XMC1400Breakout *xmc1400_breakout, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packet_buffer_read_uint8_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_get_bootloader_mode(TF_XMC1400Breakout *xmc1400_breakout, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packet_buffer_read_uint8_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_get_status_led_config(TF_XMC1400Breakout *xmc1400_breakout, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_get_chip_temperature(TF_XMC1400Breakout *xmc1400_breakout, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packet_buffer_read_int16_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_read_uid(TF_XMC1400Breakout *xmc1400_breakout, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packet_buffer_read_uint32_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(xmc1400_breakout->tfp);
    }
...
int tf_xmc1400_breakout_get_identity(TF_XMC1400Breakout *xmc1400_breakout, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packet_buffer_pop_n(&xmc1400_breakout->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 8); }
        tf_packet_buffer_pop_n(&xmc1400_breakout->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packet_buffer_read_char(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packet_buffer_read_uint8_t(&xmc1400_breakout->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 3); }
        if (ret_firmware_version != NULL) { for (i = 0; i < 3; ++i) ret_firmware_version[i] = tf_packet_buffer_read_uint8_t(&xmc1400_breakout->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 3); }
        if (ret_device_identifier != NULL) { *ret_device_identifier = tf_packet_buffer_read_uint16_t(&xmc1400_breakout->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&xmc1400_breakout->tfp->spitfp->recv_buf, 2); }
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HAL*)xmc1400_breakout->tfp->hal, xmc1400_breakout->tfp->spitfp->port_id);
        }
...
int tf_xmc1400_breakout_callback_tick(TF_XMC1400Breakout *xmc1400_breakout, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(xmc1400_breakout->tfp, tf_hal_current_time_us((TF_HAL*)xmc1400_breakout->tfp->hal) + timeout_us);
}
