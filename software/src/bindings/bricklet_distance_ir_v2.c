...
static bool tf_distance_ir_v2_callback_handler(void *dev, uint8_t fid, TF_PacketBuffer *payload) {
    ...
    switch (fid) {
        ...
        case TF_DISTANCE_IR_V2_CALLBACK_DISTANCE: {
            TF_DistanceIRV2DistanceHandler fn = distance_ir_v2->distance_handler;
            ...
            TF_HALCommon *hal_common = tf_hal_get_common((TF_HAL*)distance_ir_v2->tfp->hal);
        ...
        case TF_DISTANCE_IR_V2_CALLBACK_ANALOG_VALUE: {
            TF_DistanceIRV2AnalogValueHandler fn = distance_ir_v2->analog_value_handler;
...
int tf_distance_ir_v2_create(TF_DistanceIRV2 *distance_ir_v2, const char *uid, TF_HAL *hal) {
    ...
    distance_ir_v2->tfp->uid = numeric_uid;
...
int tf_distance_ir_v2_get_distance(TF_DistanceIRV2 *distance_ir_v2, uint16_t *ret_distance) {
    ...
    if (tf_hal_get_common((TF_HAL*)distance_ir_v2->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HAL*)distance_ir_v2->tfp->hal) + tf_hal_get_common((TF_HAL*)distance_ir_v2->tfp->hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_distance != NULL) { *ret_distance = tf_packet_buffer_read_uint16_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }

    result = tf_tfp_finish_send(distance_ir_v2->tfp, result, deadline);
...
int tf_distance_ir_v2_set_distance_callback_configuration(TF_DistanceIRV2 *distance_ir_v2, uint32_t period, bool value_has_to_change, char option, uint16_t min, uint16_t max) {
    ...
    bool response_expected = true;
    tf_distance_ir_v2_get_response_expected(distance_ir_v2, TF_DISTANCE_IR_V2_FUNCTION_SET_DISTANCE_CALLBACK_CONFIGURATION, &response_expected);
    tf_tfp_prepare_send(distance_ir_v2->tfp, TF_DISTANCE_IR_V2_FUNCTION_SET_DISTANCE_CALLBACK_CONFIGURATION, 10, 0, response_expected);

    uint8_t *buf = tf_tfp_get_payload_buffer(distance_ir_v2->tfp);
...
int tf_distance_ir_v2_get_distance_callback_configuration(TF_DistanceIRV2 *distance_ir_v2, uint32_t *ret_period, bool *ret_value_has_to_change, char *ret_option, uint16_t *ret_min, uint16_t *ret_max) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_period != NULL) { *ret_period = tf_packet_buffer_read_uint32_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_value_has_to_change != NULL) { *ret_value_has_to_change = tf_packet_buffer_read_bool(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_option != NULL) { *ret_option = tf_packet_buffer_read_char(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_min != NULL) { *ret_min = tf_packet_buffer_read_uint16_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_max != NULL) { *ret_max = tf_packet_buffer_read_uint16_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_get_analog_value(TF_DistanceIRV2 *distance_ir_v2, uint32_t *ret_analog_value) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_analog_value != NULL) { *ret_analog_value = tf_packet_buffer_read_uint32_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_set_analog_value_callback_configuration(TF_DistanceIRV2 *distance_ir_v2, uint32_t period, bool value_has_to_change, char option, uint32_t min, uint32_t max) {
    ...
    bool response_expected = true;
    tf_distance_ir_v2_get_response_expected(distance_ir_v2, TF_DISTANCE_IR_V2_FUNCTION_SET_ANALOG_VALUE_CALLBACK_CONFIGURATION, &response_expected);
    tf_tfp_prepare_send(distance_ir_v2->tfp, TF_DISTANCE_IR_V2_FUNCTION_SET_ANALOG_VALUE_CALLBACK_CONFIGURATION, 14, 0, response_expected);
...
int tf_distance_ir_v2_get_analog_value_callback_configuration(TF_DistanceIRV2 *distance_ir_v2, uint32_t *ret_period, bool *ret_value_has_to_change, char *ret_option, uint32_t *ret_min, uint32_t *ret_max) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_value_has_to_change != NULL) { *ret_value_has_to_change = tf_packet_buffer_read_bool(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_option != NULL) { *ret_option = tf_packet_buffer_read_char(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_min != NULL) { *ret_min = tf_packet_buffer_read_uint32_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_max != NULL) { *ret_max = tf_packet_buffer_read_uint32_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_set_moving_average_configuration(TF_DistanceIRV2 *distance_ir_v2, uint16_t moving_average_length) {
    ...
    bool response_expected = true;
    tf_distance_ir_v2_get_response_expected(distance_ir_v2, TF_DISTANCE_IR_V2_FUNCTION_SET_MOVING_AVERAGE_CONFIGURATION, &response_expected);
    tf_tfp_prepare_send(distance_ir_v2->tfp, TF_DISTANCE_IR_V2_FUNCTION_SET_MOVING_AVERAGE_CONFIGURATION, 2, 0, response_expected);
...
int tf_distance_ir_v2_get_moving_average_configuration(TF_DistanceIRV2 *distance_ir_v2, uint16_t *ret_moving_average_length) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_moving_average_length != NULL) { *ret_moving_average_length = tf_packet_buffer_read_uint16_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_set_distance_led_config(TF_DistanceIRV2 *distance_ir_v2, uint8_t config) {
    ...
    bool response_expected = true;
    tf_distance_ir_v2_get_response_expected(distance_ir_v2, TF_DISTANCE_IR_V2_FUNCTION_SET_DISTANCE_LED_CONFIG, &response_expected);
    tf_tfp_prepare_send(distance_ir_v2->tfp, TF_DISTANCE_IR_V2_FUNCTION_SET_DISTANCE_LED_CONFIG, 1, 0, response_expected);
...
int tf_distance_ir_v2_get_distance_led_config(TF_DistanceIRV2 *distance_ir_v2, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packet_buffer_read_uint8_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_set_sensor_type(TF_DistanceIRV2 *distance_ir_v2, uint8_t sensor) {
    ...
    bool response_expected = true;
    tf_distance_ir_v2_get_response_expected(distance_ir_v2, TF_DISTANCE_IR_V2_FUNCTION_SET_SENSOR_TYPE, &response_expected);
    tf_tfp_prepare_send(distance_ir_v2->tfp, TF_DISTANCE_IR_V2_FUNCTION_SET_SENSOR_TYPE, 1, 0, response_expected);
...
int tf_distance_ir_v2_get_sensor_type(TF_DistanceIRV2 *distance_ir_v2, uint8_t *ret_sensor) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_sensor != NULL) { *ret_sensor = tf_packet_buffer_read_uint8_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_get_spitfp_error_count(TF_DistanceIRV2 *distance_ir_v2, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packet_buffer_read_uint32_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packet_buffer_read_uint32_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packet_buffer_read_uint32_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packet_buffer_read_uint32_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_set_bootloader_mode(TF_DistanceIRV2 *distance_ir_v2, uint8_t mode, uint8_t *ret_status) {
    ...
    bool response_expected = true;
    tf_tfp_prepare_send(distance_ir_v2->tfp, TF_DISTANCE_IR_V2_FUNCTION_SET_BOOTLOADER_MODE, 1, 1, response_expected);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packet_buffer_read_uint8_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_get_bootloader_mode(TF_DistanceIRV2 *distance_ir_v2, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packet_buffer_read_uint8_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_set_write_firmware_pointer(TF_DistanceIRV2 *distance_ir_v2, uint32_t pointer) {
    ...
    bool response_expected = true;
    tf_distance_ir_v2_get_response_expected(distance_ir_v2, TF_DISTANCE_IR_V2_FUNCTION_SET_WRITE_FIRMWARE_POINTER, &response_expected);
    tf_tfp_prepare_send(distance_ir_v2->tfp, TF_DISTANCE_IR_V2_FUNCTION_SET_WRITE_FIRMWARE_POINTER, 4, 0, response_expected);
...
int tf_distance_ir_v2_write_firmware(TF_DistanceIRV2 *distance_ir_v2, const uint8_t data[64], uint8_t *ret_status) {
    ...
    bool response_expected = true;
    tf_tfp_prepare_send(distance_ir_v2->tfp, TF_DISTANCE_IR_V2_FUNCTION_WRITE_FIRMWARE, 64, 1, response_expected);
...
int tf_distance_ir_v2_set_status_led_config(TF_DistanceIRV2 *distance_ir_v2, uint8_t config) {
    ...
    bool response_expected = true;
    tf_distance_ir_v2_get_response_expected(distance_ir_v2, TF_DISTANCE_IR_V2_FUNCTION_SET_STATUS_LED_CONFIG, &response_expected);
    tf_tfp_prepare_send(distance_ir_v2->tfp, TF_DISTANCE_IR_V2_FUNCTION_SET_STATUS_LED_CONFIG, 1, 0, response_expected);
...
int tf_distance_ir_v2_get_chip_temperature(TF_DistanceIRV2 *distance_ir_v2, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packet_buffer_read_int16_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_write_uid(TF_DistanceIRV2 *distance_ir_v2, uint32_t uid) {
    ...
    bool response_expected = true;
    tf_distance_ir_v2_get_response_expected(distance_ir_v2, TF_DISTANCE_IR_V2_FUNCTION_WRITE_UID, &response_expected);
    tf_tfp_prepare_send(distance_ir_v2->tfp, TF_DISTANCE_IR_V2_FUNCTION_WRITE_UID, 4, 0, response_expected);
...
int tf_distance_ir_v2_read_uid(TF_DistanceIRV2 *distance_ir_v2, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packet_buffer_read_uint32_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_get_identity(TF_DistanceIRV2 *distance_ir_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packet_buffer_pop_n(&distance_ir_v2->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 8); }
        tf_packet_buffer_pop_n(&distance_ir_v2->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packet_buffer_read_char(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packet_buffer_read_uint8_t(&distance_ir_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 3); }
        if (ret_firmware_version != NULL) { for (i = 0; i < 3; ++i) ret_firmware_version[i] = tf_packet_buffer_read_uint8_t(&distance_ir_v2->tfp->spitfp->recv_buf);} else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 3); }
        if (ret_device_identifier != NULL) { *ret_device_identifier = tf_packet_buffer_read_uint16_t(&distance_ir_v2->tfp->spitfp->recv_buf); } else { tf_packet_buffer_remove(&distance_ir_v2->tfp->spitfp->recv_buf, 2); }
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HAL*)distance_ir_v2->tfp->hal, distance_ir_v2->tfp->spitfp->port_id);
        }
...
int tf_distance_ir_v2_register_distance_callback(TF_DistanceIRV2 *distance_ir_v2, TF_DistanceIRV2DistanceHandler handler, void *user_data) {
    ...
    if (handler == NULL) {
        ...
        distance_ir_v2->tfp->needs_callback_tick = true;
    }
...
int tf_distance_ir_v2_callback_tick(TF_DistanceIRV2 *distance_ir_v2, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(distance_ir_v2->tfp, tf_hal_current_time_us((TF_HAL*)distance_ir_v2->tfp->hal) + timeout_us);
}
