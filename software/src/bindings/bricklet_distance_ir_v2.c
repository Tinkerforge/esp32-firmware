...
int tf_distance_ir_v2_create(TF_DistanceIRV2 *distance_ir_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_DISTANCE_IR_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_DISTANCE_IR_V2_DEVICE_IDENTIFIER;
...
int tf_distance_ir_v2_destroy(TF_DistanceIRV2 *distance_ir_v2) {
    if (distance_ir_v2 == NULL || distance_ir_v2->tfp == NULL) {
        return TF_E_NULL;
    }
...
int tf_distance_ir_v2_get_distance(TF_DistanceIRV2 *distance_ir_v2, uint16_t *ret_distance) {
    ...
    int result = tf_tfp_send_packet(distance_ir_v2->tfp, response_expected, deadline, &error_code);
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(distance_ir_v2->tfp);
        if (ret_distance != NULL) { *ret_distance = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
...
int tf_distance_ir_v2_set_distance_callback_configuration(TF_DistanceIRV2 *distance_ir_v2, uint32_t period, bool value_has_to_change, char option, uint16_t min, uint16_t max) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(distance_ir_v2->tfp);
...
int tf_distance_ir_v2_get_analog_value(TF_DistanceIRV2 *distance_ir_v2, uint32_t *ret_analog_value) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(distance_ir_v2->tfp);
        if (ret_analog_value != NULL) { *ret_analog_value = tf_packet_buffer_read_uint32_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 4); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_set_moving_average_configuration(TF_DistanceIRV2 *distance_ir_v2, uint16_t moving_average_length) {
    ...
    moving_average_length = tf_leconvert_uint16_to(moving_average_length); memcpy(send_buf + 0, &moving_average_length, 2);
...
int tf_distance_ir_v2_get_moving_average_configuration(TF_DistanceIRV2 *distance_ir_v2, uint16_t *ret_moving_average_length) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(distance_ir_v2->tfp);
        if (ret_moving_average_length != NULL) { *ret_moving_average_length = tf_packet_buffer_read_uint16_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 2); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
...
int tf_distance_ir_v2_get_sensor_type(TF_DistanceIRV2 *distance_ir_v2, uint8_t *ret_sensor) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(distance_ir_v2->tfp);
        if (ret_sensor != NULL) { *ret_sensor = tf_packet_buffer_read_uint8_t(recv_buf); } else { tf_packet_buffer_remove(recv_buf, 1); }
        tf_tfp_packet_processed(distance_ir_v2->tfp);
    }
