...
static bool tf_dc_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_DC_V2_CALLBACK_EMERGENCY_SHUTDOWN: {
            TF_DCV2EmergencyShutdownHandler fn = dc_v2->emergency_shutdown_handler;
            void *user_data = dc_v2->emergency_shutdown_user_data;
            if (fn == NULL)
                return false;


            TF_HalCommon *common = tf_hal_get_common((TF_HalContext*)dc_v2->tfp->hal);
            common->locked = true;
            fn(dc_v2, user_data);
            common->locked = false;
            break;
        ...
        case TF_DC_V2_CALLBACK_VELOCITY_REACHED: {
            TF_DCV2VelocityReachedHandler fn = dc_v2->velocity_reached_handler;
        ...
        case TF_DC_V2_CALLBACK_CURRENT_VELOCITY: {
            TF_DCV2CurrentVelocityHandler fn = dc_v2->current_velocity_handler;
...
int tf_dc_v2_set_enabled(TF_DCV2 *dc_v2, bool enabled) {
    ...
    if(tf_hal_get_common((TF_HalContext*)dc_v2->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HalContext*)dc_v2->tfp->hal) + tf_hal_get_common((TF_HalContext*)dc_v2->tfp->hal)->timeout;
...
int tf_dc_v2_get_enabled(TF_DCV2 *dc_v2, bool *ret_enabled) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_enabled != NULL) { *ret_enabled = tf_packetbuffer_read_bool(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 1); }
...
int tf_dc_v2_get_velocity(TF_DCV2 *dc_v2, int16_t *ret_velocity) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_velocity != NULL) { *ret_velocity = tf_packetbuffer_read_int16_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(dc_v2->tfp);
    }
...
int tf_dc_v2_get_motion(TF_DCV2 *dc_v2, uint16_t *ret_acceleration, uint16_t *ret_deceleration) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_acceleration != NULL) { *ret_acceleration = tf_packetbuffer_read_uint16_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_deceleration != NULL) { *ret_deceleration = tf_packetbuffer_read_uint16_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(dc_v2->tfp);
    }
...
int tf_dc_v2_get_drive_mode(TF_DCV2 *dc_v2, uint8_t *ret_mode) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_mode != NULL) { *ret_mode = tf_packetbuffer_read_uint8_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(dc_v2->tfp);
    }
...
int tf_dc_v2_get_pwm_frequency(TF_DCV2 *dc_v2, uint16_t *ret_frequency) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_frequency != NULL) { *ret_frequency = tf_packetbuffer_read_uint16_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(dc_v2->tfp);
    }
...
int tf_dc_v2_get_power_statistics(TF_DCV2 *dc_v2, uint16_t *ret_voltage, uint16_t *ret_current) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_voltage != NULL) { *ret_voltage = tf_packetbuffer_read_uint16_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 2); }
        if (ret_current != NULL) { *ret_current = tf_packetbuffer_read_uint16_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(dc_v2->tfp);
    }
...
int tf_dc_v2_get_error_led_config(TF_DCV2 *dc_v2, uint8_t *ret_config) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_config != NULL) { *ret_config = tf_packetbuffer_read_uint8_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(dc_v2->tfp);
    }
...
int tf_dc_v2_get_current_velocity_callback_configuration(TF_DCV2 *dc_v2, uint32_t *ret_period, bool *ret_value_has_to_change) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_period != NULL) { *ret_period = tf_packetbuffer_read_uint32_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_value_has_to_change != NULL) { *ret_value_has_to_change = tf_packetbuffer_read_bool(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(dc_v2->tfp);
    }
...
int tf_dc_v2_get_spitfp_error_count(TF_DCV2 *dc_v2, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_error_count_ack_checksum != NULL) { *ret_error_count_ack_checksum = tf_packetbuffer_read_uint32_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_message_checksum != NULL) { *ret_error_count_message_checksum = tf_packetbuffer_read_uint32_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_frame != NULL) { *ret_error_count_frame = tf_packetbuffer_read_uint32_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 4); }
        if (ret_error_count_overflow != NULL) { *ret_error_count_overflow = tf_packetbuffer_read_uint32_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(dc_v2->tfp);
    }
...
int tf_dc_v2_set_bootloader_mode(TF_DCV2 *dc_v2, uint8_t mode, uint8_t *ret_status) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_status != NULL) { *ret_status = tf_packetbuffer_read_uint8_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 1); }
        tf_tfp_packet_processed(dc_v2->tfp);
    }
...
int tf_dc_v2_get_chip_temperature(TF_DCV2 *dc_v2, int16_t *ret_temperature) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_temperature != NULL) { *ret_temperature = tf_packetbuffer_read_int16_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 2); }
        tf_tfp_packet_processed(dc_v2->tfp);
    }
...
int tf_dc_v2_read_uid(TF_DCV2 *dc_v2, uint32_t *ret_uid) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        if (ret_uid != NULL) { *ret_uid = tf_packetbuffer_read_uint32_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 4); }
        tf_tfp_packet_processed(dc_v2->tfp);
    }
...
int tf_dc_v2_get_identity(TF_DCV2 *dc_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (ret_uid != NULL) { tf_packetbuffer_pop_n(&dc_v2->tfp->spitfp->recv_buf, (uint8_t*)ret_uid, 8);} else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 8); }
        tf_packetbuffer_pop_n(&dc_v2->tfp->spitfp->recv_buf, (uint8_t*)tmp_connected_uid, 8);
        if (ret_position != NULL) { *ret_position = tf_packetbuffer_read_char(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 1); }
        if (ret_hardware_version != NULL) { for (i = 0; i < 3; ++i) ret_hardware_version[i] = tf_packetbuffer_read_uint8_t(&dc_v2->tfp->spitfp->recv_buf);} else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 3); }
        if (ret_firmware_version != NULL) { for (i = 0; i < 3; ++i) ret_firmware_version[i] = tf_packetbuffer_read_uint8_t(&dc_v2->tfp->spitfp->recv_buf);} else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 3); }
        if (ret_device_identifier != NULL) { *ret_device_identifier = tf_packetbuffer_read_uint16_t(&dc_v2->tfp->spitfp->recv_buf); } else { tf_packetbuffer_remove(&dc_v2->tfp->spitfp->recv_buf, 2); }
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HalContext*)dc_v2->tfp->hal, dc_v2->tfp->spitfp->port_id);
        }
...
int tf_dc_v2_callback_tick(TF_DCV2 *dc_v2, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(dc_v2->tfp, tf_hal_current_time_us((TF_HalContext*)dc_v2->tfp->hal) + timeout_us);
}
