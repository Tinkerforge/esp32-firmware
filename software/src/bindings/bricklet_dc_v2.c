...
int tf_dc_v2_create(TF_DCV2 *dc_v2, const char *uid, TF_HAL *hal) {
    ...
    if (uid != NULL && *uid != '\0') {
        ...
        if (tfp->device_id != TF_DC_V2_DEVICE_IDENTIFIER) {
            return TF_E_WRONG_DEVICE_TYPE;
        }
    } else {
        uint16_t device_id = TF_DC_V2_DEVICE_IDENTIFIER;
...
int tf_dc_v2_set_enabled(TF_DCV2 *dc_v2, bool enabled) {
    ...
    uint8_t *send_buf = tf_tfp_get_send_payload_buffer(dc_v2->tfp);
    ...
    int result = tf_tfp_send_packet(dc_v2->tfp, response_expected, deadline, &error_code);
...
int tf_dc_v2_get_enabled(TF_DCV2 *dc_v2, bool *ret_enabled) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        TF_PacketBuffer *recv_buf = tf_tfp_get_receive_buffer(dc_v2->tfp);
