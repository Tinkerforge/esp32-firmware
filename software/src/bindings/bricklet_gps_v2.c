...
static bool tf_gps_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_GPS_V2_CALLBACK_PULSE_PER_SECOND: {
            ...
            TF_HalCommon *common = tf_hal_get_common((TF_HalContext*)gps_v2->tfp->hal);
            common->locked = true;
            fn(gps_v2, user_data);
            common->locked = false;
            break;
...
int tf_gps_v2_get_coordinates(TF_GPSV2 *gps_v2, uint32_t *ret_latitude, char *ret_ns, uint32_t *ret_longitude, char *ret_ew) {
    ...
    if(tf_hal_get_common((TF_HalContext*)gps_v2->tfp->hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us((TF_HalContext*)gps_v2->tfp->hal) + tf_hal_get_common((TF_HalContext*)gps_v2->tfp->hal)->timeout;
...
int tf_gps_v2_get_identity(TF_GPSV2 *gps_v2, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name((TF_HalContext*)gps_v2->tfp->hal, gps_v2->tfp->spitfp->port_id);
        }
...
int tf_gps_v2_callback_tick(TF_GPSV2 *gps_v2, uint32_t timeout_us) {
    ...
    return tf_tfp_callback_tick(gps_v2->tfp, tf_hal_current_time_us((TF_HalContext*)gps_v2->tfp->hal) + timeout_us);
}
