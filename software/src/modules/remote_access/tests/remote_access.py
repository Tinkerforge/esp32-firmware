...
def _do_full_registration_with_user_tunnel(
    ...
    Note: now that the device generates WireGuard keys on-board, this helper
    can no longer recover the device-generated PSK (it is sealed with the
    relay's NaCl seal public key).  The charger public key is still readable
    from the registration body that the device sends to the relay, but the PSK
    is not.  The dependent tests therefore currently fail this helper with a
    NotImplementedError; re-implement them to read the user-tunnel keys from
    the device's persisted state instead.

    Returns:
        (wg, user_charger_public, user_psk)

    Raises:
        RuntimeError: Either because WireGuard setup fails, or because the
            user-tunnel PSK can no longer be recovered after the device
            started generating WireGuard keys on-board.  Callers should treat
            this as "skip" and re-implement the dependent tests to read the
            keys from the device's persisted state.
        AssertionError: If registration fails within timeout.
    """
    # Generate the relay-side keys. The charger-side WireGuard keys are
    # generated by the device itself during /remote_access/register.
    wg = WireGuardTestPeer()
    wg.generate_keys()
    ...
    raise RuntimeError(
        "After moving WireGuard key generation to the device, this helper can "
        "no longer recover the user-tunnel PSK (it is sealed with the relay's "
        "NaCl seal key in transit). Re-implement the dependent tests to read "
        "the user-tunnel keys from the device's persisted state."
    )
