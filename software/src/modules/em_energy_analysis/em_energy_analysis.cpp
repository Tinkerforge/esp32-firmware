...
void EMEnergyAnalysis::load_persistent_data_v1(uint8_t *buf)
{
    ...
    if (internet_checksum(&data_v1, sizeof(data_v1)) != 0) {
...
bool EMEnergyAnalysis::load_persistent_data_v2(uint8_t *buf)
{
    ...
    if (internet_checksum(&data_v2, sizeof(data_v2)) != 0) {
    ...
    for (uint32_t slot = 1; slot < METERS_MAX_SLOTS_RECORDED; ++slot) {
...
void EMEnergyAnalysis::load_persistent_data_v3(uint8_t *buf, uint32_t start_slot)
{
    ...
    if (internet_checksum(&data_v3, sizeof(data_v3)) != 0) {
    ...
    for (uint32_t slot = start_slot; slot < start_slot + 3 && slot < METERS_MAX_SLOTS_RECORDED; ++slot) {
...
void EMEnergyAnalysis::save_persistent_data()
{
    ...
    data_v1.checksum = internet_checksum(&data_v1, sizeof(data_v1));
    ...
    data_v2.version = 2;
    data_v2.checksum = internet_checksum(&data_v2, sizeof(data_v2));
    ...
    data_v3a.version = 3;
    data_v3a.checksum = internet_checksum(&data_v3a, sizeof(data_v3a));

    for (uint32_t slot = 4; slot < METERS_MAX_SLOTS_RECORDED; ++slot) {
    ...
    data_v3b.version = 3;
    data_v3b.checksum = internet_checksum(&data_v3b, sizeof(data_v3b));
    ...
    if (rc != TF_E_OK) {
        ...
        for (int i = 0; i < 7; ++i) {
            if (power[i] != INT32_MAX) {
                snprintf(power_str[i], sizeof(power_str[i]), "%li", power[i]);
            }
        }
                                                           ...
                                                           int32_t price_max /* ct/kWh */)
{
    ...
    if (rc != TF_E_OK) {
        ...
        if (price_min != INT32_MAX) {
            snprintf(price_min_str, sizeof(price_min_str), "%li", price_min);
        }

        if (price_avg != INT32_MAX) {
            snprintf(price_avg_str, sizeof(price_avg_str), "%li", price_avg);
        }

        if (price_max != INT32_MAX) {
            snprintf(price_max_str, sizeof(price_max_str), "%li", price_max);
        }
                ...
                task_scheduler.scheduleOnce([metadata, response]{
                    ...
                    if (rc != TF_E_OK || status != 0) {
                        if (rc != TF_E_OK) {
                            ...
                            logger.printfln("Failed to continue getting wallbox 5min data point: seqnum %lu, status (%s, %hhu)", metadata->seqnum, get_data_status_string(status), status);
                        }
        ...
        if (metadata->utc_end_slots > 0) {
            if (!write_result) {
                ...
                task_scheduler.scheduleOnce([metadata, response]{
                    ...
                    if (rc != TF_E_OK || status != 0) {
                        if (rc != TF_E_OK) {
                            logger.printfln("Failed to continue getting energy manager 5min data point: seqnum %lu, error %i", metadata->seqnum, rc);
                        }
                        else if (status != 0) {
                            logger.printfln("Failed to continue getting energy manager 5min data point: seqnum %lu, status (%s, %hhu)", metadata->seqnum, get_data_status_string(status), status);
                        }
