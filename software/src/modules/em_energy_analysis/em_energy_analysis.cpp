...
{
    switch (status) {
        case TF_EM_DATA_STATUS_OK:         return "OK";
        case TF_EM_DATA_SDATUS_SD_ERROR:   return "SD error";
        case TF_EM_DATA_SDATUS_LFS_ERROR:  return "LFS error";
        case TF_EM_DATA_SDATUS_QUEUE_FULL: return "queue full";
    }
...
void EMEnergyAnalysis::collect_data_points()
{
    ...
    if (current_5min_slot != last_history_5min_slot) {
        ...
        if (all_data_common->is_valid && !deadline_elapsed(all_data_common->last_update + MAX_DATA_AGE)) {
            ...
            //flags |= all_data.input[0]             ? 0b0010 : 0; // TODO FIXME
...
bool EMEnergyAnalysis::set_wallbox_5min_data_point(const struct tm *utc, const struct tm *local, uint32_t uid, uint8_t flags, uint16_t power /* W */)
{
    ...
    if (rc != TF_E_OK) {
        ...
        if (status == TF_EM_DATA_SDATUS_QUEUE_FULL) {
            return false; // retry
        }
        else {
...
static void wallbox_5min_data_points_handler(TF_EM_BRICKLET_TYPE *device, uint16_t data_length, uint16_t data_chunk_offset, uint8_t data_chunk_data[60], void *user_data)
{
...
static void wallbox_daily_data_points_handler(TF_EM_BRICKLET_TYPE *device,
...
static void energy_manager_5min_data_points_handler(TF_EM_BRICKLET_TYPE *device,
                                                    uint16_t data_length,
                                                    uint16_t data_chunk_offset,
...
static void energy_manager_daily_data_points_handler(TF_EM_BRICKLET_TYPE *device,
                                                     uint16_t data_length,
                                                     uint16_t data_chunk_offset,
