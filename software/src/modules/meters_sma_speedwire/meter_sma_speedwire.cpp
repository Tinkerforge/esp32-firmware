...
int MeterSMASpeedwire::parse_header(SpeedwireHeader *header)
{
    ...
    // Protocol ID 0x6073 is for unknown use case with unknown data, but we have seen it in the wild (generated by sunny home manager).
    if (header->protocol_id == 0x6073) {
        return 0;
    }
