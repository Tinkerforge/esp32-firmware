...
extern const char local_uid_str[32];
...
// Default: 30 files with 256 records each: 7680 records @ ~ max. 10 records per day = ~ 2 years and one month of records.
// With charge management: up to max(30, managed_chargers * 2) files to accommodate multiple managed chargers.
...
bool ChargeTracker::startCharge(uint32_t timestamp_minutes, float meter_start, uint8_t user_id, uint32_t evse_uptime, uint8_t auth_type, Config::ConfVariant auth_info, const char *directory) {
...
void ChargeTracker::endCharge(uint32_t charge_duration_seconds, float meter_end, const char *directory)
{
    std::lock_guard<std::mutex> lock{records_mutex};
    ChargeEnd ce;
...
void ChargeTracker::register_urls()
{
    ...
    api.addState("charge_tracker/current_charges", &current_charges, {}, {"tag_id", "tag_type"});
    ...
    server.on_HTTPThread("/charge_tracker/pdf", HTTP_PUT, [this](WebServerRequest request) {
        ...
        this->generate_pdf(callback, user_filter, device_filter, start_timestamp_min, end_timestamp_min, current_timestamp_min, language, letterhead, letterhead_lines, &request);
        return request.endChunkedResponse();
    });
...
#define TABLE_LINE_LEN (17 /* start date: 01.02.3456 12:34\0 or 3456-02-01 12:34\0 */ \
    ...
    init_pdf_generator(callback,
        ...
        while (any_charges_tracked && current_file <= last_file) {
            ...
            if (!f) {
                f =  LittleFS.open(chargeRecordFilename(current_file, nullptr));
