...
int64_t Event::registerEvent(const String &path, const std::vector<ConfPath> values, std::function<EventResult(const Config *)> &&callback)
{
    ...
    // Walk ConfPath to find requested member inside state.
    ...
    if (values_count > 0) {
        conf_path = heap_alloc_array<ConfPath>(values_count);

        for (const ConfPath &value : values) {
...
{
    char msg[128];
    snprintf(msg, ARRAY_SIZE(msg), "Reached end of nested config in %s before reaching end of ConfPath", state_path);
    esp_system_abort(msg);
}
...
bool Event::pushStateUpdate(size_t stateIdx, const String &/*payload*/, const String &path)
{
    ...
    if (*reg_ptr) {
        ...
        do {
            ...
            // Walk registration's ConfPath to find desired Config.
            for (size_t conf_path_idx = 0; conf_path_idx < reg->conf_path_len; ++conf_path_idx) {
                const ConfPath *value = &reg->conf_path[conf_path_idx];
                const char *const *obj_variant = strict_variant::get<const char *>(value);
