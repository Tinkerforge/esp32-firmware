...
void EVSEV2::update_all_data()
{
...
#if OPTIONS_PRODUCT_ID_IS_WARP2()
            logger.printfln("Contactor error %u", contactor_error);
    ...
    if (evse_common.management_enabled.get("enabled")->updateBool(SLOT_ACTIVE(active_and_clear_on_disconnect[CHARGING_SLOT_CHARGE_MANAGER]))) {
#if MODULE_AUTOMATION_AVAILABLE()
        automation.set_enabled(AutomationActionID::EVSEChargeMode, SLOT_ACTIVE(active_and_clear_on_disconnect[CHARGING_SLOT_CHARGE_MANAGER]));
#endif
    }
    ...
    evse_common.eebus_enabled.get("enabled")->updateBool(SLOT_ACTIVE(active_and_clear_on_disconnect[CHARGING_SLOT_EEBUS]));
    evse_common.p14a_enwg_enabled.get("enabled")->updateBool(SLOT_ACTIVE(active_and_clear_on_disconnect[CHARGING_SLOT_P14A_ENWG]));
...
bool EVSEV2::is_shutdown_input_closed()
{
#if OPTIONS_PRODUCT_ID_IS_WARP2()
