...
struct SunSpecDeltaConnectThreePhaseMeterModel204_s {
                    uint16_t ID;              //   0
                    uint16_t L;               //   1
                    int16_t  A;               //   2
                    int16_t  AphA;            //   3
...
#define SUNSPEC_MODEL_204_NAME "Delta-Connect Three Phase Meter"

// Total size in bytes, includes ID and length registers.
#define SUNSPEC_MODEL_204_SIZE (214)

// Register count without ID and length registers, should match content of length register.
#define SUNSPEC_MODEL_204_LENGTH (105)

union SunSpecDeltaConnectThreePhaseMeterModel204_u {
    struct SunSpecDeltaConnectThreePhaseMeterModel204_s model;
    uint16_t registers[SUNSPEC_MODEL_204_SIZE / sizeof(uint16_t)];
};

static_assert(sizeof(SunSpecDeltaConnectThreePhaseMeterModel204_s) == 214, "Incorrect Delta-Connect Three Phase Meter length.");
static_assert(sizeof(SunSpecDeltaConnectThreePhaseMeterModel204_u) == 214, "Incorrect Delta-Connect Three Phase Meter length.");
static_assert(sizeof(static_cast<SunSpecDeltaConnectThreePhaseMeterModel204_u *>(nullptr)->registers) == 214, "Incorrect Delta-Connect Three Phase Meter length.");
