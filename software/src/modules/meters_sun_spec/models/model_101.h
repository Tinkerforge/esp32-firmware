...
#define SUNSPEC_MODEL_101_SIZE (104)

// Register count without ID and length registers, should match content of length register.
#define SUNSPEC_MODEL_101_LENGTH (50)

union SunSpecInverterModel101_u {
    struct SunSpecInverterModel101_s model;
    uint16_t registers[SUNSPEC_MODEL_101_SIZE / sizeof(uint16_t)];
};

static_assert(sizeof(SunSpecInverterModel101_s) == 104, "Incorrect Inverter length.");
static_assert(sizeof(SunSpecInverterModel101_u) == 104, "Incorrect Inverter length.");
static_assert(sizeof(static_cast<SunSpecInverterModel101_u *>(nullptr)->registers) == 104, "Incorrect Inverter length.");
