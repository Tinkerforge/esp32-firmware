...
struct SunSpecSplitSinglePhaseMeterModel212_s {
                    uint16_t ID;              //   0
                    uint16_t L;               //   1
...
#define SUNSPEC_MODEL_212_NAME "Split Single Phase Meter"

// Total size in bytes, includes ID and length registers.
#define SUNSPEC_MODEL_212_SIZE (252)

// Register count without ID and length registers, should match content of length register.
#define SUNSPEC_MODEL_212_LENGTH (124)

union SunSpecSplitSinglePhaseMeterModel212_u {
    struct SunSpecSplitSinglePhaseMeterModel212_s model;
    uint16_t registers[SUNSPEC_MODEL_212_SIZE / sizeof(uint16_t)];
};

static_assert(sizeof(SunSpecSplitSinglePhaseMeterModel212_s) == 252, "Incorrect Split Single Phase Meter length.");
static_assert(sizeof(SunSpecSplitSinglePhaseMeterModel212_u) == 252, "Incorrect Split Single Phase Meter length.");
static_assert(sizeof(static_cast<SunSpecSplitSinglePhaseMeterModel212_u *>(nullptr)->registers) == 252, "Incorrect Split Single Phase Meter length.");
