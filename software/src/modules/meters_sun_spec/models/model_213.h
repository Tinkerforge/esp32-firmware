...
struct SunSpecWyeConnectThreePhaseMeterModel213_s {
                    uint16_t ID;              //   0
                    uint16_t L;               //   1
...
#define SUNSPEC_MODEL_213_NAME "Wye-Connect Three Phase Meter"

// Total size in bytes, includes ID and length registers.
#define SUNSPEC_MODEL_213_SIZE (252)

// Register count without ID and length registers, should match content of length register.
#define SUNSPEC_MODEL_213_LENGTH (124)

union SunSpecWyeConnectThreePhaseMeterModel213_u {
    struct SunSpecWyeConnectThreePhaseMeterModel213_s model;
    uint16_t registers[SUNSPEC_MODEL_213_SIZE / sizeof(uint16_t)];
};

static_assert(sizeof(SunSpecWyeConnectThreePhaseMeterModel213_s) == 252, "Incorrect Wye-Connect Three Phase Meter length.");
static_assert(sizeof(SunSpecWyeConnectThreePhaseMeterModel213_u) == 252, "Incorrect Wye-Connect Three Phase Meter length.");
static_assert(sizeof(static_cast<SunSpecWyeConnectThreePhaseMeterModel213_u *>(nullptr)->registers) == 252, "Incorrect Wye-Connect Three Phase Meter length.");
