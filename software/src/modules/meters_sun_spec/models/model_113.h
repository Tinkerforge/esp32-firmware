...
#define SUNSPEC_MODEL_113_SIZE (124)

// Register count without ID and length registers, should match content of length register.
#define SUNSPEC_MODEL_113_LENGTH (60)

union SunSpecInverterFLOATModel113_u {
    struct SunSpecInverterFLOATModel113_s model;
    uint16_t registers[SUNSPEC_MODEL_113_SIZE / sizeof(uint16_t)];
};

static_assert(sizeof(SunSpecInverterFLOATModel113_s) == 124, "Incorrect Inverter FLOAT length.");
static_assert(sizeof(SunSpecInverterFLOATModel113_u) == 124, "Incorrect Inverter FLOAT length.");
static_assert(sizeof(static_cast<SunSpecInverterFLOATModel113_u *>(nullptr)->registers) == 124, "Incorrect Inverter FLOAT length.");
