...
struct SunSpecSinglePhaseMeterModel201_s {
                         uint16_t ID;              //   0
                         uint16_t L;               //   1
                         int16_t  A;               //   2
                         int16_t  AphA;            //   3
...
#define SUNSPEC_MODEL_201_NAME "Single Phase Meter"

// Total size in bytes, includes ID and length registers.
#define SUNSPEC_MODEL_201_SIZE (214)

// Register count without ID and length registers, should match content of length register.
#define SUNSPEC_MODEL_201_LENGTH (105)

union SunSpecSinglePhaseMeterModel201_u {
    struct SunSpecSinglePhaseMeterModel201_s model;
    uint16_t registers[SUNSPEC_MODEL_201_SIZE / sizeof(uint16_t)];
};

static_assert(sizeof(SunSpecSinglePhaseMeterModel201_s) == 214, "Incorrect Single Phase Meter length.");
static_assert(sizeof(SunSpecSinglePhaseMeterModel201_u) == 214, "Incorrect Single Phase Meter length.");
static_assert(sizeof(static_cast<SunSpecSinglePhaseMeterModel201_u *>(nullptr)->registers) == 214, "Incorrect Single Phase Meter length.");
