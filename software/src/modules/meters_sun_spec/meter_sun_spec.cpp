...
void MeterSunSpec::setup(Config *ephemeral_config)
{
    ...
    model_parser      = MetersSunSpecParser::new_parser(slot, manufacturer_name.c_str(), model_id, dc_port_type);
                ...
                if (scan_model_id == 1) { // Common model
                    ...
                    if (scan_device_found) {
                        if (is_kostal(m->Mn)) {
                            ...
                            quirks |= SUN_SPEC_QUIRKS_ACTIVE_POWER_IS_INVERTED;
                            quirks |= SUN_SPEC_QUIRKS_PHASE_TO_PHASE_VOLTAGE_IS_INVALID;
                        }
