...
void MeterMeta::register_events()
{
    if (source_meter_a == slot || (source_mode == SourceMode::Double && source_meter_b == slot)) {
        logger.printfln_meter("Invalid source meter. Meta meter cannot listen to itself. Slot A is %u, slot b is %u", source_meter_a, source_meter_b);
        return;
    }
...
EventResult MeterMeta::on_value_ids_change(const Config *value_ids)
{
    ...
    if (mode == ConfigMode::Add || mode == ConfigMode::Mul) {
        logger.printfln_meter("Mode %u not supported yet", static_cast<uint32_t>(mode));
        ...
        for (size_t i = 0; i < ARRAY_SIZE(index_cache); i++) {
            if (index_cache[i] == UINT32_MAX) {
                logger.printfln_meter("Value ID %u in position %u not provided by meter", static_cast<uint32_t>(value_ids_pf2current[i]), i);
                return EventResult::OK; // Try again, if possible.
            }
            if (index_cache[i] > UINT8_MAX) {
                logger.printfln_meter("Index %u of value ID %u in position %u is out of range", index_cache[i], static_cast<uint32_t>(value_ids_pf2current[i]), i);
                return EventResult::OK; // Try again, if possible.
            }
        }
...
void MeterMeta::on_values_change_single(const Config *source_values)
{
    if (mode == ConfigMode::Pf2Current) {
        ...
        logger.printfln_meter("Unsupported single values mode %u", static_cast<uint32_t>(mode));
    }
}
...
void MeterMeta::on_values_change_task_double()
{
    ...
    for (size_t i = 0; i < value_count; i++) {
        ...
        if (mode == ConfigMode::Sum) {
            ...
            logger.printfln_meter("Unsupported mode %u", static_cast<uint32_t>(mode));
            value = NAN;
        }
