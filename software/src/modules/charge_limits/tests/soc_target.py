...
def test_default_limits_has_soc_field(tc: TestContext):
    ...
    if tc.device_type().is_warp(4):
        tc.assert_in(config, "soc_pct")
    else:
        tc.assert_not("soc_pct" in config)
...
def test_set_default_soc_target(tc: TestContext):
    ...
    tc.assert_eq(80, updated["soc_pct"])
...
def test_active_limits_has_soc_field(tc: TestContext):
    ...
    if tc.device_type().is_warp(4):
        tc.assert_in(active, "soc_pct")
    else:
        tc.assert_not("soc_pct" in active)
...
def test_override_soc_updates_active_limits(tc: TestContext):
    ...
    tc.api("charge_limits/override_soc", {"soc_pct": 75})

    def _check_set():
        active = tc.api("charge_limits/active_limits")
        tc.assert_eq(75, active["soc_pct"])

    tc.wait_for(_check_set)

    # Disable by setting to 0
    tc.api("charge_limits/override_soc", {"soc_pct": 0})

    def _check_reset():
        active = tc.api("charge_limits/active_limits")
        tc.assert_eq(0, active["soc_pct"])
...
def test_state_has_soc_fields(tc: TestContext):
    ...
    if tc.device_type().is_warp(4):
        tc.assert_in(state, "target_soc_pct")
    else:
        tc.assert_not("target_soc_pct" in state)
...
def test_override_soc_updates_state(tc: TestContext):
    ...
    tc.api("charge_limits/override_soc", {"soc_pct": 90})

    def _check():
        tc.assert_eq(90, tc.api("charge_limits/state/target_soc_pct"))
