...
void MeterRS485Bricklet::cb_read_meter_type(TF_RS485 *rs485, uint8_t request_id, int8_t exception_code, uint16_t *holding_registers, uint16_t holding_registers_length) {
    if (request_id != callback_data.expected_request_id || callback_data.expected_request_id == 0) {
        logger.printfln_meter("Unexpected request ID %u, expected %u", request_id, callback_data.expected_request_id);
    ...
    if (exception_code != 0) {
        if (exception_code != -1)
            logger.printfln_meter("Request ID %u: Exception code %d", request_id, exception_code);
        callback_data.done = MeterRS485Bricklet::UserDataDone::ERROR;
        return;
    }

    if (callback_data.value_to_write == nullptr) {
        logger.printfln_meter("Value to write was nullptr");
        callback_data.done = MeterRS485Bricklet::UserDataDone::ERROR;
        return;
    }
    ...
    for (size_t i = 0; i < ARRAY_SIZE(supported_meters); ++i) {
        ...
        logger.printfln_meter("%s detected", this->meter_in_use->meter_name);
        return;
    }

    logger.printfln_meter("Found unknown meter type 0x%x. Assuming this is a SDM72DM", meter_id);
...
void MeterRS485Bricklet::cb_write_reset(TF_RS485 *device, uint8_t request_id, int8_t exception_code)
{
    ...
    if (exception_code != 0 && exception_code != TF_RS485_EXCEPTION_CODE_TIMEOUT) {
        logger.printfln_meter("Exception code %d", exception_code);
        callback_data.done = MeterRS485Bricklet::UserDataDone::ERROR;
        return;
    }
...
void MeterRS485Bricklet::setupMeter()
{
    ...
    if ((this->type_override - 1) < ARRAY_SIZE(supported_meters)) {
        ...
        logger.printfln_meter("Meter type override set to %s", this->meter_in_use->meter_name);
    } else {
        if (this->type_override != METER_TYPE_AUTO_DETECT)
            logger.printfln_meter("Meter type override set to unknown value %u. Ignoring", this->type_override);
...
void MeterRS485Bricklet::setup(Config *ephemeral_config)
{
    ...
    if (this->type_override == METER_TYPE_NONE) {
        logger.printfln_meter("Meter type override set to NONE (0). Disabling energy meter support");
        return;
    }
...
void MeterRS485Bricklet::tick()
{
    ...
    if (callback_data.done == UserDataDone::NOT_DONE) {
        logger.printfln_meter("RS485 deadline reached!");
