...
void Meters::register_urls()
{
    for (uint32_t slot = 0; slot < OPTIONS_METERS_MAX_SLOTS(); slot++) {
        ...
        if (meter_slot.meter->supports_reset()) {
            api.addCommand(get_path(slot, Meters::PathType::Reset), Config::Null(), {}, [this, &meter_slot, slot](Language /*language*/, String &/*errmsg*/) mutable {
    ...
    if (cached_index == UINT32_MAX) {
        *value_out = NAN;
        ...
        switch (kind) {
            ...
            case INDEX_CACHE_SOC:            supported = true; break;
...
MeterValueAvailability Meters::get_soc(uint32_t slot, float *soc, micros_t max_age)
{
    return get_single_value(slot, INDEX_CACHE_SOC, soc, max_age);
}
...
void Meters::declare_value_ids(uint32_t slot, const MeterValueID new_value_ids[], uint32_t value_id_count)
{
    ...
    meter_slot.index_cache_single_values[INDEX_CACHE_SOC]            = meters_find_id_index(total_value_ids, total_value_id_count, MeterValueID::StateOfCharge);
