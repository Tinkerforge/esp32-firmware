...
void WebServer::post_setup()
{
    ...
    if (config.transport_mode == HTTPD_SSL_TRANSPORT_SECURE) {
        ...
        if (result_https != ESP_OK) {
            httpd = nullptr;
            logger.printfln("Failed to start https web server: %s (0x%X)", esp_err_to_name(result_https), result_https);
            return;
        }
        ...
        if (result_http != ESP_OK) {
            httpd = nullptr;
            logger.printfln("Failed to start http web server: %s (0x%X)", esp_err_to_name(result_http), result_http);
            return;
        }
#if HTTPS_AVAILABLE()
    }
