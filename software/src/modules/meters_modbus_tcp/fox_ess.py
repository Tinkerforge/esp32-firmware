specs = [
    {
        'name': 'Fox ESS Hybrid Inverter',
        'register_type': 'HoldingRegister',
        'values': [
            {
                'name': 'Grid Voltage Phase R [0.1 V]',
                'value_id': 'VoltageL1N',
                'start_address': 31006,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'Grid Voltage Phase S [0.1 V]',
                'value_id': 'VoltageL2N',
                'start_address': 31007,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'Grid Voltage Phase T [0.1 V]',
                'value_id': 'VoltageL3N',
                'start_address': 31008,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'Grid Current Phase R [0.1 A]',
                'value_id': 'CurrentL1ImExDiff',  # FIXME: direction?
                'start_address': 31009,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'Grid Current Phase S [0.1 A]',
                'value_id': 'CurrentL2ImExDiff',  # FIXME: direction?
                'start_address': 31010,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'Grid Current Phase T [0.1 A]',
                'value_id': 'CurrentL3ImExDiff',  # FIXME: direction?
                'start_address': 31011,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'Grid Power Phase R [W]',
                'value_id': 'PowerActiveL1Export',
                'start_address': 31012,
                'value_type': 'U16',
            },
            {
                'name': 'Grid Power Phase S [W]',
                'value_id': 'PowerActiveL2Export',
                'start_address': 31013,
                'value_type': 'U16',
            },
            {
                'name': 'Grid Power Phase T [W]',
                'value_id': 'PowerActiveL3Export',
                'start_address': 31014,
                'value_type': 'U16',
            },
            {
                'name': 'Grid Frequency Phase R [0.01 Hz]',
                'value_id': 'FrequencyL1',
                'start_address': 31012,
                'value_type': 'U16',
                'scale_factor': 0.01,
            },
            {
                'name': 'Inverter Temperature [0.1 °C]',
                'value_id': 'Temperature',
                'start_address': 31031,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
        ],
    },
    {
        'name': 'Fox ESS Hybrid Inverter Grid',
        'register_type': 'HoldingRegister',
        'values': [
            {
                'name': 'Smart Meter Import Total [0.1 kWh]',
                'value_id': 'EnergyActiveLSumImport',
                'start_address': 32012,
                'value_type': 'U32BE',
                'scale_factor': 0.1,
            },
            {
                'name': 'Smart Meter Export Total [0.1 kWh]',
                'value_id': 'EnergyActiveLSumExport',
                'start_address': 32016,
                'value_type': 'U32BE',
                'scale_factor': 0.01,
            },
            {
                'name': 'Smart Meter Power Phase R [W]',
                'value_id': 'PowerActiveL1ImExDiff',
                'start_address': 31026,
                'value_type': 'S16',
                'scale_factor': -1.0,
            },
            {
                'name': 'Smart Meter Power Phase S [W]',
                'value_id': 'PowerActiveL1ImExDiff',
                'start_address': 31027,
                'value_type': 'S16',
                'scale_factor': -1.0,
            },
            {
                'name': 'Smart Meter Power Phase T [W]',
                'value_id': 'PowerActiveL1ImExDiff',
                'start_address': 31028,
                'value_type': 'S16',
                'scale_factor': -1.0,
            },
        ],
    },
    {
        'name': 'Fox ESS Hybrid Inverter Battery',
        'register_type': 'InputRegister',
        'values': [
            {
                'name': 'Battery Voltage [0.1 V]',
                'value_id': 'VoltageDC',
                'start_address': 31034,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'Battery Current [0.1 A]',
                'value_id': 'CurrentDCChaDisDiff',
                'start_address': 31035,
                'value_type': 'S16',
                'scale_factor': -0.1,
            },
            {
                'name': 'Battery Power [W]',
                'value_id': 'PowerDCChaDisDiff',
                'start_address': 31036,
                'value_type': 'S16',
                'scale_factor': -1.0,
            },
            {
                'name': 'Battery Temperature [0.1 °C]',
                'value_id': 'Temperature',
                'start_address': 31037,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'Battery State Of Charge [%]',
                'value_id': 'StateOfCharge',
                'start_address': 31038,
                'value_type': 'S16',
            },
            {
                'name': 'Battery Charge Energy Total [0.1 kWh]',
                'value_id': 'EnergyDCCharge',
                'start_address': 32004,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'Battery Discharge Energy Total [0.1 kWh]',
                'value_id': 'EnergyDCDischarge',
                'start_address': 32007,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
        ],
    },
    {
        'name': 'Fox ESS Hybrid Inverter Load',
        'register_type': 'HoldingRegister',
        'values': [
            {
                'name': 'Load Power Phase R [W]',
                'value_id': 'PowerActiveL1ImExDiff',
                'start_address': 31029,
                'value_type': 'S16',
                'scale_factor': -1.0,
            },
            {
                'name': 'Load Power Phase S [W]',
                'value_id': 'PowerActiveL1ImExDiff',
                'start_address': 31030,
                'value_type': 'S16',
                'scale_factor': -1.0,
            },
            {
                'name': 'Load Power Phase T [W]',
                'value_id': 'PowerActiveL1ImExDiff',
                'start_address': 31031,
                'value_type': 'S16',
                'scale_factor': -1.0,
            },
            {
                'name': 'Load Energy Total [0.1 kWh]',
                'value_id': 'EnergyActiveLSumImport',
                'start_address': 32022,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
        ],
    },
    {
        'name': 'Fox ESS Hybrid Inverter Backup Load',
        'register_type': 'HoldingRegister',
        'values': [
            {
                'name': 'EPS Voltage Phase R [0.1 V]',
                'value_id': 'VoltageL1N',
                'start_address': 31016,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'EPS Voltage Phase S [0.1 V]',
                'value_id': 'VoltageL2N',
                'start_address': 31017,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'EPS Voltage Phase T [0.1 V]',
                'value_id': 'VoltageL3N',
                'start_address': 31018,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'EPS Current Phase R [0.1 A]',
                'value_id': 'CurrentL1ImExDiff',  # FIXME: direction?
                'start_address': 31019,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'EPS Current Phase S [0.1 A]',
                'value_id': 'CurrentL2ImExDiff',  # FIXME: direction?
                'start_address': 31020,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'EPS Current Phase T [0.1 A]',
                'value_id': 'CurrentL3ImExDiff',  # FIXME: direction?
                'start_address': 31021,
                'value_type': 'S16',
                'scale_factor': 0.1,
            },
            {
                'name': 'EPS Power Phase R [W]',
                'value_id': 'PowerActiveL1ImExDiff',
                'start_address': 31022,
                'value_type': 'S16',
                'scale_factor': -1.0,
            },
            {
                'name': 'EPS Power Phase S [W]',
                'value_id': 'PowerActiveL1ImExDiff',
                'start_address': 31023,
                'value_type': 'S16',
                'scale_factor': -1.0,
            },
            {
                'name': 'EPS Power Phase T [W]',
                'value_id': 'PowerActiveL1ImExDiff',
                'start_address': 31024,
                'value_type': 'S16',
                'scale_factor': -1.0,
            },
            {
                'name': 'EPS Frequency Phase R [0.01 Hz]',
                'value_id': 'FrequencyL1',
                'start_address': 31015,
                'value_type': 'U16',
                'scale_factor': 0.01,
            },
        ],
    },
]
