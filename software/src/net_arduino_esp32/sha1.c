...
static uint32_t tf_sha1_transform(TF_SHA1 *sha1, const uint8_t buffer[TF_SHA1_BLOCK_LENGTH]) {
	uint32_t a, b, c, d, e;
	uint32_t block[TF_SHA1_BLOCK_LENGTH / 4];

	memcpy(&block, buffer, TF_SHA1_BLOCK_LENGTH);
