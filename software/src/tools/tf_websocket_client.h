...
typedef struct {
    ...
    void (*tls_configure)(void *ssl_config, void *ctx);     /*!< Optional Mbed TLS configuration hook, called after trust configuration. */
    void *tls_configure_ctx;                                /*!< Must remain valid for the client's lifetime. */
} tf_websocket_client_config_t;

/**
 * @brief      Start a Websocket session
 *             This function must be the first function to call,
 *             and it returns a tf_websocket_client_handle_t that you must use as input to other functions in the interface.
 *             This call MUST have a corresponding call to tf_websocket_client_destroy when the operation is complete.
 *
 * @param[in]  config  The configuration
 *
 * @return
 *     - `tf_websocket_client_handle_t`
 *     - NULL if any errors
 */
 ...
 * using tf_websocket_client_close().
 *
 *  Notes:
 ...
 *             It is the opposite of the tf_websocket_client_init function and must be called with the same handle as input that a tf_websocket_client_init call returned.
 ...
 * * Client is stopped the same way as by the `tf_websocket_client_stop()`
 *
 *  Notes:
 *  - Cannot be called from the websocket event handler
 *
 ...
 *             Closing sequence is the same as for tf_websocket_client_close()
 *
 *  Notes:
 *  - Cannot be called from the websocket event handler
 *
