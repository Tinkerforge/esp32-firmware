...
let x = {
    "evse": {
        ...
        "content": {
            ...
            "error_state_help": <>
                <ul>
                    <li><p><strong>Switch error:</strong> The charger was not installed correctly. The charge controller's switch setting remains at its factory default.{options.WARP_DOC_BASE_URL.length > 0 ? <span> See <a href={`${options.WARP_DOC_BASE_URL}/en/docs/warp_charger/assembly_and_installation#einstellen-des-ladestroms`}>Charging current configuration</a>.</span> : undefined}</p></li>
                    ...
                    <li><p><strong>Communication error:</strong> An error occurred during communication with the vehicle. On first occurrence, disconnect the charging cable from the vehicle, wait 10 seconds, and then reconnect the cable (restart the charging process).{options.WARP_DOC_BASE_URL.length > 0 ? <span> If the problem persists, see the <a href={`${options.WARP_DOC_BASE_URL}/en/docs/warp_charger/troubleshooting#led-blinkt-f%C3%BCnfmal-rot-im-intervall-dass-webinterface-zeigt-kommunikationsfehler`}>instructions</a>.</span> : undefined}</p></li>
                </ul>
            </>,
            "error_ok": "OK",
            "error_switch": "Switch",
        ...
        "automation": {
            ...
            "automation_led_action_text": /*FFN*/(indication_number: number, indication_text: string, duration: number, color: string) => {
                let c = color == "" ? "" : <>in <span class="px-2 me-1 border" style={"background-color: " + color}></span></>
                if (indication_number == 0)
                    return <>turn the status LED <b>{indication_text}</b> for <b>{toLocaleFixed(duration / 1000)} seconds</b>.</>
