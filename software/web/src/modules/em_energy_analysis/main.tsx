...
const METERS_MAX_SLOTS_RECORDED = Math.min(options.METERS_MAX_SLOTS, 7);
...
interface EnergyManager5minData extends CachedData {
    ...
    power_sum: number[/*location*/][/*timestamp_slot*/]; // W
    power_sum_empty: boolean[/*location*/];
...
//#endif
    ...
    constructor() {
        ...
        for (let meter_slot = 0; meter_slot < METERS_MAX_SLOTS_RECORDED; ++meter_slot) {
...
//#endif
    ...
    update_uplot_5min_status_cache(date: Date) {
        ...
        if (energy_manager_data) {
            ...
            if (!energy_manager_data.power_sum_empty[MeterLocation.Grid]) {
                timestamp_slot_count = Math.max(timestamp_slot_count, energy_manager_data.power_sum[MeterLocation.Grid].length);

                uplot_data.keys.push('em_power_sum_grid');
                uplot_data.names.push(translate_meter_location(MeterLocation.Grid));
                uplot_data.values.push(energy_manager_data.power_sum[MeterLocation.Grid]);
                uplot_data.stacked.push(false);
                uplot_data.paths.push(UplotPath.Line);
            }

            if (!energy_manager_data.power_sum_empty[MeterLocation.Battery]) {
                timestamp_slot_count = Math.max(timestamp_slot_count, energy_manager_data.power_sum[MeterLocation.Battery].length);

                uplot_data.keys.push('em_power_sum_battery');
                uplot_data.names.push(translate_meter_location(MeterLocation.Battery));
                uplot_data.values.push(energy_manager_data.power_sum[MeterLocation.Battery]);
                uplot_data.stacked.push(false);
                uplot_data.paths.push(UplotPath.Line);
            }

            if (!energy_manager_data.power_sum_empty[MeterLocation.Load]) {
                timestamp_slot_count = Math.max(timestamp_slot_count, energy_manager_data.power_sum[MeterLocation.Load].length);

                uplot_data.keys.push('em_power_sum_load');
                uplot_data.names.push(translate_meter_location(MeterLocation.Load));
                uplot_data.values.push(energy_manager_data.power_sum[MeterLocation.Load]);
                uplot_data.stacked.push(false);
                uplot_data.paths.push(UplotPath.Line);
            }
        }
    ...
    update_uplot_daily_cache(date: Date) {
        ...
        let energy_manager_data = this.energy_manager_daily_cache[key];
        let energy_manager_previous_data = this.energy_manager_daily_cache[previous_key];
        let energy_import_5min_total = new Array(METERS_MAX_SLOTS_RECORDED);
        let energy_import_daily_total = new Array(METERS_MAX_SLOTS_RECORDED);
        let energy_export_5min_total = new Array(METERS_MAX_SLOTS_RECORDED);
        let energy_export_daily_total = new Array(METERS_MAX_SLOTS_RECORDED);
        ...
        if (!this.energy_manager_5min_cache_update_promise[key]) {
            let promise = new Promise<boolean>(async (resolve, reject) => {
                ...
                let data: EnergyManager5minData = {
                    ...
                    complete: key < this.date_to_5min_key(new Date(now)),
                    flags: new Array(timestamp_slot_count),
                    power: new Array(METERS_MAX_SLOTS_RECORDED),
                    power_empty: new Array(METERS_MAX_SLOTS_RECORDED),
        ...
        if (!this.energy_manager_daily_cache_update_promise[key]) {
            let promise = new Promise<boolean>(async (resolve, reject) => {
                ...
                let data: EnergyManagerDailyData = {
                    ...
                    empty: true,
                    complete: key < this.date_to_daily_key(new Date(now)),
                    energy_import: new Array(METERS_MAX_SLOTS_RECORDED),
                    energy_export: new Array(METERS_MAX_SLOTS_RECORDED),
    ...
    render(props: {}, state: Readonly<EMEnergyAnalysisState>) {
        ...
        return (
            <SubPage name="em_energy_analysis" colClasses="col-xl-10">
                <PageHeader title={__("em_energy_analysis.content.em_energy_analysis")} childrenColClass="col-12 col-sm-auto align-content-end">
                    {this.state.data_type == '5min'
                    ? <InputDate date={this.state.current_5min_date} onDate={(date) => this.set_current_5min_date(date)} buttons="day" style="width: 10.5rem">{data_select}</InputDate>
                    : <InputMonth date={this.state.current_daily_date} onDate={(date) => this.set_current_daily_date(date)} buttons="month" style="width: 10.5rem">{data_select}</InputMonth>}
                </PageHeader>
                ...
                <div class="pb-3">
                    <div style="position: relative;"> {/* this plain div is necessary to make the size calculation stable in safari. without this div the height continues to grow */}
                        <UplotLoader ref={this.uplot_loader_5min_ref}
                            ...
                            <UplotWrapperB ref={this.uplot_wrapper_5min_power_ref}
                        ...
                        <UplotLoader ref={this.uplot_loader_daily_ref}
                                        ...
                                        no_data={__("em_energy_analysis.content.no_data")}
                                        loading={__("em_energy_analysis.content.loading")} >
                            <UplotWrapperB ref={this.uplot_wrapper_daily_ref}
                                            class="em-energy-analysis-chart"
                                            sub_page="em_energy_analysis"
