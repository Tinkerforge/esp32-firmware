            ...
            get_edit_children: (battery_slot: number, config: ModbusTCPBatteriesConfig, on_config: (config: ModbusTCPBatteriesConfig) => void): ComponentChildren => {
                    ...
                    <FormRow label={__("batteries_modbus_tcp.content.table")}>
                        <InputSelect
                            ...
                            items={[
                                ...
                                [BatteryModbusTCPTableID.SMAHybridInverter40793.toString(), __("batteries_modbus_tcp.content.table_sma_hybrid_inverter_40793")],
                                [BatteryModbusTCPTableID.SMAHybridInverter41467.toString(), __("batteries_modbus_tcp.content.table_sma_hybrid_inverter_41467")],
                ...
                if (util.hasValue(config[1].table)
                 && (config[1].table[0] == BatteryModbusTCPTableID.VictronEnergyGX
                  ...
                  || config[1].table[0] == BatteryModbusTCPTableID.SMAHybridInverter40793
                  || config[1].table[0] == BatteryModbusTCPTableID.SMAHybridInverter41467)) {
                    if (config[1].table[0] == BatteryModbusTCPTableID.SMAHybridInverter40793) {
                        edit_children.push(
                            <FormRow label="">
                                <Alert variant="warning" className="mb-0">{__("batteries_modbus_tcp.content.sma_forecast_based_charging_warning")}</Alert>
                            </FormRow>);
                    }
                    else if (config[1].table[0] == BatteryModbusTCPTableID.SMAHybridInverter41467) {
                        edit_children.push(
                            <FormRow label="">
                                <Alert variant="warning" className="mb-0">{__("batteries_modbus_tcp.content.sma_energy_management_warning")}</Alert>
                            </FormRow>);
                    }
                    ...
                    if (config[1].table[0] == BatteryModbusTCPTableID.VictronEnergyGX) {
                        edit_children.push(
                            <FormRow label={__("batteries_modbus_tcp.content.grid_draw_setpoint")} label_muted={__("batteries_modbus_tcp.content.grid_draw_setpoint_muted_normal")}>
                                <InputNumber
                                    required
                                    min={-2147483648}
                                    max={2147483647}
                            ...
                            <FormRow label={__("batteries_modbus_tcp.content.grid_draw_setpoint")} label_muted={__("batteries_modbus_tcp.content.grid_draw_setpoint_muted_force_discharge")}>
                                <InputNumber
                                    required
                                    min={-2147483648}
                                    max={0}
                    ...
                    else if (config[1].table[0] == BatteryModbusTCPTableID.SMAHybridInverter40793) {
                        edit_children.push(
                            <FormRow label={__("batteries_modbus_tcp.content.max_normal_charge_power")}>
                                <InputNumber
                                    required
