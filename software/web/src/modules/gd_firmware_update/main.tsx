...
//type FirmwareUpdateConfig = API.getType['evse/hardware_configuration'];
...
export class FirmwareUpdate extends Component<{}, FirmwareUpdateConfig> {
    ...
    async checkGDFirmware(f: File) {
        try {
            await util.upload(f.slice(0x0, 0xd000), "check_gd_firmware", () => {})
        } catch (error) {
    ...
    render(props: {}, state: Readonly<FirmwareUpdateConfig>) {
        ...
        return (
            <>
                ...
                <FormRow label={__("firmware_update.content.gd_firmware_update_label")} label_muted={__("firmware_update.content.gd_firmware_update_desc")}>
                    <InputFile
                        browse={__("firmware_update.content.browse")}
                        select_file={__("firmware_update.content.gd_select_file")}
                        upload={__("firmware_update.content.update")}
                        url="/write_gd_firmware"
                        accept=".bin,.hex"

                        timeout_ms={120 * 1000}
                        onUploadStart={async (f) => this.checkGDFirmware(f)}
                        onUploadSuccess={() => util.postReboot(__("firmware_update.script.flash_success"), __("util.reboot_text"))}
                        onUploadError={error => {
                            if (typeof error === "string") {
                                util.add_alert("firmware_update_failed","alert-danger", __("firmware_update.script.flash_fail"), error);
