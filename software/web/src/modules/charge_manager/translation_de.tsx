...
let x = {
    "charge_manager": {
        ...
        "script": {
            ...
            "zpd_yes_waiting_for_rotation": /*SFN*/ (timespan: string) => `Warte auf nächste Rotation: ${timespan}` /*NF*/,
            "zpd_yes_not_active": "TODO not active",
            "zpd_yes_rotated_for_b1": "Deaktiviert um andere wartende Wallbox zu aktivieren",
            "zpd_yes_rotated_for_higher_prio": "Deaktiviert um Wallbox mit höherer Priorität (durch Lademodus) zu aktivieren",
            "zpd_yes_phase_overload": /*SFN*/ (phase: string, overload: number) => `Phase ${phase} um ${overload} mA überlastet` /*NF*/,
            "zpd_yes_pv_excess_overload": /*SFN*/ (overload: number) => `PV-Überschuss um ${overload} mA überlastet` /*NF*/,
            "zpd_no_cloud_filter_blocks_until": /*SFN*/ (timespan: string, overload: number) => `PV-Überschuss überlastet, aber Wolkenfilter blockiert für ${timespan}` /*NF*/,
            "zpd_no_hysteresis_blocks_until": /*SFN*/ (timespan: string) => `PV-Überschuss überlastet, aber Schalt-Hysterese blockiert für ${timespan}` /*NF*/,

            "opd_yes_welcome_charge_until": /*SFN*/ (timespan: string) => `Willkommensladung für ${timespan}` /*NF*/,
            "opd_no_phase_minimum": /*SFN*/ (timespan: string, phase: string, required: number, minimum: number) => `Würde Phase ${phase} überlasten: required ${required} mA, seen minimum ${minimum} mA (${timespan})` /*NF*/,
            "opd_no_phase_improvement": /*SFN*/ (timespan: string, phase: string, allocable: number, minimum: number) => `Auf Phase ${phase} verfügbarer Strom  (Minimum ${minimum} mA für ${timespan}) kann auf bereits aktive Wallboxen (max ${allocable}) verteilt werden.` /*NF*/,
            "opd_no_force_3p_until": /*SFN*/ (timespan: string) => `Wegen kürzlicher Phasenumschaltung nur dreiphasiges Laden erlaubt für ${timespan}` /*NF*/,
            ...
            "opd_yes": "TODO yes",
            "opd_no_hysteresis_blocks_until": /*SFN*/ (timespan: string) => `Schalt-Hysterese blockiert für ${timespan}` /*NF*/,
            "opd_yes_switched_to_fixed_1p": "Wallbox hat ein einphasig ladendes Fahrzeug detektiert",

            "tpd_yes_welcome_charge_until": /*SFN*/ (timespan: string) => `Willkommensladung für ${timespan}` /*NF*/,
            "tpd_no_phase_minimum": /*SFN*/ (timespan: string, phase: string, required: number, minimum: number) => `Würde Phase ${phase} überlasten: required ${required} mA, seen minimum ${minimum} mA (${timespan})` /*NF*/,
            "tpd_no_phase_improvement": /*SFN*/ (timespan: string, phase: string, allocable: number, minimum: number) => `Auf Phase ${phase} verfügbarer Strom  (Minimum ${minimum} mA für ${timespan}) kann auf bereits aktive Wallboxen (max ${allocable}) verteilt werden.` /*NF*/,
            "tpd_no_force_1p_until": /*SFN*/ (timespan: string) => `Wegen kürzlicher Phasenumschaltung nur einphasiges Laden erlaubt für ${timespan}` /*NF*/,
            "tpd_no_fixed_1p": "Wallbox ist einphasig angeschlossen und unterstützt keine Phasenumschaltung",
            ...
            "tpd_yes": "TODO yes",
            "tpd_no_hysteresis_blocks_until": /*SFN*/ (timespan: string) => `Schalt-Hysterese blockiert für ${timespan}` /*NF*/,
            "tpd_yes_switched_to_fixed_3p": "Wallbox hat ein dreiphasig ladendes Fahrzeug detektiert",

            "cd_minimum": "Minimalstrom",
            "cd_enable_not_charging": "Einschaltstrom; Fahrzeug lädt nicht",
            "cd_phase_limit": "Phasenlimit",
