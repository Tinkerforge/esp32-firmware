...
def parse_lines(input_path, input_lines, defines, ifs_elses):
    output_lines = []
        ...
        if m != None:
            ...
            if verb == 'include':
                ...
                if not any_zero(ifs_elses):
                    ...
                    with include_path.open(encoding='utf-8') as include_file:
                        parse_lines(include_path, include_file.readlines(), defines, ifs_elses)
            elif verb == 'define':
...
def tfpp_lines(input_path, input_lines):
    defines = {}
    ifs_elses = []
    output_lines = parse_lines(pathlib.Path(input_path), input_lines, defines, ifs_elses)
...
def tfpp_paths(input_path, output_path, overwrite=False):
    ...
    with input_path.open(encoding='utf-8') as input_file:
        output_lines = tfpp_lines(input_path, input_file.readlines())
...
def main():
    ...
    tfpp_paths(args.input_path, args.output_path, overwrite=args.overwrite)
